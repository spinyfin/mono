...
Host facts here were verified against the live hosts (`ssh empiricist` / `ssh zoologist` / `ssh diziet` as the `bduff` user) and the live Buildkite agent registrations (`bk agent list`) on 2026-07-27, and the power/boot facts in "Power loss and unattended recovery" on 2026-09-02/2026-09-03, except where explicitly noted as unverified. There is **no passwordless sudo** on any of these hosts for the `bduff` account, so anything that requires root is called out explicitly in "Needs operator input" at the end rather than guessed.
...
Every modern x86 BIOS/UEFI has a setting that decides what the machine does when AC power returns after a loss: stay off, power on, or resume whatever state it was last in. **This setting could not be read from any of the three Linux hosts over SSH — there is no supported OS-level read path for it (see "No BMC" below) — so its current value is not stated here as a fact.** What can be stated as fact is the observed behavior, and it differs by host: `empiricist` and `zoologist` were up and running until the outage itself (their journals stop at 2026-08-23 01:15:22 and 01:11:31 respectively — see the boot table above), AC returned at a known time, and neither powered back on. That is consistent with their AC-restore policy being set to something other than "power on" (or with a firmware bug that produces the same symptom), but it is an inference from behavior, not a read value. `diziet` is weaker evidence for the same inference: it had already been off since 2026-08-21 for unrelated, undiagnosed reasons (see the open question above), so unlike the other two hosts its state immediately before the outage was never verified as healthy. An already-off machine should still power on at AC restore under a "Power On" policy, so `diziet`'s failure to come back is still consistent with the same non-"power on" policy — it just isn't independent confirmation of it the way `empiricist` and `zoologist` are.
...
- It starts fast and unattended: on `diziet`, `journalctl` records its boot-log start at 19:09:52 and `Started buildkite-agent.service` at 19:10:00 — 8 seconds after the kernel starts logging, not 8 seconds after power-on. (`journalctl`'s boot timeline begins at kernel start, so it excludes the 25.8s firmware and 2.3s loader phases in the `systemd-analyze` breakdown below — those happen before the kernel, and thus before `journalctl`, is even running.) That 8-second figure is comfortably inside the 47.8s total boot time, i.e. the agent is up well before userspace finishes settling, with nobody logged in.
...
2. If it's offline, it did not power back on — Tailscale needs a running OS to phone home, so there is no in-band way to reach a powered-off host. The only remedy is physical access: press the power button, or wait for the next scheduled physical check.
3. Once the host is online (whether that's because it was already up, or because you just powered it on physically), run `journalctl --list-boots` and compare the most recent boot's start time against when you expect it to have come back (e.g. against a known AC-restore time), to confirm the outage gap. **`last` is not installed on any of the three hosts — do not reach for it.** `journalctl --list-boots` is the tool that actually works here.
4. If the host is up but the agent isn't registered in `bk agent list`, check `systemctl status buildkite-agent.service` and the unit's journal (`journalctl -u buildkite-agent.service`) for why it failed to start, rather than assuming a power problem.
5. Anything requiring `sudo` needs a human physically or interactively present: all three hosts prompt for a password for the `bduff` account, and there is **no passwordless sudo** on any of them (consistent with the note at the top of this doc).
...
- **Reading, and if needed setting, the firmware AC-restore policy on all three hosts** (see "The firmware AC-restore policy is a fleet invariant" above) — this requires physical presence at each machine's BIOS setup menu; there is no OS-level or remote read/write path. Use the per-vendor menu paths documented there: NUC (`diziet`, `zoologist`) via `F2` at power-on → Power → Secondary Power Settings → "After Power Failure"; Beelink SER (`empiricist`) via `Del`/`F7` at power-on → "Restore AC Power Loss" / "State After G3". Set all three to **Power On**.
- **Optionally, updating `diziet`'s BIOS to match `zoologist`'s** (see "Known inconsistency: BIOS version drift" above) — also requires physical presence; this closes the firmware-version gap between the two otherwise-identical NUC11PABi5 boards but is not required to fix the AC-restore behavior itself.
