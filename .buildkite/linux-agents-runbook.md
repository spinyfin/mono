...
Per `.buildkite/README.md` ("Pushing from CI"), the Linux Buildkite agents are bootstrapped with a **read-only** deploy key/credential — pulls work, but any `git push` from a job that lands on a Linux agent is denied. This was not independently re-verified from the host in this pass (the `buildkite-agent` user's `~/.ssh` is not readable by `bduff` and there's no sudo to check it — see "Needs operator input"), but it's load-bearing for `.buildkite/steps/checkleft-release.sh`, whose `prepare` phase pushes a release tag with the agent's ambient git credential: that step flaps between success and a `denied to deploy key` failure purely based on whether it happens to land on a Mac (write-capable, personal identity) or a Linux agent (read-only) within the shared `bazel-any` queue. See the README's "Pushing from CI" section for the full detail and the standing gap (no scoped `GITHUB_TOKEN` push path yet).
...
- **The `buildkite-agent` user's SSH/deploy-key material** (to directly confirm the read-only posture described in "Deploy-key posture" from the host side, rather than relying solely on `.buildkite/README.md`): `sudo ls -la /var/lib/buildkite-agent/.ssh/` and `sudo cat /var/lib/buildkite-agent/.ssh/config` (if present).
