...
The two inputs above are not peers. `LiveWorkerState.activity` is fed event-by-event from the worker's own hook stream and is the authority on moment-to-moment activity. `work_executions.status` is the durable projection of it, and it is what every other consumer reads — dispatch admission, the liveness sweeps, the reaper, attention surfacing, the kanban's fallback path, and every CLI/JSON reader. Revision completion is deliberately not inferred from either liveness view: the merge poller retains a staged PR URL until the worker's own turn boundary, and recovery finalizes only from the exact contributed head that boundary recorded. A projection does not get to contradict what it projects, and liveness does not double as a completion signal.
