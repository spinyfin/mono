...
- **Status:** Implemented — all 15 project tasks done (design #1791 merged 2026-07-13; implementation complete 2026-07-14). **[Postmortem](#postmortem-2026-07-19-updated-2026-07-24) added 2026-07-19**, covering as-built reality, decisions that diverged, and the production-hardening follow-up family. (The P2264 project row still read `planned` at postmortem time — stale bookkeeping, corrected via the postmortem's follow-ups; every task is in fact done.) The postmortem is kept current by later review passes, folded in place rather than appended:
  - **2026-07-21** (input: #2154, the postmortem PR itself) — spot-checked the postmortem's PR citations against the PR record. One citation (the Era-A trigger incident's PR number) did not check out and is corrected in place; the T4 row's scope (rung 1 only, not rung 0+1) is corrected in place. No other divergence.
  - **2026-07-24** (input: #2275) — a fourth cause of rung 0's zero live successes surfaced and was fixed: `cube workspace push` never described the resolution commit, so `jj git push` refused every rung-0 landing outright. Recorded as [Era D](#the-hardening-tail-2026-07-14--07-24); `LadderOutcome::FellThrough` gained a `reason` field in the same PR, correcting a routing verdict that had been misreporting exactly this failure.
...
## Postmortem (2026-07-19, updated 2026-07-24)

- **Date:** first written 2026-07-19, six days after the design landed; extended in place by the review passes listed in the status bullet above (most recently 2026-07-24).
- **Provenance:** execution `exec_18c3e7eb568e5a48_e9b` (investigation), per the design-postmortem mechanism (T2935); the 2026-07-24 pass is `exec_18c54f7df448f8d8_b0`.
- **Scope:** as-built reality vs. this design — the project's own 15 tasks, **plus the fix rows and their satellites filed outside the project** from 2026-07-14 onward. The follow-up family is the core of this postmortem: the build took about 22 hours; making it production-real has taken every day since.
- **Sources:** the merged PR record (bodies and diffs of mono #1957–#2139 and #2275, and the flunge PRs cited below), the engine source as of this writing, and operator-reported engine telemetry where noted. Claims are attributed throughout.

### TL;DR

The design was implemented fast and faithfully: all 15 tasks landed as 14 PRs plus one data insert within ~22 hours of the design merging (2026-07-13 20:50Z → 2026-07-14 19:01Z). Structurally, what shipped is what was designed — the rung 0–3 ladder, the resolver-registry crate, the telemetry columns, the hotspot report, `merge_order` edges, speculative prediction, recipes. What the days since revealed is that the design specified **rungs and gates, but not a lifecycle or an environment**: rung 0 went to production gated off with no alarm and had still not recorded a confirmed live success eleven days later, under four successive causes (a compile-time gate, a path-parsing mismatch, a non-hermetic workspace environment, and — through 07-24 — a landing step that pushed a descriptionless commit `jj` refuses outright); transient infrastructure errors were conflated with genuine declines and bought full LLM revisions; an engine restart vaporized an in-flight attempt, leaked its workspace lease for cube's full 30-minute TTL, and left the conflict stranded; and two worker "success" claims had no verifier behind them. Every one of these defects degraded to either the expensive-but-correct rung-3 path or a stuck-but-safe wedge — **no ladder defect ever produced a wrong merge** — and each was root-caused and fixed within one to two days of surfacing. Separately, six days after this design rejected merge queues, flunge adopted the Trunk merge queue (P2951) — for a failure class this design's rejection never considered. The positions are compatible and now compose; see [the reconciliation](#the-merge-queue-reversal-p2951-reconciled).
...
| 07-24            | **Era D closed.** A lockfile-only conflict on flunge #1108 (trace `crz_18c53198f6f70eb0_9e`) is resolved by rung 0 in full, then refused by `jj git push` for having no commit description; the attempt falls through to a worker and the routing verdict misreports it as "rung 0 did not resolve every residual file". #2275 fixes both: `cube workspace push` describes an undescribed `@` before pushing, and `LadderOutcome::FellThrough` carries the true per-site reason. |
...
- **"Make the common conflict free" (rung 0).** Shipped, and live today — but **not yet demonstrably successful even once.** It took until 07-15 to be turned on, until 07-17 to be able to _match_ a resolver in a real workspace, and until 07-24 to be able to _land_ what it resolved (four successive zero-success causes, below). Operator-reported lifetime telemetry stood at **0 of 42 live attempts through 07-16** (engine telemetry, not the PR record; #2098's body independently states the bazel resolver had "a 0% live success rate"). The first post-Era-C live attempt on record (07-18) was killed mid-rung by the engine restart; the next one on record (07-24, flunge #1108) resolved every file and then failed to push. The PR record still contains **no confirmed live rung-0 success** — this remains the single most important open fact about the project, and it took four separate live incidents rather than any monitoring to get this far (see theme 3).
...
- **The ladder's outcome vocabulary grew a cause, not just a verdict.** The design treated fall-through as a single routing decision; as-built `LadderOutcome::FellThrough` carries `{ residual_conflict_files, reason }`, where `reason` is the specific cause captured at the failure site (`&'static str`) rather than a message reconstructed by the caller (#2275). Routing still keys only on `residual_conflict_files` via `rung2_eligible` — the `reason` exists purely so the canonical `conflict_ladder: routing verdict` line tells the truth. This is the outcome-vocabulary lesson of theme 4 applied a second time: the ladder's enum has to be able to say _why_, not only _what next_.
- **`cube workspace push` owns the landing contract, not the resolver.** The design left "it commits, pushes" unelaborated. As-built the cube verb is the single place that knows what `jj git push` requires of the commit it is handed: a non-empty description (#2275) and a passing checkleft push gate (T2576), on top of the compare-and-swap on the tracked bookmark. Rung 0's resolvers stay pure file edits and describe nothing; every landing precondition lives in the verb — the same consolidation `cube pr update` got for the worker path when #2057 taught it to refuse a detached push.

### The hardening tail (2026-07-14 → 07-24)

Fifteen fix rows plus satellites in the first six days, and a sixteenth on 07-24 — all filed outside the project. Grouped by what they revealed.

**Rung 0's four eras of zero.** Why has the design's highest-conviction feature never recorded a live success? Four successive causes, each masking the next, each found by a live incident rather than by monitoring:
...
- **Era D — the undescribed commit (#2275, 07-24).** With gating, matching and the environment all fixed, rung 0 finally resolved a real conflict end to end — a lockfile-only conflict on flunge #1108 (an extraction of `backend/dates`, exactly the `Cargo.lock`-churning shape rung 0 exists for) — and then could not land it. Deterministic resolvers edit the conflicted files in the working copy and never run `jj describe`; `cube workspace push` advanced the bookmark and pushed without describing `@`; `jj git push` refuses a descriptionless commit outright (`Won't push commit … since it has no description`). Every rung-0 attempt that got as far as pushing died there, and the attempt fell through to a full worker for a conflict it had already resolved. Fix: `workspace_push` reads `@`'s description first and stamps a deterministic one (`Resolve merge conflict on PR #<n> (<branch>)`, or the branch-only form when no PR number is known) when it is blank, leaving any existing description — a human's, or one an earlier step set — untouched. Note the shape: this is the same class as Era B (a tool's actual input/output contract differing from the one the code assumed), and like Era B it was invisible to every unit test because the fake runner had no opinion about descriptions until #2275 gave it one.
...
2. **"Deterministic" ≠ hermetic.** The central bet — formula beats agent — was right about the formula and silent about the formula's _environment_. `bazel mod deps` needs a gitignored xcframework; the push gate needs a modern Node; the registry needs bare paths, not jj's annotated ones; `jj git push` needs a described commit. Four independent assumptions about the surrounding tooling, each producing a 100% failure rate in exactly the automated contexts (cold workspaces, engine-context pushes) that never occur on a developer's machine — where a human has always already run `jj describe`, and where the xcframework and a modern Node are simply present. A deterministic resolver's contract must include its environmental preconditions — provisioned (setup.yaml stubs) or preflighted (version gates) — and an environment failure must surface as `Failed`, never as a decline. The generalisation Era D adds: this applies to the _landing_ step as much as the resolving step. Rung 0 is only free if the whole path from conflicted file to pushed head is automated, and every hop on that path has preconditions a human satisfies without noticing.
3. **The measurement project shipped its own mechanism dark.** This doc says "measurement is a deliverable, not an afterthought" — yet the ladder's own decision points were uninstrumented: the flag-off skip logged at debug, the compile gate invisible at runtime, resolver invocations traceless, and a 0-of-42 lifetime success count alarming no one for days. The design measured _conflicts_ diligently and its _own feature_ not at all. Anything shipped dark needs an INFO-level decision line at every terminal branch from day one, and an expiry on the darkness — a gate without an alarm becomes a forgotten switch. **This theme's own remedy was never completed**, which is why Era D took five more days to surface: the INFO trace lines landed (#2043), but no counter, threshold, or alarm ever did, so "rung 0 has still never succeeded" remained something a human had to go looking for. Four eras, four live incidents, zero proactive detections.
...
5. **Reporting fidelity is a correctness surface.** Rung-1 residue enumeration looked only at the working-copy tip while the push refuses the whole range for any conflicted ancestor; decline logs blamed "no resolver" for resolver-ran-and-declined cases. Downstream routing — rung-2 eligibility, diagnosis paths — consumed those reports as truth. Where a report drives routing, its fidelity is load-bearing and deserves tests as such. Era D showed the failure mode recurring one level up, in the instrumentation added _as this list's remedy_: the canonical routing verdict reconstructed its explanation at the call site from a two-branch `if RUNG0_APPLY_LIVE`, so a resolved-but-unpushed result was reported as "rung 0 did not resolve every residual file" — a diagnosis pointing at the resolvers when the resolvers had worked perfectly. A verdict line assembled by the caller from what it can see is a guess; only the failure site knows the cause, which is why `reason` now travels with the outcome. Every `FellThrough` assertion in `conflict_ladder_tests.rs` now pins the reason string too, so a future mislabel fails a unit test rather than a live incident.
...
- **Live** (where the operator has enabled the deliberately default-off `conflict_ladder_mechanical_rebase` flag): rung-1 engine-direct rebase; rung-0 apply (`RUNG0_APPLY_LIVE = true` since #2043) with the `cargo_lock`, `bazel_module_lock`, and `registry_append_union` resolvers via `ResolverRegistry::with_builtins()`, landing through a `cube workspace push` that now describes its own resolution commit (#2275); rung-2 single-file Sonnet/`trivial`; tripwire halt-for-sign-off on rungs 0/1; restart survival (durable in-flight marker, boot reconcile, lease registry/reap/heartbeat).
- **Fixed 2026-07-29 — replay-only rebase conflicts (the fifth era).** Rung 1 was reached and ran, but almost never retired: 7 of 281 ladder-era `conflict_resolutions` attempts resolved mechanically, and 91 of the 107 attempts carrying a trustworthy diagnosis had a _completely clean_ `git merge-tree` yet still spawned an agent. Cause: a rebase replays commits individually while GitHub and the `git merge-tree` diagnosis merge final trees, so a branch that touched a region and later rewrote or reverted it left an **ancestor** of the branch head conflicted while the head's rebased tree was already correct. `jj git push` refuses a range containing any conflicted commit, so `cube workspace rebase` reported `REBASED_WITH_CONFLICTS` for a branch needing no decision at all — and because the flagged commits dedupe to one path, the residue was almost always exactly one file, which `rung2_eligible` routed straight to a small agent (that is why rung-2 stamps outnumber every other outcome ~3:1). `cube workspace rebase` now collapses replay-only conflicted ancestors forward and reports `linearized_commits` on its payload, but only after proving the pushed tree byte-identical to the rebased head; a conflicted head, a bookmarked ancestor (stacked-PR boundary), a fork, or any tree drift still declines and hands off unchanged. Every decline now records _which_ guard refused, as a stable slug on the payload (`linearize_decline`) that the ladder stamps on its rung-1 fall-through trace, so "a real conflict at the head" can be told from "the collapse ran and the tree drifted" without reading code — the rung-health gap below is about a missing _alarm_, and this at least makes the underlying counts readable. Collapsed commits' messages are not lost: each is captured before its squash and appended to the surviving head's description via `jj describe -m`. Rollback of a declined attempt uses `jj op undo` on exactly the operations cube itself issued, never `jj op restore`: cube workspaces are secondary jj workspaces sharing one repo and one operation log, so a repo-global restore would revert every concurrent sibling worker's bookmarks, working-copy pointers, and fetches. This era was again found by hand, not by an alarm — see the rung-health bullet below, which it re-confirms rather than resolves.
...
- **Shipped but unwired:** declarative recipes — `RecipeResolver`/`ConflictRecipesStore` (#1975) still has no live consumer; `attempt_rung0` builds `with_builtins()` only and nothing loads `conflict-recipes.toml`. Unchanged since 07-19, and now filed as its own follow-up rather than left as a standing observation in this doc.
- **Missing, and load-bearing:** any alarm on rung 0's own health. Theme 3's remedy stopped at INFO trace lines; there is no rung-outcome counter in `metrics_init.rs` and no threshold on "N live attempts, zero successes", which is why each of the four eras cost a live incident to discover. Also filed as a follow-up.
...
- **Unknowns the telemetry should answer next:** whether rung 0 can land a resolution at all now that Era D is closed (still no confirmed live success in the record — eleven days and four fixes in), the true conflict rate vs. the ~12% floor, and whether `merge_order` and hotspot refactoring move frequency at all.
