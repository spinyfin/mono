...
Model selection, effort selection, and routing are settled. There is no size classifier, routing pre-pass, escalation, or user-facing low/high selector. This feature does not replace automated PR review, create approval verdicts, execute tests during generation, publish repository markdown or unsolicited GitHub comments, add another merge implementation, or bulk-backfill historical PRs. An operator can explicitly generate one existing PR's guide from its card, including merged or closed PRs. It does not change models for implementation or comment-answer agents, and does not add a general document version-control product.
...
Protocol additions provide `GetReviewGuideSummary` and `GetReviewGuideContent`, `GenerateReviewGuide` (capture if needed, then idempotent generation), `RetryReviewGuide` (retry an already selected comparison), and a guide-state invalidation event. Board/task detail replies include only summary metadata, not full Markdown. Follow the existing request-ID/event pattern, publish only after durable writes, and re-fetch summaries on reconnect. The document viewer's existing singleton must carry an explicit content identity so a late response for a previously opened PR cannot overwrite the current document.
...
Use `ExecutionKind::PrReviewGuide` plus a bound attempt, following the answer-agent precedent of an execution without a `tasks` row. Route it through the review pool alongside `pr_review` executions, not the automation pool — a guide is part of the PR review flow, not automation-originated activity, so it is claimed under the same pool and pause rules as reviews and is never held by an automation pause. It keeps its own fixed driver/model policy (`codex`/`gpt-6-astra`), overriding the review pool's usual model pin. Do not create another pool or send it through PR-review batch admission. Extend launch lookup, synthetic work context, closed-owner checks, resume/recovery, and finalization for the new binding; otherwise code that assumes `work_item_id` is a task will strand these runs. Implementation feedback continues to use the normal revision queue and serializing dependencies.
...
Add `WorkerKind::ReviewGuide` to the existing driver/worker-policy integration. Its only source operations are list/search/read against the engine-created immutable packet and a bounded revision-aware source-read broker. Arbitrary shell execution, file edits, VCS mutation, network mutation, connector tools, worker dispatch, comment posting, and merge commands are unavailable. The worker submits the finished Markdown through the single allowlisted command `"$BOSS_BIN" propose review-guide --body '<literal>'`; the proposal API attributes that submission to the running attempt. A run that ends without a submission fails the attempt. There is no other model-callable publish/reply command and no model-selected output path.
...
| Valid output for desired key                           | In one transaction check epoch, attempt/execution binding, latest selected comparison, and validation success; insert version and advance readable pointer.                                                                                                                 | Current guide ready; event updates card immediately.                  |
| Late output for a superseded request                   | Store attempt outcome as superseded/cancelled history; do not advance readable pointer or card lifecycle.                                                                                                                                                                   | Existing history remains readable; no new ready/merge/approval state. |
...
| Manual Generate / Regenerate                           | Resolve the chain root; require a PR URL and repository; refresh sources for the current PR with trigger `manual`, then admit generation even for an existing idle packet. Reuse a live attempt and remember its token; otherwise create a new epoch and immutable version. | Progress; older readable version and comments survive.                |
...
Persist queue intent before notifying the scheduler. On restart, resume ready requests from the database and reconcile running attempts against their specific execution binding and completion records. Re-adopt a confirmed live run; recover recorded output once; mark a vanished run interrupted and offer/requeue a bounded retry after fencing its old execution. Do not restart a job merely because the app reconnects. Generation and publication use the selected pinned comparison; observed source changes fence obsolete attempts without mutating prior versions. Automatic lifecycle triggers apply to open PRs. Explicit per-PR generation also supports merged and closed PRs as read-only context; publication has no open-PR fence, and feedback-to-revision remains refused for those PRs.

Transport retries reuse `boss_http_retry` with persisted attempt counts and a bounded policy, rather than an unbounded polling loop. Model-unavailable, essential-context-missing, malformed-output, and validation failures end in explicit retryable states. Manual Generate/Regenerate always awaits fresh pinned source capture for the current PR without an expected head branch and surfaces capture failures. Admission uses that PR’s series, so replaced PRs and changed heads cannot reuse retired comparisons. The clicked card shows progress throughout capture. Both manual generation and Retry retain the fixed model/effort and source provenance. A prompt update changes the key for subsequent creation, refresh, or explicit regeneration; it does not trigger a historical mass backfill.

### Review card and viewer

Every root card with a non-empty PR URL offers **Generate Review Guide…**, or **Regenerate Review Guide…** when a readable version exists, regardless of board lane. Confirmation explains that generation spends one fixed `gpt-6-astra` / `high` job and preserves prior versions and their comments. Errors are visible in the app. The manual action works with both `review_guide_source_capture` and `review_guide_generation` off; those flags gate only automatic seams. The existing task-detail guide row makes completed PR guides readable. Existing retention must keep a live manual attempt and newly generated versions even when they reuse an old source packet on a completed PR; packet age alone must not immediately collect a backfilled guide.
...
Template SHA-256 (UTF-8, excluding the fence and terminal newline): `cc4bddde22f1162c21484ff22aca69810215066a0eb0f27a281b25411cfa9911`.
