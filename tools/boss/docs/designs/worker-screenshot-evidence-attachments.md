...
- **Rows outlive blobs.** Reclaiming deletes bytes and stamps `reclaimed_at`; it does not delete the row. A gallery link in an already-merged PR body then answers "captured by execution X on <date>, reclaimed by retention on <date>" instead of a bare 404 that reads like a bug. A tombstone is not removed when its execution is pruned: the row's provenance is denormalized, so the row outlives the execution and only attachment retention takes its bytes. That independent lifetime is the point — a failed run's screenshots are exactly the evidence someone wants after the run row is gone. Tombstone rows themselves are kept indefinitely: they are small metadata, they are what a stale gallery link needs, and they cannot be reconstructed later. Live attachment caps count only unreclaimed rows.
