...
- **Queue administration from Boss.** Creating/deleting/pausing queues, changing concurrency/batching modes, and `setImpactedTargets` optimization stay in the Trunk web app. Boss reads queue state; the only write verbs it ever issues are entry-level — `submitPullRequest`, `cancelPullRequest`, and (solely as a flake-retrigger on a still-live entry, per the eviction section) `restartTestsOnPullRequest` — never queue-lifecycle administration.
...
- Reuses the repo's existing HTTP stack (whatever `claude_client` uses) with timeouts and bounded retries with jittered backoff on 5xx/transport errors **and on 429, honoring `Retry-After` when present** (the day-one commitment the non-goals and risks sections rely on for a later mono switch); no retry on any other 4xx. 401/403 is surfaced as a distinct `TrunkAuthError` so callers can emit the "token missing/expired" attention.
...
**What it writes:** the same two columns the UI already consumes — `merge_queue_state = "queued"` (any live Trunk state) and `merge_queue_detail = {source: "trunk", state: <trunk state>, position: <1-based index into enqueuedPullRequests>, enqueued_at, queue_state: <RUNNING|PAUSED|...>, section_order: <Merging-lane sort key>}` — plus `trunk_merge_intents.last_trunk_state`. `position` is 1-based to match the existing GitHub-path convention (`MergeQueueDetail` documents it as the 1-indexed queue position and the badge renders `#n` from it), so the probe adds 1 to the array index. `section_order` is not optional chrome: `mergingSection()` sorts the lane by `MergeQueueDetail.parse(...)?.sectionOrder ?? .max`, so a detail JSON without it would leave every Trunk card tied at `.max` and the lane unordered. After each sweep the Trunk probe feeds the same engine-side renumbering pass the GitHub path uses (`renumber_merge_queue`, `merge_poller.rs:3747`, extended or mirrored to order Trunk entries by queue position) so the key is populated consistently across both mechanisms. Queue-level anomalies (`PAUSED`/`DRAINING` while intents are active) emit a deduplicated attention item ("Trunk queue for flunge is PAUSED — 2 merges waiting") rather than per-card noise.
...
- **Rate limits are undocumented.** The cadence chosen (≤ 5 req/min active, 0 idle) is conservative, but if Trunk introduces limits, the poller's existing backoff absorbs 429s; the client treats 429 like a transport error with `Retry-After` respect from day one (specified in the `trunk_client` section and task 1).
...
1. **`trunk_client` crate** — New crate `tools/boss/trunk_client`: typed models for the six queue endpoints, `TrunkPrState`/`TrunkQueueState` enums with unknown-variant tolerance, token-as-parameter auth, structured errors (`Auth`/`NotFound`/`QueueUnavailable`/`Transport`), bounded retry/backoff on 5xx/transport and on 429 with `Retry-After` respect, unit tests against canned JSON fixtures. No engine dependency; minimal visibility. Effort: **medium**. Dependencies: none.
...
5. **Trunk queue poller** — `TrunkQueueProbe` in the merge-poller sweep: per-repo `getQueue`, per-missing-entry `getSubmittedPullRequest`, cadence tiers (15 s testing / 30 s pending / idle off), exponential backoff + unreachable-attention, writes `merge_queue_state`/`merge_queue_detail {source:"trunk", …, section_order}` (1-based `position`; `section_order` populated via the shared `renumber_merge_queue` pass so the Merging-lane sort works) and intent state, low-frequency `listPullRequests since=` reconciliation, `PAUSED`/`DRAINING` attention items, terminal `cancelled` → intent retirement + Review snap-back. Includes the write-ownership gate: for `trunk_queue` products the GitHub probe's `merge_queue_state`/`merge_queue_detail` writes and `check_merge_queue_rebounce()` are skipped (coordination rules) — without it the GH sweep clears the Trunk-written state within one cycle. Metrics counters via the existing counter framework. Effort: **large**. Dependencies: 4.
