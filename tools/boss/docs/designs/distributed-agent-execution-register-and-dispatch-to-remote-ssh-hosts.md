...
Bootstrap (user-installed, out of scope for this design): the user runs `cube repo ensure --origin <repo>` on the remote. Pre-cloning a workspace or two ahead of time is an optional warm-up — it avoids first-lease clone latency — never a capacity requirement: `cube workspace lease` auto-creates a fresh workspace on demand whenever none is free, so an unprimed host still leases successfully, just slower on the first call. Boss reports `cube workspace lease` failures clearly (see Q6) but does not try to materialize workspaces itself.
...
| `cube workspace lease` fails (repo not registered, remote unreachable, stale lock, clone failure) | non-zero exit, JSON error                                                            | run reason `host_lease_failed`; attention item quotes cube's error verbatim                                                                                                                                             | do not retry on the same host; retry on another eligible host if any                                                                                                               |
| Remote resolved driver missing or unauthed                                                        | wrapper script exits with a documented sentinel code                                 | run reason `host_missing_driver`; host marked `degraded` (not removed)                                                                                                                                                  | do not retry on the same host; surface registration-time checklist mismatch                                                                                                        |
...
- **No new privileged surface on the coordinator.** Engine code that handles SSH errors must not shell out with unsanitized host fields. Host id and ssh*target are validated at insert time (regex: `^[a-zA-Z0-9.*@:-]+$`).
