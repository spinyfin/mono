...
Three ordered pieces. (1) A new `boss-host-suspension` crate samples the clock pair, keeps a persisted ledger of suspension intervals, exposes `awake_elapsed_secs(since, now)`, and publishes `Event::HostResumed` on the engine event bus, with no app involvement. (2) The six horizons split cleanly: the three in-memory trackers only need an awake clock source and do not depend on piece 1 at all; the four persisted-stamp horizons subtract ledger time and depend on the ledger but not on the resume event. (3) The SHA-delta gate gets a bounded retry using the existing `boss-http-retry` policy math and a `gh`-stderr transient classifier, and an inconclusive-because-the-fetch-failed gate is re-evaluated on `HostResumed` under an attempt cap. Nothing pins the machine awake, nothing raises a threshold, and no check is silenced.
...
- A single transient network failure at a Stop boundary no longer wedges a run in `AwaitingInput` until a Stop that never comes. The gate retries with bounded backoff, and a gate that stayed inconclusive because the network was gone is re-evaluated when the host resumes.
...
- **Redefining the completion contract.** A separate project owns making the run-done declaration terminal at submit and collapsing terminalization paths. This design re-runs the _existing_ Stop-boundary evaluation after a resume; it introduces no new way for a run to finish and no finalize that depends on a network call.
...
`nudge_breaker.rs` and `merge_poller/schedule.rs` already use `Instant` for their debounce and adaptive poll schedule and are therefore already sleep-immune; they are listed to show the codebase has both conventions with no rule choosing between them. `epoch_time.rs` documents itself as "the single source of truth for reading the current wall-clock time" and is correct about that: the defect is not in the helper but in horizons that use a wall-clock reading to answer an awake-time question.
...
### One adjacent wall-clock horizon outside the six
...
A new leaf crate at `tools/boss/engine/host-suspension` (per the crates-over-modules convention; it has its own vocabulary, its own tests, and three consumers). It depends on `libc` only. `engine/core` depends on it; it never depends on the engine.
...
**Sampler.** A tokio loop with a 5 s cadence reads `suspended_total_secs()` and compares with the previous reading. Growth `Δ ≥ 1 s` means a suspension of length `Δ` ended within the last tick: append `(wall_now − Δ, wall_now)` to the ledger, persist it, log one `info` line with the interval and the running total, then publish `Event::HostResumed { suspended_secs, resumed_at_epoch }`. The cadence is `Instant`-based, so it ticks during DarkWake slices and records them individually as they happen; several slices between two ticks collapse into one interval whose _length_ is exact and whose _placement_ is off by at most one tick, which is harmless because horizons only consume totals over a range. At boot the sampler seeds `previous = suspended_total_secs()` and records nothing, because sleep before the engine started is not the engine's to measure (see the deferred backfill entry).

**Ordering invariant.** The ledger is appended and persisted _before_ `HostResumed` is published. Any consumer that runs on resume therefore already sees the suspension when it computes awake-elapsed time. Without this a resume-triggered sweep could reap on the stale wall-clock age one last time.

**Event.** `EventKind::HostResumed` / topic `host_resumed` is added to `engine/event-bus` alongside the existing kinds, following the event-bus design's "add a variant, subscribe by kind" pattern. Naming is deliberately `Host`+`Resumed` rather than reusing `HostDisabled`'s "host" sense (a dispatch host record); the doc comment on the variant says so.
...
The answer to "does piece 2 depend on piece 1" is: half of it does not, and the other half depends on the ledger but not on the event.

**In-memory trackers (no dependency on piece 1).** `BuildWaitTracker` is in-memory and resets on restart, so an engine-uptime clock is exactly right for it. Its `record()`/`decide()` API keeps `now: i64` seconds so existing tests stay meaningful, but production callers pass `awake_secs()` from the `HostClock` instead of `now_epoch_secs()`. That single change fixes the run_done backstop (20/40 min), the build-wait suppression (45 min), and the background-children suppression, because all three go through that tracker. The `waited_secs` they log becomes awake seconds, which is what a human reading the log wanted anyway.
...
**Distinguish "could not fetch" from "no baseline".** `ShaDeltaGateOutcome::Inapplicable` gains a reason: `FetchFailed { transient: bool }` versus `NoBaseline` versus the structural cases. `stop.rs` handles them identically today except for one addition below, and the log line carries the reason so the next incident is diagnosable from the log alone.

**Retry on wake (depends on pieces 1 and 3a).** When the `Inapplicable` arm returns `AwaitingInput` with reason `FetchFailed { transient: true }`, the completion handler records the execution id in an in-memory `pending_gate_rechecks` map (execution id → bound PR URL, attempt count, first-failed awake time). Two things drain it:

- `Event::HostResumed`: every pending entry is re-evaluated.
- A bounded timer (`TimerWheel::schedule_after`, 60 s then 5 min then 15 min of awake time) so a network blip with no suspension also recovers.

A re-evaluation is **not a Stop event**. It re-runs only the SHA-delta gate and the arms that follow it in `stop.rs`, after first confirming the worker is still at a Stop boundary (activity not `Working`, no in-flight tool, no live descendants) so a worker that has since resumed working is left alone and its entry dropped. It does not consume the nudge breaker unless it actually issues a nudge, and it reaches the same `evaluate_satisfied_deliverable_on_stop` / `await_run_done_declaration` arms the real Stop reaches, so when the completion-contract project collapses terminalization paths, this recheck is inside the same path rather than beside it. After five failed re-evaluations the entry is dropped and a `sha_delta_gate_unreachable` attention item is raised, so the run parks visibly instead of looping forever across DarkWake slices. The map is in-memory; an engine restart loses it, and the run then waits for the next real Stop or a human exactly as today, which is a known v1 hole listed under risks rather than a reason to add a column now.
...
3. Drain `pending_gate_rechecks` (piece 3b).
...
### The six horizons across a five-hour suspension, before and after
...
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| run_done backstop 20/40 min  | First Stop after wake sees `waited = 5 h` ⇒ `Ask`; a worker merely between turns is probed, the breaker is charged; a few sleeps later it parks in `waiting_human`.                                                                                                                                                     | `waited` counts awake seconds ⇒ `HoldingWithinHorizon`; the probe fires only after 20/40 awake minutes of silence.                               |
...
Not one of the six but changed by the same table: the cube lease heartbeat still cannot beat a 25 h sleep against a 24 h TTL on its own; see risks.
...
- `boss-host-suspension`: `FakeHostClock` drives the sampler; assertions cover one interval, several intervals between ticks (length exact, placement within a tick), a sub-threshold jitter that must record nothing, persistence round-trip through a fake `LedgerStore`, and that the store write completes before the event is published (the fake store records ordering).
...
- Gate retry: a `BranchVerifier` fake that fails N times with a transient stderr then succeeds; asserts the attempt count, that a permanent error is not retried, and that `Inapplicable { FetchFailed { transient: true } }` arms a recheck while `NoBaseline` does not. The recheck drain is tested with a fake bus publishing `HostResumed` and with the attempt cap reached.
...
**Manual procedure, executed once and recorded (its own task).** On a dev machine, with an isolated engine (`--socket-path /tmp/boss-sleep-<uuid>.sock`, `BOSS_EVENTS_SOCKET` unset), start a worker execution that reaches a Stop and idles, note its `progress_at`, then `sudo pmset relative wake 240 && sudo pmset sleepnow` (the `-i` idle-only assertion from `sleep_assertion.rs` does not block forced sleep, by its own documentation, so this is a faithful clamshell stand-in). After the scheduled wake, assert from the engine log and DB: one `host_suspensions` row of about 240 s; one `HostResumed`; the next stale-worker pass logs `Healthy` for the slot; zero `dropped a wakeup` warnings for rows created before the sleep; a `pending_gate_rechecks` drain line if the gate was armed (arm it by running the procedure with network disabled during the Stop). Repeat once with a real lid close of at least two hours to cover DarkWake slices, and record `pmset -g log` alongside the ledger rows to show they agree. The evidence goes into `tools/boss/docs/investigations/host-suspension-validation-<date>.md`.

## Risks / open questions

- **DarkWake network state is not modelled.** During a DarkWake slice the sampler ticks, `HostResumed` fires, and a gate recheck may run against a network that is up (TCPKeepAlive) or not. The attempt cap bounds the damage; a smarter policy (skip rechecks while `pmset` reports a DarkWake) is deliberately not designed until the cap is observed to bite.
...
- **The in-memory recheck map** is lost on engine restart. The run then waits for a real Stop or a human, as today. Persisting it is a one-column change if the restart hole turns out to matter in practice.
...
Breakdown size: 8 entries (7 in-scope, 1 deferred) — the change has one new measurement seam (a crate with a sampler, a ledger and an event), two independent horizon families (three in-memory trackers behind one clock swap, and four persisted-stamp sites of which the destructive stale-worker sweep carries enough review weight to stand alone), a gate retry that splits into a network-only half and a resume-dependent half, one thin resume-consumer wiring PR, and a manual validation campaign the project explicitly demands; the deferred entry is the one ledger gap the design names and does not close in v1.

Parallelism: the three depth-0 entries (`boss-host-suspension` crate, in-memory trackers, gate bounded retry) touch disjoint files and may run concurrently. After the crate lands, the stale-worker entry, the remaining persisted-stamp entry and the resume-consumer entry may run concurrently; the two persisted-stamp entries share no files with each other, but both add the ledger parameter to sweep-construction call sites in `app/server.rs`, so the second to land forward-ports the first's wiring preservingly. The retry-on-wake entry waits for both the crate and the bounded retry because it edits the same `stop.rs` and `metadata_gate.rs` regions as the latter.

### Host-suspension crate: clock pair, ledger, sampler, `HostResumed` event

Create `tools/boss/engine/host-suspension` with the `HostClock` trait (production impl over `libc::clock_gettime` using `CLOCK_UPTIME_RAW`/`CLOCK_MONOTONIC` on Apple and `CLOCK_MONOTONIC`/`CLOCK_BOOTTIME` on Linux) and `FakeHostClock`; the `SuspensionLedger` with `awake_elapsed_secs(since, now)` and a `LedgerStore` trait; the 5 s sampler that appends and persists an interval before publishing. In `engine/core`: the `host_suspensions` migration and `LedgerStore` impl, `EventKind::HostResumed` in `engine/event-bus`, boot wiring in `app/server.rs` that loads the ledger and starts the sampler, and the per-interval `info` line. Minimal Bazel visibility (`engine/core` only). Unit tests as listed under Verification, including the store-before-publish ordering test.
...
Add `classify_gh_failure` to `boss_github::gh_runner` next to `parse_http_status_from_stderr`, covering the connect-failure texts observed in both incidents, and point `trees.rs`/`contents.rs`'s transport bucket at it. Wrap `CommandBranchVerifier`'s three head/ref fetches in a `boss_http_retry::RetryPolicy::new(3, 2 s, 4 s)` loop with jitter that retries only on `Transient`. Split `ShaDeltaGateOutcome::Inapplicable` into `FetchFailed { transient }` / `NoBaseline` / structural reasons and carry the reason in the existing `stop.rs` and `execution_started.rs` log lines without changing their decisions. Tests: attempt counting against a failing fake verifier, no retry on permanent errors, classifier table tests including the exact incident stderr.
...
### Retry the inconclusive SHA-delta gate on resume and on a bounded timer

Add `pending_gate_rechecks` to the completion handler, armed from the `Inapplicable { FetchFailed { transient: true } }` arm in `stop.rs`; drain it on `HostResumed` and on a `TimerWheel` schedule of 60 s / 5 min / 15 min awake time. A recheck confirms the worker is still at a Stop boundary, re-runs only the gate and its downstream arms, does not charge the nudge breaker unless it nudges, drops the entry once conclusive or once the worker is `Working` again, and after five failures raises a `sha_delta_gate_unreachable` attention item (registered in `attention_lifecycle.rs` with its clearing rule). Tests cover arming, draining on each trigger, the worker-resumed drop, and the cap.

- **Effort:** medium
- **Dependencies:** Host-suspension crate: clock pair, ledger, sampler, `HostResumed` event; SHA-delta gate bounded retry and `gh` failure classification
- Scope: in-scope

### Manual suspend/resume validation against a live isolated engine

Execute the manual procedure in the Verification section on a dev machine: the `pmset relative wake` forced-sleep run and one real lid-close run of at least two hours, with the network-disabled variant to arm a gate recheck. Record the ledger rows, `pmset -g log` excerpts, sweep verdicts, heartbeat warning counts and recheck drain lines in `tools/boss/docs/investigations/host-suspension-validation-<date>.md`, and add the procedure to `tools/boss/docs/runbooks/` so it is re-runnable. This is a validation of the chosen approach, not a comparison; a failure is filed as a defect against the entry it implicates.

- **Effort:** medium
- **Dependencies:** Stale-worker sweep measures awake time, including the auto-reap recheck; Orphan, spawn-ack and scheduler-heartbeat horizons measure awake time; Engine-side resume consumers: scheduler kick and cube-lease heartbeat pass; Retry the inconclusive SHA-delta gate on resume and on a bounded timer
- Scope: in-scope
