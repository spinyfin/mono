...
creates a **revision task** on the _original producing task_, carrying the
...
updated. We hook the _same_ detection point used to move a task into
...
2. Run the **no-op / scope gate** (see _No-op skipping_). If it says
...
The `pr_review` execution is routed to the review pool (see _Pool &
routing_). The producing task does **not** advance to "human Review" until
...
> of whether the PR was created _or_ updated — because updates (e.g.
...
Reviewer effort is bounded (see _Loop termination & bounds_ and _Cost_):
...
```jsonc
{
  ...
  "regression_check": {
    // first-class, always present
    "performed": true,
    "suspected_deletions": [
      /* findings of category=regression */
    ],
  },
}
```
...
- `revision_warranted` is computed by the reviewer but **gated by the
  engine** against a severity bar (see _Bounds_): the engine creates a
...
- `revision_instructions` = a rendered, human-readable digest of the
  ...
  severity). This is the _internal_ feedback; it lives in Boss only.
- `pr_url` = the producing task's `pr_url`, **explicitly inherited** so the
  revision does not strand in the T955 nudge-loop (see _T955 interaction_).
...
pool added a _second_ pool with: `WorkerPool::new_automation`,
...
3. **Convergence / severity decay** — if successive reviews only produce
   findings _below_ the severity gate, no revision is created (this is just
...
2. **Tool denylist** (primary enforcement, the contract): the reviewer worker
   ...
   This is the _enforceable_ guarantee, not just a polite request.
...
- **P576 / editorial controls.** `editorial.rs` governs what agents say on
  ...
  conflict — but to be safe the reviewer worker kind is _exempt by being
  denied GitHub-write tools entirely_ (section 9), so there is no
...
- **unify-pr-remediation-on-revisions.** The reviewer is a new remediation
  _source_ (`pr_review`) feeding the same revision mechanism that designates
...
1. **Mandatory-pass vs. throughput.** Holding the producing task out of
   human Review until the review pass resolves adds latency to _every_ PR.
   Is the default `max_review_cycles = 3` the right ceiling, and should the
   _first_ review be blocking while later cycles are best-effort
...
7. **Reviewer reading the diff.** _(Resolved.)_ The engine checks out the
...
1. **Confirm/add per-worker-kind tool denylist** — _Depth 0_
...
2. **Land T955 fix (revision pr_url nudge-loop)** — _Depth 0_
   - **Scope:** Ensure revisions always carry the parent's `pr_url` and a
     PR _update_ (not just create) satisfies the "PR exists" condition, so
...
3. **Add `pr_review` execution kind** — _Depth 0_
   ...
   _Depth 0 tasks (1, 2, 3) may run in parallel._

4. **Add the review worker pool + dispatch routing** — _Depth 1_
...
5. **Per-pool always-Opus model override for the review pool** — _Depth 1_
...
6. **Reviewer worker prompt + structured `ReviewResult` output** — _Depth 1_
   ...
   _Tasks 5 and 6 may run in parallel once 4 lands (6 also needs 1)._

7. **Trigger: enqueue `pr_review` from the pr_url capture hook** — _Depth 2_
...
8. **Feedback → revision wiring (internal, never GitHub)** — _Depth 3_
...
9. **Loop termination & bounds** — _Depth 3_
...
10. **No-op / trivial-diff skip gate** — _Depth 3_
    ...
    _Tasks 9 and 10 may run in parallel after 8, with a small ordering on
    `last_reviewed_sha` ownership (define it in 9, consume in 10)._

11. **Reviewers page on the macOS agents view** — _Depth 2_
...
12. **Docs-only light-review rubric branch** — _Depth 3_
...
13. **End-to-end test + tuning pass** — _Depth 4_
...
- **Configurable review-rubric policy layer** — move the baked-in rubric to
  ...
  evolution). _future / not a v1 blocker._
- **Per-cycle severity-gate decay** — auto-raise the gate on later cycles
  ...
  off. _future / not a v1 blocker._
- **Hard per-day review-budget circuit breaker** — global cost cap beyond
  pool-size + cycle caps. _future / not a v1 blocker._
- **Stacked-PR per-entry review semantics** — review each PR in a stack
  against its stack parent. _future / not a v1 blocker._
- **Surfacing unresolved-at-bound findings to the human reviewer UI** —
  richer presentation than the internal attachment. _future / not a v1
  blocker._
- **Non-blocking later cycles** — make only the first review blocking and
  later cycles best-effort to protect throughput (open question 1). _future
  / not a v1 blocker._
