...
- **`UtilityModel` provider seam** for the four direct-Anthropic call sites (§1.8) + P783 Planner. Decouples the engine's own inference from the worker driver. _future / not a v1 blocker_ (operator may pull into v1 — see open questions). **Effort: medium.** Dependencies: none structural.
