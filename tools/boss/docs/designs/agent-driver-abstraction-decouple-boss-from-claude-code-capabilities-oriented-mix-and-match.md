...
- **Implementation shape.** `engine/core/src/live_worker_state.rs`'s `apply_event` — the one place the `Notification`→`Stop` proximity is interpreted — now consults a per-slot `awaiting_input_capable` flag (seeded `true` at `register_spawn` to keep every existing call site behaviour-preserving without threading the driver through 30-odd test call sites, overridden via `set_awaiting_input_capable` from the resolved driver's `CapabilitySet` at the two production spawn sites — local pane spawn in `spawn_flow.rs` and the remote virtual-slot path in `app/worker_events.rs::register_remote_worker_slot`). When the flag is `false`, a `Notification` event (however it arrived) is _not_ interpreted as an awaiting-input signal — activity is left untouched rather than guessed, so `Stop` falls through to `Idle` as it would for any ordinary turn boundary. Both production spawn sites still hardcode `ClaudeDriver` today (the runtime spawn path is not yet driver-generic — see the "Extract the Claude Spawn capability" task below), so this resolves to `true` everywhere in practice until a second driver is actually wired into spawn; the capability declaration is what a future driver's absence will flow through without further engine-side change.
