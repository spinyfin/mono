...
- **Ownership: P1422, not P3330, and not "neither."** `UtilityModel` is already listed under this doc's own "Future / not a v1 blocker" task breakdown (`UtilityModel` provider seam, "Dependencies: none structural") — it was scoped here from the start, just not flagged as the authoritative answer to T3527's contradiction until now. P3330 (the Codex driver design)'s capability gap table covering only the 12 `AgentDriver` capabilities and omitting `UtilityModel` is **correct, not a gap**: `UtilityModel` is not an `AgentDriver` capability, so a driver's capability declaration has nothing to say about it. P3330 does not need a `UtilityModel` row and should not grow one; if a reader lands on P3330 looking for it, this section is the pointer back to P1422.
- **What T3527's Deliver section should say instead:** route the four call sites (`live_status.rs`, `pane_summary.rs`, `magic_wand.rs`, `attentions_detector.rs`) through a standalone `UtilityModel` provider seam — a small trait plus config-driven provider selection, scoped and owned by P1422 — instead of a method on `AgentDriver`. The seam must let the operator's utility-model choice vary independently of any work item's `driver`/`backend` value. T3527 remains deferred per its existing annotation and, per this chore's constraints, keeps every call site on Claude when it does eventually implement the seam — only the shape (independent seam vs. driver method) changes, not the model choice.
...
- **`UtilityModel` provider seam** for the four direct-Anthropic call sites (§1.8) + P783 Planner. Decouples the engine's own inference from the worker driver, as a standalone seam independent of `AgentDriver` — see "[Decision: UtilityModel shape and ownership](#decision-utilitymodel-shape-and-ownership-resolves-a-t3527-contradiction)". Owned by P1422; not a P3330 (Codex driver) concern. _future / not a v1 blocker_ (operator may pull into v1 — see open questions). **Effort: medium.** Dependencies: none structural.
