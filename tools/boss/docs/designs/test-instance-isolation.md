...
| 15  | Engine-control token (shutdown-RPC auth secret)                                                          | `$HOME/Library/Application Support/Boss/engine-control.token`                             | `BOSS_ENGINE_CONTROL_TOKEN_PATH`; now also derived in `IsolationPaths` (row 1/2/3 pattern) when `--socket-path` is non-default | engine mints/writes; `bossctl`/client reads | `tools/boss/engine/core/src/engine_control.rs`; `tools/boss/engine/core/src/app.rs` (`IsolationPaths`); `tools/boss/client/src/lib.rs:341-351`                                    |
