...
| 15  | Engine-control token (auth for the `shutdown` RPC)                                                       | `<state root>/engine-control.token`                                                       | `BOSS_ENGINE_CONTROL_TOKEN_PATH`                                                  | engine mints + deletes on exit; app reads | `tools/boss/engine/core/src/engine_control.rs:65-72,135-150`; `tools/boss/app-macos/Sources/BossEnginePaths.swift:84-91`                                                          |
| 16  | Engine trace JSONL (macOS Activity Log viewer)                                                           | `<state root>/engine-trace.jsonl`                                                         | **none** (rotates in place)                                                       | engine writes; macOS app reads            | `tools/boss/engine/core/src/main.rs:56-70`; `tools/boss/log-files/src/paths.rs`                                                                                                   |

> **Rows 15–16 were missing from the original audit**, and both were clobbered by a test-fixture engine during the 2026-07-23 incident. The control token was resolved entirely outside the isolation guard, so the fixture overwrote production's token and `ControlTokenGuard::drop` deleted it on shutdown — every engine-control path stayed broken until the next restart. The trace log has no override at all, so the fixture took over the canonical `engine-trace.jsonl` filename and pushed production's stream into a rotated file. Both now resolve through `app::isolation::IsolationPaths` and get stem-scoped names in fixture mode.
...
Rows 13/14 are derived state — they're populated from whatever the engine resolves at spawn time.

> **Correction (2026-07-23).** This section originally read "a test engine launched with a private `BOSS_EVENTS_SOCKET` will (correctly) bake that socket path into every worker's `settings.json`", and concluded that workers spawned by a test engine talk to the test engine. That was only ever true in the forward direction — when the env var _is_ the isolation mechanism. It was false the other way round: `runner::engine_events_socket_path` re-read `$BOSS_EVENTS_SOCKET` instead of consulting the path the engine had actually bound, so an engine isolated by any other route (a derived socket from `--socket-path`) still handed its workers production's socket. The engine now stamps the bound socket onto `WorkConfig::events_socket_path` and every consumer reads it from there; nothing but config load re-reads the env var.
...
I'd ship Phase 4 with `BOSS_TEST_WORKERS_ENABLED` gating worker spawn entirely — test instance can do everything _except_ spawn paying workers unless explicitly unlocked. Cheap belt.
