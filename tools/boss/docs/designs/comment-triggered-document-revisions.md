...
- **Known as-built gaps** (detailed in [Risks](#risks--open-questions)): a `CommentsSetIntent` manual override does not re-run routing; completion reconciliation does not push a `comment_topic` invalidation to open viewers.
- **2026-07-24 revision (two operator-reported bugs, fixed together):**
  1. **The classifier's `directive`/`larger_change` split is retired.** Every downstream consumer (the `[Revise]` batch predicate, the nudge trigger, the `answered`/`active` transitions, manual override) always matched the two together and never branched on which one a comment carried — the distinction was a three-way choice surfaced to the operator (a `Directive ⌄` dropdown) for zero behavioral difference. Collapsed onto a single `revision` value (`INTENT_REVISION` in `boss-protocol`); the classifier prompt, `work_comments.intent`, the `CommentsSetIntent` wire type, and the macOS `CommentIntent` enum all now speak `revision` | `question`. A one-time data migration (`migrate_collapse_directive_larger_change_intent`) re-homes any existing `directive`/`larger_change` rows onto `revision` so they keep matching the `[Revise]` candidate predicate.
  2. **The nudge no longer dead-ends on a nonexistent `[Revise]` button.** Per the as-built divergence noted below, the classifier (and, before this fix, the nudge it triggers) ran unconditionally on every artifact kind, including ones `resolve_doc_owner` can never resolve (e.g. `work_item` comments — see [Non-goals](#non-goals)). Those comments got the "click [Revise] to start one" nudge posted in their thread with no `[Revise]` banner ever reachable for them (the banner is gated on `resolve_doc_owner` resolving an owner). `post_nudge_once` (`engine/core/src/app/comments.rs`) now checks the same `resolve_doc_owner` gate `comments_banner_state`/`CommentsReviseDoc` already use, before posting any nudge — mirroring the scope guard `spawn_answer_agent` (bucket 2) already had. The classifier itself is unchanged (still unconditional, still just badges the comment); only the nudge is gated, so an out-of-scope comment still gets classified and badged, it just doesn't get a promise it can't keep.

> **Supersedes** the original narrower scope of this design ("larger change → revision/update task" only). Restructured around a comment-intent model: every new comment on a design/investigation doc is classified by an LLM and routed accordingly. As originally shipped this was a **three-intent model** (**directive**, **question**, **larger change**); the 2026-07-24 revision above collapsed `directive`/`larger_change` into a single `revision` intent, since buckets 1 and 3 always shared one handling path and no consumer ever branched on which of the two a comment carried — the classifier and every routing consumer described below now speak **revision** / **question**. This **replaces the per-comment "magic wand" entirely** (see [Migration](#migration-retiring-the-magic-wand)). Bucket 2 (question) is new: a read-only mini-coordinator agent answers in the comment thread. The classifier (bucket 1&3) is the prerequisite piece — nothing else in this design can run before every comment has an intent.
...
> **As-built divergence (scope guard).** The design intended classification to be gated on `resolve_doc_owner` resolving to a `Design`/`Investigation` task, so the classifier "never runs on out-of-scope artifacts." As shipped, **the classifier runs unconditionally on every top-level comment on every artifact kind** — including `work_item` comments — and the doc-owner scope guard is enforced _downstream_ instead: at answer-agent spawn (`spawn_answer_agent` leaves an unresolvable comment `active` with no bucket-2 affordance), at `CommentsReviseDoc` (`NotApplicable`), and on the banner read (`revisable` false). P1a (PR #1690) landed before the resolver (1c) and flagged the missing gate; nothing went back to wire it, and the code comment in `app/comments.rs` acknowledges "the classifier itself runs unconditionally today." Consequence: out-of-scope comments still cost a classifier LLM call and (until the 2026-07-24 fix below) got a nudge thread entry posted on a `revision` result, with no `[Revise]` affordance to follow it.
>
> **Resolved (partially), 2026-07-24.** The classifier call itself is still unconditional (unchanged — a comment still gets classified and badged regardless of scope). But `post_nudge_once` now checks `resolve_doc_owner` before posting the "click [Revise] to start one" nudge, so an out-of-scope comment no longer gets a nudge pointing at a banner it can never reach. This closes the specific operator-visible symptom (a dead-end nudge) without fully closing the underlying gap (the classifier LLM call is still spent on out-of-scope comments) — that narrower cost question is left as the remaining follow-up in [Risks](#risks--open-questions).
...
**Output.** One of `revision | question` (originally `directive | question | larger_change` — collapsed 2026-07-24, see the revision note at the top of this doc), plus a confidence score. Written to new columns on `work_comments` (shipped verbatim in `migrate_work_comments_intent_columns`, schema v22, PR #1690; the two failure columns were added later):

```sql
ALTER TABLE work_comments ADD COLUMN intent TEXT;              -- 'revision'|'question', NULL while classifying
...
**Override re-homes the comment.** An override does not just relabel: it re-runs routing so the comment lands where it would have been had it been classified that way to begin with. `intent` is the label, but `status` is what every consumer gates on, and the sidebar's intent badge and `[Revise]` banner both gate on `status`/`revise_task_id` directly, so both move together. The directed transitions, keyed on the comment's status _before_ the override:

| before                                           | new intent | what happens                                                                                                          |
| ------------------------------------------------ | ---------- | --------------------------------------------------------------------------------------------------------------------- |
| `answering`                                      | `revision` | the in-flight answer-agent run is **stood down** (below), the comment goes `→ active` in the same write as the intent |
| `answered` / `awaiting_followup`                 | `revision` | `→ active`                                                                                                            |
| `active`                                         | `revision` | already on the revise track; label only                                                                               |
| `active`                                         | `question` | answer agent spawns, `→ answering`                                                                                    |
| `revision` → `revision`, `question` → `question` | —          | label only; both are already on the right track                                                                       |
| `in_revision`                                    | anything   | label only — see below                                                                                                |

(As built, an override — or any classification — never posts a thread entry; see [Buckets 1 & 3](#buckets-1--3--unified-directive--larger-change) below.)

**Standing down an in-flight answer agent.** Reclassifying away from `question` while a run is live is a retraction, so the engine both supersedes the run and cancels its execution:

- The `answer_agent_runs` row is completed with the terminal status **`superseded`** (`error_kind='reclassified'`, no `reply_body`). This is the durable half. Because the row is no longer `running`, a late `CommentsPostAnswer` from the agent's still-live process is rejected rather than posting a thread entry and dragging the comment back to `answered`, and `finalize_answer_agent` takes its already-completed branch. Because it carries no reply, `compose_doc_comment_directive` — which now requires `status = 'replied'` — never feeds a retracted question's answer into a revision as context. `superseded` is deliberately distinct from `failed`: nothing went wrong, and `CommentWithThread.answer_agent_failed` must not light up a failure chip for a run the operator themself retracted.
- The bound `answer_agent` execution is cancelled and its pane + cube lease released, so the worker slot is reclaimed instead of spent finishing an answer nobody will read.

Crucially the comment reaches `active` in `override_comment_intent`'s own write — **not** deferred to the run's termination. Deferring it was the original bug: the comment kept rendering "Thinking…", stayed off the banner's `unresolved_count`, and was silently dropped by `[Revise]`, for as long as the agent kept running. The intent-aware fallback in `transition_comment_to_answered` (which lands a revisable-intent comment on `active` rather than `answered`) is retained only as a backstop for paths that reclassify without going through this handler — notably the engine's own `reclassify_comment_intent`.

**Why `in_revision` is not re-homed.** A claimed comment stays claimed. The claiming task's directive was assembled once, at creation, and is immutable; pulling the comment out from under a live task would leave it addressing a comment it no longer owns and destroy the `revise_task_id` provenance. The new intent takes effect on the next cycle instead — if the batch is abandoned, reconciliation returns the comment to `active` with the overridden intent already in place. The operator is told rather than left to infer: see [Reporting what a batch left behind](#reporting-what-a-batch-left-behind).
...
```
                    ...
                    ┌─────────────────────────┼─────────────────────────┐
                    │ intent=revision           │ intent=question         │
                    ▼                          ▼                         │
             ┌────────────┐            ┌───────────────┐                │
  ...
  └───────────┘          │  active   │◀────────────│  reply — see above)  │
                         │(reopened) │             └───────────┬───────────┘
                         └───────────┘                          │
                                                     intent=question │ intent=revision
                                                                 │                 │
                                                                 ▼                 ▼
...
| `awaiting_followup` | `active`                                 | reclassified reply has `intent=revision`                  | engine                                                     | bridges into the 1&3 path (see below); comment's `status` becomes `active` so the next `[Revise]` batch picks it up                    |
...
**On classification as `directive` or `larger_change`,** the sidebar's intent badge renders `Revision` on the comment itself and the always-visible `[Revise]` button is available — no separate engine-authored thread entry announces the classification. The original design (and this section, through 2026-07-24) called for the engine to also post an engine-authored reply in the comment's thread nudging the operator toward starting a revision (an `entry_kind='nudge'` `comment_thread_entries` row, e.g. _"This looks like it wants a doc change — click [Revise] to start one."_); that nudge shipped in 2b (PR #1727) and was **removed** as redundant with the intent badge + always-visible `[Revise]` button — see the as-built note below. The `[Revise]` banner (`{n} unresolved comment(s). [Revise]`) still exists and still batches.
...
2. Load `active` comments with `intent = revision` (or the `comment_ids` intersection); if empty → `NoUnresolvedComments`.
...
#### Reporting what a batch left behind

Candidacy for a batch is `status = 'active' AND intent = 'revision'`. That predicate had three independent string-literal copies — the banner's `unresolved_count`, `query_revisable_comments`, and the guarded claim `UPDATE` — which agreed only by convention; they now share one `revisable_comment_predicate()`. A divergence between them presents as the banner counting a comment `[Revise]` then silently drops, so there is exactly one definition.

The `intent` half of that predicate is what the sidebar badge renders, but the `status` half is invisible in the UI. So a comment badged `Larger Change` whose status is `in_revision` (claimed by an earlier batch), `orphaned` (anchor no longer resolves), or `answering` (live run) is dropped by a batch that then reports a flat success — indistinguishable from "everything was addressed". `ReviseDocOutcome::Created` therefore carries **`excluded_comment_ids`**: the complement of the candidate predicate over the same intent set, minus the `resolved`/`dismissed` comments the sidebar already hides. The macOS client turns a non-empty list into a banner message naming the counts. Empty (and omitted on the wire) in the common case.

This is reporting, not a loosening: a comment with a live answer agent genuinely is not ready. The fix for reclassification is to re-home `status`, never to widen the predicate.

#### Recovering comments stranded in `answering`

`answering` is transient by construction — `finalize_answer_agent` moves the comment on when the agent's Stop hook fires. If that Stop never arrives (pane killed, engine restarted mid-run), _nothing_ moves it: `terminal_work_sweep` classes `AnswerAgent` as never-task-bound and, like every execution reaper, only touches `work_executions`; the registry-driven reapers are blind to anything from a previous engine process. The comment then sits permanently `answering` — off the banner count, off the candidate query, rendering an indefinite "Thinking…", with no operator-visible signal.

`stranded_answering_sweep` closes this. It is comment-driven and DB-backed (so it clears strandings left by a previous engine instance at boot), runs on the same 60s cadence as the other reapers, and acts only on positive evidence that no agent will finish: the comment has no non-terminal `answer_agent` execution, confirmed across two consecutive passes. Recovery is deliberately _identical_ to `finalize_answer_agent`'s no-reply-posted path — mark the run `failed` (`error_kind='stranded_no_stop'`, distinguishing it from an ordinary `no_reply_posted`), post the same apology thread entry, then `transition_comment_to_answered`, which is intent-aware. It is a substitute for a Stop that never came, not a second recovery policy.
...
This is the minimal "conversation" shape the brief asks for (classification badge, agent replies, operator follow-ups, re-routing) without generalizing the base comment model's deliberate single-level non-threading — `comment_thread_entries` rows are _always_ children of exactly one `work_comments.id` and are rendered inline under that comment, never as independent top-level comments.

- **1&3 state:** the sidebar renders the live chip **`⟳ In revision → R2 ↗`** / **`⟳ Update task ↗`** from the comment's own `revise_task_id`, and **`↺ Reopened`** from `work_comments.reopened_at` (see the as-built note below) — no thread entry is involved on this track any more.
...
- ~~**`CommentsSetIntent` overrides do not re-run routing.**~~ **Closed.** `handle_comments_set_intent` now performs the full re-home for every transition — status, answer-agent lifecycle, and nudge. See [Misclassification / override](#the-classifier-p1-foundation).
...
- **The bucket-1&3 `nudge` thread entry and its "Nudge sent" chip are removed.** They duplicated the sidebar's intent badge (already says "Revision") and the always-visible `[Revise]` button, and the 2026-07-24 fix above shows the dead-end-nudge failure mode was itself a symptom of the nudge being redundant rather than load-bearing. `post_nudge_once`, `create_nudge_thread_entry`, `NUDGE_BODY`, and every call site are deleted; `nudge` is removed from the set of `entry_kind` values `create_comment_thread_entry` will write. The `Reopened` chip state, which used to key off a nudge entry's `revise_task_id`, now rides `work_comments.reopened_at` — a column the engine actually maintains (stamped by `reconcile_comments_for_task`'s `Reopened` outcome, cleared by every path that lands a comment back on `active`) instead of a carrier nothing ever populated correctly. Pre-existing `nudge` rows are left in place in `comment_thread_entries` as inert history — `WorkDb::list_comment_thread_entries` excludes them at the source, so the app, `bossctl`, the CLI, the revision worker's directive, and the follow-up classifier's prompt all agree they no longer appear. The "Buckets 1 & 3" and "Override re-homes the comment" sections above are amended in place to describe this as-built state rather than the original nudge-posting requirement.
