...
**Override re-homes the comment.** An override does not just relabel: it re-runs routing so the comment lands where it would have been had it been classified that way to begin with. `intent` is the label, but `status` is what every consumer gates on and the nudge thread entry is what the sidebar's chip and banner gate on, so all three move together. The nine directed transitions, keyed on the comment's status _before_ the override:

| before                                                 | new intent                    | what happens                                                                                                                                   |
| ------------------------------------------------------ | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `answering`                                            | `directive` / `larger_change` | the in-flight answer-agent run is **stood down** (below), the comment goes `→ active` in the same write as the intent, and the nudge is posted |
| `answered` / `awaiting_followup`                       | `directive` / `larger_change` | `→ active`, nudge posted                                                                                                                       |
| `active`                                               | `directive` / `larger_change` | nudge posted (already on the revise track)                                                                                                     |
| `active`                                               | `question`                    | answer agent spawns, `→ answering`                                                                                                             |
| `directive` ↔ `larger_change`, `question` → `question` | —                             | label only; both are already on the right track                                                                                                |
