...
**Override re-homes the comment.** An override does not just relabel: it re-runs routing so the comment lands where it would have been had it been classified that way to begin with. `intent` is the label, but `status` is what every consumer gates on and the nudge thread entry is what the sidebar's chip and banner gate on, so all three move together. The directed transitions, keyed on the comment's status _before_ the override:

| before                                           | new intent | what happens                                                                                                                                                                                             |
| ------------------------------------------------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `answering`                                      | `revision` | the in-flight answer-agent run is **stood down** (below), the comment goes `→ active` in the same write as the intent, and the nudge is posted (nudge is scope-gated on `resolve_doc_owner`, 2026-07-24) |
| `answered` / `awaiting_followup`                 | `revision` | `→ active`, nudge posted (scope-gated)                                                                                                                                                                   |
| `active`                                         | `revision` | nudge posted (already on the revise track; scope-gated)                                                                                                                                                  |
...
The nudge is posted at most once per comment: flipping the classification back and forth must not spam the thread, and `revisionChipState` only needs one entry to render.
