...
Boothby (after the Starfleet Academy groundskeeper) is an autonomous, coordinator-privileged maintenance agent. He is "the Final Boss": full coordinator-level access plus elevated powers the ordinary coordinator lacks.

## What he does

His primary job is keeping the Boss taxonomy tidy — closing stale and duplicate tasks, merging attention items, archiving empty projects, unwedging stuck work. His secondary job is improving Boss itself — mining the engine's logs and agent transcripts for errors, anomalies, and friction, and filing chores or `boss shake` issues. The elevated powers are chiefly the ability to read the coordinator's own transcripts.

Every action he takes is durably journaled, visible in a dedicated UI pane, and — where the action is reversible — undoable with one click.
...
- **Sweep fleet**: all periodic reconcilers live in `engine/core/src/app/server.rs::serve` on `spawn_sweep_loop` (fire once at boot, then per-interval). Known residual gaps flagged in-tree: recovery patches under `<state_root>/recovery/` are never GC'd — note these are now load bearing (replayed on resume by `coordinator.rs::reconcile_workspace_recovery`), so only `*.patch.applied` is safe to age out (`docs/post-crash-recovery.md`); closed-unmerged PRs collapse the task to `done` because `chore-lifecycle-pr-closed-unmerged.md` is unbuilt (`work/pr_flow.rs`); boot-time `run_reconcile.rs` `Unknown` liveness verdicts are only `warn!`-logged; `NudgeBreakerParked` / `waiting_human` rows have no auto-unpark; abandoned revision rows are never pruned; `envelope_watch.rs` and `syspolicyd_monitor.rs` are detect-only.
...
- **Privilege machinery**: `RpcTier { User, AppOrBoss, BossOnly }` with two trust roots (`app_pid` from the app process, `boss_pid` from the engine-owned coordinator tmux pane pid) enforced by `authorize_rpc` peer-pid ancestry (`engine/core/src/app.rs`). The engine installs `boss_pid` when it creates or recovers the coordinator session (no app-side registration RPC). Coordinator-ness is "bossctl on PATH + pid ancestry", not a credential.
...
| 17  | Recovery-patch GC                           | delete `<state_root>/recovery/<exec-id>.patch.applied` older than `boothby.recovery_patch_ttl` (default 30 d). **Un-consumed `*.patch` files are NOT auto-deletable** — see the note below | I     | —                                                                                                                                     | auto (consumed only); `propose` for un-consumed                     | 20 files           |
| 18  | Cancel a ghost execution                    | `cancel_execution` on rows the sweeps flagged as unowned                                                                                                                                   | S     | re-request execution                                                                                                                  | propose                                                             | 2                  |

> **Action #17 and the load-bearing patch problem.** This row was written when recovery patches were write-only artifacts nothing read, so deleting an aged one cost nothing. That is no longer true: `coordinator.rs::reconcile_workspace_recovery` replays them on resume dispatch (see `docs/post-crash-recovery.md`). An un-consumed `<exec-id>.patch` is either (a) waiting for a resume that has not been dispatched yet — a work item can sit in `todo` far longer than any TTL — or (b) the residue of a **failed** apply, deliberately left on disk as the only remaining copy of that work. Deleting either on a timer destroys exactly what the recovery pipeline exists to preserve. Successful recoveries rename the file to `<exec-id>.patch.applied`, which is safely aged out because the work it held is already in a workspace. So: `.patch.applied` GCs automatically; a bare `.patch` may only ever be _proposed_ for deletion, with the finding naming the execution and its work item's current status so a human can see whether a resume is still owed.
...
- **Boothby never gains**: engine shutdown (token-gated, unchanged), coordinator lifecycle / trust-root installation, editorial-rule editing, settings writes (operator-only), or undo of his own actions.
...
3. **Coordinator-transcript discovery is heuristic.** With no engine-held pointer to the coordinator's session, discovery-by-cwd-slug can miss sessions (nonstandard launch dirs) or over-match. A follow-up that records the coordinator's `session_id` on the engine-owned coordinator creation path (when the durable tmux singleton is created or recovered) would make it exact; v1 ships the heuristic plus that chore.
