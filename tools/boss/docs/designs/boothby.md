...
3. **The Boothby tab** (SwiftUI): his live pane, the audit feed with undo buttons, pass history, findings, and controls.
...
```text
            timer (adaptive)      engine events (debounced)     boss boothby run / UI
              ...
              (audit journal)   chore/shake filing, cube verb)
                           │
                           ▼
                    Boothby tab: audit feed + undo   /   boss boothby undo <id>
```

### Activation model

**Timer.** A new `boothby_scheduler` loop wired in `server.rs::serve`, following `automation_scheduler.rs`'s adaptive pattern: sleep until the next due instant, clamped, woken early by a `kick` `Notify`. Cadence comes from the settings registry key `boothby.schedule` — a cron expression evaluated with the existing `automation_schedule.rs` cron/timezone math. **Default: every 2 hours** during v1 bring-up (groundskeeping is never urgent; a laptop that sleeps through fires simply catches up on the next wake — one pass, never a backlog, because passes are stateless-by-design and always evaluate _current_ state).
...
- **Dedicated slot, not a pool.** One reserved pane slot in a new `boothby` slot namespace (`boothby-1`), hosted by the app inside the Boothby tab the way `workBossPanel` hosts the Picard pane — Boothby never competes for, or appears in, the worker/automation/review pools.
...
| 4   | Merge duplicate attentions                  | `AttentionMerge` fold (trigger value `boothby`), `retire_group_if_empty`                                                                  | R     | un-fold: restore member per pre-image, mark ledger row reversed | auto                                                                | 3                  |
...
A new top-level tab: `case boothby = "Boothby"` on `NavigationMode` (`Models.swift`), rendered in `ContentView` following the `AutomationsView` pattern (structurally conditional `NavigationSplitView`). Layout:

- **Sidebar**: pass history (rows: trigger icon, outcome badge, counts, relative time — the `AutomationRunRow` shape), plus a findings section with status chips.
- **Detail, default**: the **audit feed** — a freshest-first timeline of `boothby_actions` modeled on `ActivityLogView`'s `ActivityRow` (headline = verb + target short-id, subline = rationale, outcome color), each row expandable to pre/post images, with an **Undo** button on `undoable` rows using the `PlannerUndoButton` confirmation-dialog pattern (and a "Conflicted — review" state showing both images). Selecting a pass filters the feed to it and links to his transcript in the transcript viewer.
- **Detail, live**: Boothby's terminal pane (the `workBossPanel`/`BossPaneModel` hosting pattern), shown while a pass runs — collapsed to a status strip when idle.
- **Header controls**: mode picker (`off`/`propose`/`auto`), Run now, next-due time, and a proposals badge deep-linking to the Notifications window (proposals ride the existing `AttentionsView` yes/no cards — approve/deny is one click there).
...
2. **`last_status_actor` widening.** Adding `boothby` to a value set consumed in several places (`human|boss|engine` today) needs an exhaustive-match audit; cheap but easy to fumble. Alternative: reuse `engine` and rely on the journal for attribution — rejected in this draft because human-veto detection keys on actor identity, but flagging it.
...
5. **Undo of attention merges** restores the folded member but cannot un-ring group retirement side effects in every corner (e.g. an actioned group). v1 classifies merge-undo as best-effort R with a `conflicted` fallback; reviewer confirmation wanted that this is acceptable rather than demoting merges to `propose`.
...
7. **Cost.** A 2-hour cadence of opus passes is nontrivial spend. Mitigations: `nothing_to_do` short-circuit (the brief composer can conclude _before spawn_ that no candidates exist and skip the session entirely — recorded as a pass with no session), plus the manifest question on cadence.
...
Entries are one reviewable PR by one worker in one session. Depth markers group entries that may start in parallel once their dependencies merge.

1. **Boothby schema + protocol types** — Migrations for `boothby_passes`, `boothby_actions`, `boothby_findings`, `boothby_cursors`; `bon::Builder` protocol structs; the `boothby` `last_status_actor` constant + exhaustive-match audit; `created_via` prefix `boothby:`. Effort: `medium`. Depends on: none.
2. **Actor-attributed pre-image capture in WorkDb** — Extend the mutation layer (`updates.rs`/`workitems.rs`/`attentions.rs`) so actor-`boothby` mutations capture pre/post images of touched columns in-transaction and append `boothby_actions` rows; no behavior change for other actors. Effort: `medium`. Depends on: 1.
3. **Guarded action executor** — Engine module owning the verb catalogue table (autonomy, caps, reversibility), per-pass cap accounting, live-work/human-touch/lease guards, `confirm_two_pass` for I-class verbs, fingerprint refusal, and journal writes for non-WorkDb effects; exposed via `boothby`-tier RPCs (`boothby.act`). Effort: `large`. Depends on: 2.
4. **Undo engine + retention sweep** — Conflict-checked pre-image restore, `undo_state` lifecycle, fingerprint auto-suppression on undo/human-veto detection, `UndoBoothbyAction` RPC, retention prune of passes/actions. Effort: `medium`. Depends on: 3.
5. **Boothby scheduler + pass lifecycle + settings** — `boothby_scheduler` loop (adaptive sleep + kick), settings keys (`boothby.mode/schedule/caps/…`), `boothby_passes` lifecycle, `nothing_to_do` pre-spawn short-circuit, `ListBoothbyPasses`/`GetBoothbyState`/`RunBoothbyPass`/`SetBoothbyMode` RPCs + `boothby.activity` topic. Effort: `medium`. Depends on: 1. (Parallel with 2–3.)
6. **Event triggers + debounce** — Fan-out `DispatchEventSink` + attention-creation hooks feeding the scheduler's kick with coalescing, `event_delay`, `min_pass_gap`; unknown stages are no-ops. Effort: `small`. Depends on: 5.
7. **Pass brief composer** — SQL prefilters (stale tasks, empty projects, aged attentions, near-dup titles), sweep-leftover collection (parked rows, `Unknown` verdicts, open worker-signal attentions), caps-remaining snapshot, brief.json rendering. Effort: `medium`. Depends on: 5. (Parallel with 6.)
8. **Boothby session spawn** — `WorkerKind::Boothby`, `<state_root>/boothby/home/` provisioning, contract `CLAUDE.md` + allowlist settings template (answer-agent pattern), dedicated `boothby-1` slot via `SpawnWorkerPane`, `boothby_pid` trust-root registration, session/transcript capture onto the pass, pass timeout, `pass-summary` verb. Effort: `large`. Depends on: 5, 7.
9. **RPC tier extension + transcript access** — `BoothbyOrApp` tier in `authorize_rpc` (incl. worker-denylist exclusion), `ListAgentSessions`/`ReadTranscriptSegments` RPCs over worker scopes, coordinator-session discovery heuristic + the exact-pointer chore (record coordinator `session_id` at `RegisterBossSession`). Effort: `medium`. Depends on: 8.
10. **Findings ledger + filing router** — Fingerprint upsert/dedup, `file-finding` verb with routing (`direct_developer_mode` OR Boss-product presence → chore with `created_via`; else shake path with `boothby` label + finding trailer), search-before-file, outbound sanitizer + editorial pass, suppression on human-close. Effort: `medium`. Depends on: 3. (Parallel with 5–9.)
11. **Log-mining inputs** — Cursored readers over `engine-trace.jsonl` (+ rotation), `engine-audit.log`, dispatch-events JSONL; error-burst counter for the trigger; extracted spans summarized into the brief. Effort: `medium`. Depends on: 7.
12. **Transcript-mining inputs** — Worker/coordinator transcript enumeration + cursors, friction heuristics (retry chains, escalation patterns, audit-effort cross-check), spans into the brief. Effort: `medium`. Depends on: 9, 11. (Co-edits the brief composer with 11 — sequenced after it; forward-port 11's changes preservingly.)
13. **Grounds-keeping verb set** — Executor wiring for catalogue #1–8, #11–12 (taxonomy verbs): archived-reason conventions, duplicate cross-links, attention dismiss/merge/resolve with pre-images, effort re-run, PR-drift fix, review nudge. Effort: `medium`. Depends on: 3. (Parallel with 10; distinct files.)
14. **Unwedge verb set** — Executor wiring for #13–15, #18: reap path reuse, redispatch/unpark (incl. nudge-breaker reset verb), engine-mediated cube force-release, ghost-execution cancel; two-pass confirmation throughout. Effort: `medium`. Depends on: 3. (Parallel with 13; touches coordinator/sweep call sites, not 13's files.)
15. **Hygiene GC verbs** — #16–17: dry-run-then-apply `cube workspace reconcile`/`gc` invocation, recovery-patch GC with TTL. Effort: `small`. Depends on: 3. (Parallel with 13–14.)
16. **Proposal flow via attentions** — Propose-mode conversion of intended actions into attention groups (yes/no members), approve→execute through the executor, deny→suppress; proposals badge data. Effort: `medium`. Depends on: 13 (first consumers; 10/14/15 verbs join for free).
17. **CLI `boss boothby` noun** — All operator verbs (`status/run/enable/disable/mode/passes/actions/undo/findings/suppressions`), `--json` via `print_entity`. Effort: `medium`. Depends on: 4, 5.
18. **App: Boothby tab + pass history + audit feed** — `NavigationMode.boothby`, split view, pass rows, `ActivityRow`-style action timeline with pre/post-image expansion, topic subscription. Effort: `large`. Depends on: 5 (RPCs), 4 (undo states render read-only until 19).
19. **App: undo affordance + proposals badge** — `PlannerUndoButton`-pattern undo with confirmation + conflicted-state review UI; header controls (mode picker, Run now); proposals badge → Notifications deep-link. Effort: `medium`. Depends on: 18. (Co-edits the Boothby views with 18 and 20 — sequenced 18 → 19 → 20; each later task forward-ports preservingly.)
20. **App: live Boothby pane** — Host the `boothby-1` pane in the tab (`BossPaneModel` pattern), live/idle states, transcript-viewer link. Effort: `medium`. Depends on: 8, 18. (Sequenced after 19 per the co-edit note on 19.)
21. **End-to-end validation pass** — Seeded test-instance fixture (stale tasks, dup attentions, parked execution, dead-pid execution, log error burst); drive a propose-mode and an auto-mode pass; assert journal completeness, undo round-trips, fingerprint dedup, cap enforcement, veto suppression. Effort: `medium`. Depends on: 13, 14, 15, 16, 17 (engine surface complete; app not required).
22. **Trust ratchet (auto-promotion of verbs after N approvals)** — `future / not a v1 blocker`. Depends on: 16.
23. **Remote-host awareness (Boothby acts on remote executions/leases)** — `future / not a v1 blocker`. Depends on: 14.
24. **Metrics-anomaly detection (statistical baselines over `metrics_counter`)** — `future / not a v1 blocker`. Depends on: 11.
25. **Dispatch pause/resume as a Boothby verb** — `future / not a v1 blocker`; deliberately excluded from v1 blast radius. Depends on: 14.

Parallelism at a glance: after 1 merges, {2, 5} run in parallel; after 3, {4, 10, 13, 14, 15} are all parallel (distinct subsystems/files); {6, 7} parallel under 5; {11} then {12}; app track {18 → 19 → 20} is serialized by design (same view files) but runs parallel to the engine tracks once 5 lands; 21 is the integration gate before enabling `propose` mode by default.
