...
- **Planner** — the reusable LLM mini-coordinator. Entry point `Planner::plan(api_key, &PlannerInput) -> PlannerOutcome`. (This is the P6 "planner agent.")
...
The breakdown is authoritative: its parsed entry count determines the emitted task-row count. There is no per-task decomposition gate; the Planner does not re-prompt to subdivide an entry based on its prose.
...
6. **Engine: validation layer** — **PR #1444.** Seven ordered no-op-safe checks; `breakdown_found` and confidence handling. It was later extracted into the `boss-engine-planner-validation` crate; its subsequently added oversize-task decomposition gate was removed in mono#2695. _Depends on: 1._
