...
- **Implicit force-push.** `cube pr push` never force-pushes implicitly. An explicit `--force-with-lease` flag is required for any rewrite scenario; it is included in v1 as a guarded opt-in (see *Fast-forward vs force* in the `cube pr push` section).
...
- **R3 — `cube pr push` PR-inference ambiguity.** Inferring the PR from `@`'s ancestry could be ambiguous if multiple `pr/<n>` bookmarks sit on the ancestry (e.g. stacked work). *Decided:* ancestry inference is the default (nearest `pr/<n>` ancestor of `@`); `--pr`/`--branch` are explicit overrides for disambiguation. If no `pr/<n>` ancestor is found, `cube pr push` errors with an instruction to pass `--pr`. Boss always knows the PR number and can pass it when disambiguation is needed.
- **R4 — Force-push policy.** *Decided:* include a guarded `--force-with-lease` flag in v1 (see *Fast-forward vs force* in the `cube pr push` section). The default remains fast-forward-only; `--force-with-lease` is always explicit and verified against the last-fetched remote head before pushing. An implicit force is still never permitted.
- **R5 — Merged/closed PR handling.** *Decided:* hard error for both `--resume_pr` and `cube pr push` — cube refuses to operate on a non-open PR in either case. `gh pr view <n> --json state` is checked; a MERGED or CLOSED result is a hard error with a clear message. Inspection of merged branches is out of scope; callers should use `gh` directly for read-only purposes.
