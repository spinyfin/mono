...
Two properties carried over deliberately: a **read-only preflight step** (build, lint, dry-run) runs unconditionally and separately from the **mutating deploy step**, which alone declares secrets and alone is gated by `concurrency: 1` on a named `concurrency_group` so two deploys can never race. The concurrency group name is relay-specific — `mono-relay-release-mutation`, not `flunge-release-mutation` — because concurrency groups are a cluster-wide namespace and joining flunge's would serialize unrelated deploys against each other for no reason.
...
**Pipeline config lives in the repo, not only in Buildkite's stored web config.** Mono's current pipeline-registration story is a hand-run markdown checklist ([`tools/checkleft/docs/buildkite-release-setup.md:26-100`](../../../checkleft/docs/buildkite-release-setup.md)) whose result — the server-side bootstrap stub — is not itself version-controlled. Flunge learned this the hard way and codified the fix: `infrastructure/buildkite/pipelines/*.stub.yml` holds each pipeline's bootstrap stub in-repo, and `infrastructure/scripts/sync-buildkite-pipeline.sh --check` detects drift between the stored config and the repo copy. The motivating incident is on the record at `docs/infrastructure.md:101` (flunge): a stored-config-only pipeline had `BUILDKITE_API_TOKEN` removed from the repo YAML with nothing to push that change to the live pipeline, so the live config kept requesting a secret that no longer existed, and every trigger failed with "Secret not found or access denied" until someone diagnosed it by hand — in production. The relay's deploy task adopts the same shape: `infrastructure/buildkite/pipelines/mono-relay-release.stub.yml` in-repo, synced with the same `--check` pattern. Two gotchas carry over directly, both hit on first setup if skipped: **the stub's `agents: queue:` must be a literal string** — Buildkite's stored-config steps editor rejects `${VAR:-default}` interpolation outright (`422 Interpolating "..." is not supported in the steps editor`), even though the _uploaded_ pipeline YAML can and does use `${BUILDKITE_RELEASE_QUEUE:-linux-release}`-style interpolation freely; and **an untargeted stub step fails outright** with "no queue has been targeted" if the queue is omitted, because the cluster has no default queue.

**Trigger model: left as an operator decision, not picked here.** See item 10 of [Risks / open questions](#risks--open-questions) — this is a monorepo, so a naive push-to-`main` trigger fires the relay deploy on every mono merge regardless of what changed, which neither of the two in-org precedents does carelessly (flunge's staging pipeline is push-to-`main` but flunge is a single-product repo; `mono-checkleft-release` is push/PR-triggers-off, cron plus manual, specifically because mono is not). Do not default to push-to-`main` without a path gate or a durable release tag; do not silently pick cron-and-manual either. Flagged for the operator, with both options and their precedents named.
...
This section covers the relay being compromised at runtime. It is a distinct threat model from a compromised or malicious CI agent reading the relay's secrets straight out of the shared Buildkite cluster before the relay ever runs — that is the concern raised in [Repo placement](#repo-placement) and item 4 of [Risks / open questions](#risks--open-questions), and it is not mitigated by anything below.
