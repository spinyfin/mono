...
- **Project:** "GitHub webhook ingress: push-based GitHub events for Boss engines".
- **Provenance:** authored by a design-producing run on that project; the questions it raises for a human are in the sibling `.attentions.json`.
- **Code citations:** all against `spinyfin/mono` @ `2c618cb36bb75f197ce1f882f1f52fa8a5bff731` unless stated. The task brief's pre-gathered inventory cites `e7fb0b11f271efc75771ff4154ef53f21dfafb68`; every claim reused from it was re-verified at the newer sha and line numbers below are the newer ones. **Deploy-pipeline citations against `spinyfin/flunge`** (the [Deploy pipeline](#deploy-pipeline) section and its dependents) are against `9ca40b9ec5822d84981c580520f3a162b7a4d9ad`, re-verified directly against `main@origin` in the shared flunge checkout during this revision — both repos move, re-verify before relying on line numbers.
- **Related design docs:** [`engine-event-bus-…`](./engine-event-bus-event-driven-reconcilers-via-an-in-process-message-queue.md) (the landing zone this design produces onto; scoped webhooks out and named them a follow-up — this is that follow-up); [`engine-counter-metrics-framework.md`](./engine-counter-metrics-framework.md) (the counter surface used for shadow mode); [`trunk-merge-queue-integration-…`](./trunk-merge-queue-integration-queue-backed-merges-merging-ui.md) (the credential-storage precedent this reuses verbatim); [`maintenance-tasks.md`](./maintenance-tasks.md) (catalogues the sweeps reclassified here); [`auto-populate-project-tasks-on-design-pr-merge.md`](./auto-populate-project-tasks-on-design-pr-merge.md) (consumer of this doc's task-breakdown section); [`flunge-buildkite-pipeline-reference.md`](./flunge-buildkite-pipeline-reference.md) (mono's own prior art surveying flunge's Buildkite pipelines, cited throughout [Deploy pipeline](#deploy-pipeline)).
- **Related work items**, referred to by these names throughout: **the GitHub API instrumentation chore** (hard prerequisite, in progress, PR #2349); **the merge-poller drain chore** (overlapping, blocked on the instrumentation chore); **the `boss pr status` / `boss pr body` chore**; **the GitHub-App installation-token chore** (not started, opt-in); **the PR/CI/conflict reconciliation project**; **the engine event-bus project** (design merged).
...
- **Cut GitHub GraphQL spend by at least 50% without lengthening worst-case transition-detection latency**, measured with the instrumentation chore's per-subsystem counters, against the measured 3,900–6,900 pts/hr baseline.
...
- **Batching or trimming the poller's own queries.** That is the merge-poller drain chore's scope. See [Relationship to the instrumentation and merge-poller-drain chores](#relationship-to-the-instrumentation-and-merge-poller-drain-chores).
- **Fixing the shared-personal-token problem.** The webhook secret is a different credential axis from the polling token. This design neither fixes nor entrenches it; the GitHub-App installation-token chore owns that.
- **Assuming the GitHub-App installation-token chore's App exists.** That chore has not started and is opt-in. v1 ships against a repo-level webhook and treats the App as a later, separately-decided migration.
...
Assumptions, stated so they can be checked: N ≈ 25 tracked open PRs (the merge-poller drain chore's figure); Boss opens and merges ~20 PRs/day on a busy day; ~20 merges/day to `main`; per-PR debounce W = 40s. Steady state, per hour.
...
- **Cross-PR batch window, ±5s, owned by the merge-poller drain chore.** Input: the due set. Effect: PRs becoming due within 5s of each other go through `probe_batch` in **one** GraphQL query, `ceil(81N/100) + ceil(20N/100)` instead of `2N`.
...
### Alternative E: Azure Container Instances running a plain `axum` service

A conventional Rust binary in a container: `axum` for HTTP, native WebSocket/SSE support, a real long-lived process, built by bazel `rules_oci`, pushed to ACR, deployed with `az container delete` + `az container create`. This is not a hypothetical shape — it is flunge's actual backend deploy, verbatim: `cli/release_orchestrator/src/mutate.rs:103-129` in `spinyfin/flunge`. (An earlier draft of this section named Azure Container **Apps** and `az containerapp update`; that has no in-org precedent — flunge runs on Container **Instances**, not Container Apps, and there is no `containerapp` reference anywhere in flunge's deploy tooling. Corrected here because the choice of what to reject should be argued against what actually exists, not against a fancier Azure product nobody at the org runs.)

**Rejected, but the trade is narrower than it first looks, because flunge already paid most of the "second cloud" cost.** It is the _only_ option where "a simple Rust HTTP service" means exactly what it sounds like, with no wasm, no `wasm-bindgen`, and no toolchain novelty. It does **not** need a new container registry, a new image-build path, or a second cloud account's credentials in mono's CI — ACR, a working bazel `oci_push` path, and Azure service-principal credentials for deployment all already exist in the shared Buildkite cluster (flunge's release pipelines use them today; see [Deploy pipeline](#deploy-pipeline) and the Buildkite secret-scoping item in [Risks / open questions](#risks--open-questions) for why that is a secrets-boundary concern rather than a provisioning one). What it does still cost, and what keeps it rejected: ACI has **no scale-to-zero** — unlike Container Apps, there is no `min-replicas` knob, the container simply runs or doesn't — so holding client long-poll connections means paying for a continuously-running instance, roughly $10–20/month, against $0 on Cloudflare's free tier. And flunge's own deploy shape is delete-then-recreate (`az container delete` followed by `az container create`), which is a real downtime window on every deploy, not a rolling or blue-green update — a property this design would inherit, not invent, if it followed flunge's precedent. See [Hosting](#hosting-cloudflare-vs-azure) for the full trade, and the [attentions manifest](#risks--open-questions) — this is flagged for an operator decision, because the argument turns on how much the Rust-on-wasm friction is worth against a downtime-on-deploy, always-on-cost alternative.
...
Operator-decided and not re-litigated: the service lives in `spinyfin/mono` under `tools/boss/`, sibling to `engine/`. Consequences are inherited and worth naming rather than discovering later.

- **A deployed network service now lives in the monorepo.** `checkleft`, `file/size`, and the bazel lint/format checks apply to it like any other crate, which is good. But mono has never hosted a production deployment before — every mono "release" today publishes a GitHub Release artifact (the checkleft prebuilt binaries; the macOS app's DMG), not a running service. This is a first, and it is worth being explicit that mono's own prior art deliberately scoped this out: [`flunge-buildkite-pipeline-reference.md:194`](./flunge-buildkite-pipeline-reference.md) — _"Release pipelines stay out of scope… Release pipelines for the macOS app or any future surface are a follow-up."_ This design is that follow-up, for the relay specifically.
- **Deploy cadence does _not_ couple to mono's PR CI.** The relay deploys from its own Buildkite pipeline, not a step in `.buildkite/pipeline.yml`. This isn't a novel ask inside mono: `mono-integrity` and `mono-checkleft-release` are both already independent pipelines with their own cron/manual triggers, disconnected from the three-check PR gate — [`tools/checkleft/docs/buildkite-release-setup.md:5`](../../../checkleft/docs/buildkite-release-setup.md) draws the identical contrast against the in-pipeline `boss-release` step: _"Boss releases run as a step inside the existing `mono` pipeline, while checkleft releases run as a **separate pipeline** with its own cron schedule and its own manual trigger."_ The relay pipeline follows the checkleft shape, not the boss-release shape. See [Deploy pipeline](#deploy-pipeline) for the concrete design. A relay hotfix therefore does _not_ need to ride an unrelated mono PR through the three required PR checks — but a relay change still originates as a commit in `spinyfin/mono`, so getting a fix from commit to deployed artifact is bounded by this new pipeline's own trigger and queue, not by mono's PR CI cadence.
- **Hosted-service secrets widen what the shared Buildkite cluster reaches — not just "mono's CI."** This is the corrected framing; see the Buildkite secret-scoping item in [Risks / open questions](#risks--open-questions) for the full argument. The short version: flunge and mono share one Buildkite org and one cluster, all existing cluster secrets are currently unscoped (`policy: ""`, no pipeline or queue restriction), and mono CI agents already reach production Azure and Cloudflare credentials as a result. Any relay secret added to the cluster becomes readable by all of flunge's release pipelines too, not just the relay's own pipeline — "the deploy step runs only on `main`" restricts _when_ the step runs, not _which pipelines can read the secret_, so it does not mitigate this. Genuine mitigation is secret-level pipeline scoping, which nothing in the cluster uses today; whether the Buildkite plan tier even supports it is unverified. See [Blast radius](#blast-radius) for what a compromised relay secret can and cannot do to the relay itself, independent of this CI-boundary concern.
...
**The relay deploys from its own Buildkite pipeline, not a step in `.buildkite/pipeline.yml`.** Config lives at `.buildkite/pipeline-relay-release.yml`, registered as a new Buildkite pipeline with a `mono-*` slug (`mono-relay-release`), matching the existing `mono-integrity` / `mono-checkleft-release` naming convention. This decouples the relay's release cadence from mono's PR-gating CI: a relay deploy is not blocked on, and does not block, the three required checks in `.buildkite/pipeline.yml`.

**Deploy shape, adopted from flunge's release pipelines rather than invented fresh** (`spinyfin/flunge` `.buildkite/release-staging.pipeline.yml:1-26`):

```yaml
steps:
  - label: ":mag: Relay release preflight"
    key: "relay-release-preflight"
    command: ".buildkite/steps/relay-release.sh preflight"
    agents:
      queue: "linux-release"

  - label: ":satellite: Relay release deploy"
    key: "relay-release-deploy"
    depends_on: "relay-release-preflight"
    command: ".buildkite/steps/relay-release.sh deploy"
    agents:
      queue: "linux-release"
    concurrency: 1
    concurrency_group: "mono-relay-release-mutation"
    secrets:
      - "RELAY_WEBHOOK_SECRET"
      - "RELAY_CLOUDFLARE_API_TOKEN"
```

Two properties carried over deliberately: a **read-only preflight step** (build, lint, dry-run) runs unconditionally and separately from the **mutating deploy step**, and that mutating step alone is gated by `concurrency: 1` on a named `concurrency_group` so two deploys can never race. A third property is a deliberate **tightening of flunge's shape rather than a copy of it**, and is labelled as such because everything else in this section is checkable against flunge and this one would not check out: here the mutating step is the only step that declares secrets at all, whereas flunge's preflight also carries `REGISTRY_USERNAME` and `REGISTRY_PASSWORD` (`.buildkite/release-staging.pipeline.yml:7-9`) and its mutate step declares those plus five more. The relay's preflight needs no credential to build and lint, so it should not hold one. The concurrency group name is relay-specific — `mono-relay-release-mutation`, not `flunge-release-mutation` — because concurrency groups are a cluster-wide namespace and joining flunge's would serialize unrelated deploys against each other for no reason.

**Queue: `linux-release`.** It already exists in the shared Buildkite cluster, is unused by any mono pipeline today, and its single agent is proven green for bazel, docker, `az`, and `npx` — flunge's release pipelines run all of those on it. Two caveats that make this a considered choice, not a default: it is a **single point of failure** (one agent, no redundancy), and it is **shared with flunge's own release train** — a relay deploy and a flunge release can contend for the same agent. Neither caveat is disqualifying (deploys are infrequent and the relay's failure mode is graceful), but both should be visible rather than discovered during an incident. There is also a toolchain gap to close, not assume away: per [`flunge-buildkite-pipeline-reference.md:185`](./flunge-buildkite-pipeline-reference.md), flunge's agents do not have Rust installed — the queue is proven for bazel/docker/az/npx, not for a bare Rust build. The relay's deploy step must provision mono's toolchain itself (e.g. `rustup` from `rust-toolchain.toml` at job start, the same install-on-boot approach the mono CI reference design proposes for v1) rather than assume a flunge-green host is mono-ready.

**Bazel stays the source of truth for the deploy artifact — no Dockerfile.** Mirroring flunge's backend exactly: an `oci_image` + `oci_push` pair in the relay's `BUILD` file (flunge's own is `backend/BUILD:32-84`), pushed via `bazel run` with BuildBuddy registry-credential headers passed as `--remote_exec_header` flags (flunge's `cli/release_orchestrator/src/mutate.rs:61-80`). If the Azure fallback from [Hosting](#hosting-cloudflare-vs-azure) is ever taken, the container lifecycle itself also mirrors flunge verbatim: `az container delete` followed by `az container create --file <manifest>` (`mutate.rs:103-129`) — a delete/recreate cycle, not a rolling update.

**Pipeline config lives in the repo, not only in Buildkite's stored web config.** Mono's current pipeline-registration story is a hand-run markdown checklist ([`tools/checkleft/docs/buildkite-release-setup.md:26-100`](../../../checkleft/docs/buildkite-release-setup.md)) whose result — the server-side bootstrap stub — is not itself version-controlled. Flunge learned this the hard way and codified the fix: `infrastructure/buildkite/pipelines/*.stub.yml` holds each pipeline's bootstrap stub in-repo, so the **only** thing Buildkite stores server-side is a stub that uploads the real step graph from the commit being built. The motivating incident is on the record at `docs/infrastructure.md:101` (flunge): a stored-config-only pipeline had `BUILDKITE_API_TOKEN` removed from the repo YAML with nothing to push that change to the live pipeline, so the live config kept requesting a secret that no longer existed, and every trigger failed with "Secret not found or access denied" until someone diagnosed it by hand — in production.

**The load-bearing property is the stub, not a drift checker, and the distinction is worth stating because getting it wrong provisions a credential this design otherwise avoids.** Flunge's own record frames the two as opposed rather than complementary: a stub-only stored config "makes config drift structurally impossible rather than merely detectable… because there is nothing in it to go stale" (`docs/infrastructure.md:101`), whereas "a drift _checker_ would need [a Buildkite user API token]: Buildkite's REST `GET /pipelines/{slug}` accepts only a user API access token… That credential was already judged too risky for build dispatch… and removing the need for it is the point of this arrangement" (`:103`). Flunge does keep `infrastructure/scripts/sync-buildkite-pipeline.sh --check`, but strictly as a **rare operator action** — "needed when bootstrapping a new pipeline, changing a stub's queue, or changing one of the settings booleans" (`:126`) — and never as a step any job runs. The relay must adopt it on exactly those terms: **`--check` is an operator-run bootstrap and verification tool, not a CI gate.** Wiring it into the relay pipeline would require provisioning the long-lived org-scoped user API access token that the stub arrangement exists to make unnecessary.

The relay's deploy task therefore adopts the same shape: `infrastructure/buildkite/pipelines/mono-relay-release.stub.yml` in-repo, with `--check` available to the operator for bootstrap and periodic verification. Two gotchas carry over directly, both hit on first setup if skipped: **the stub's `agents: queue:` must be a literal string** — Buildkite's stored-config steps editor rejects `${VAR:-default}` interpolation outright (`422 Interpolating "..." is not supported in the steps editor`), even though the _uploaded_ pipeline YAML can and does use `${BUILDKITE_RELEASE_QUEUE:-linux-release}`-style interpolation freely; and **an untargeted stub step fails outright** with "no queue has been targeted" if the queue is omitted, because the cluster has no default queue.

**Trigger model: left as an operator decision, not picked here.** See the trigger-model item in [Risks / open questions](#risks--open-questions) — this is a monorepo, so a naive push-to-`main` trigger fires the relay deploy on every mono merge regardless of what changed, which neither of the two in-org precedents does carelessly (flunge's staging pipeline is push-to-`main` but flunge is a single-product repo; `mono-checkleft-release` is push/PR-triggers-off, cron plus manual, specifically because mono is not). Do not default to push-to-`main` without a path gate or a durable release tag; do not silently pick cron-and-manual either. Flagged for the operator, with both options and their precedents named.
...
**The asymmetry worth naming:** the webhook secret is a different credential axis from the polling token. This project does not fix the shared-personal-token problem (the GitHub-App installation-token chore's job) but it does not entrench it either — nothing here makes the ambient `gh` login more load-bearing than it already is, and the relay adds no new consumer of it.
...
|                     | Cloudflare Workers + DO                                                                                                                                               | Azure Container Instances                                                                                                                                                                                                                                                                   |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Bazel integration   | Toolchain already carries `wasm32-unknown-unknown` (`MODULE.bazel:134`), but `worker-build` is a non-bazel glue step; needs an `sh_binary` wrapper around `wrangler`. | `rules_oci` `oci_image` + `oci_push` → ACR → `az container delete` + `az container create`. Flunge's exact shape (`backend/BUILD:32-84`, `mutate.rs:61-129`), already proven in-org.                                                                                                        |
| Connection state    | Durable Object per repo. Wakes in ms, no cold-start penalty worth naming.                                                                                             | Real process; **no scale-to-zero** — ACI has no `min-replicas` concept, unlike Container Apps. Runs continuously or not at all. Flunge's own deploy is delete-then-recreate: a downtime window on every deploy, not a rolling update.                                                       |
| Cost at this volume | **$0** (free tier: 100k req/day; 5 engines long-polling at 25s ≈ 17k/day).                                                                                            | ~$10–20/month always-on.                                                                                                                                                                                                                                                                    |
| Ops surface         | One `wrangler deploy`, two secrets.                                                                                                                                   | Image build + ACR push + `az container create`/`delete` — on infra that already exists (see [Deploy pipeline](#deploy-pipeline)). Not "two clouds' credentials in CI": Azure SP credentials and ACR access are already Buildkite cluster secrets, used today by flunge's release pipelines. |
| Availability        | Anycast edge; no region to lose.                                                                                                                                      | Single region unless multi-region is configured.                                                                                                                                                                                                                                            |

The decisive argument is that **relay availability is a soft requirement**: when it is down, engines fall back to polling and nothing breaks. Paying for always-on compute, and accepting a delete/recreate downtime window on every deploy, to protect a property that degrades gracefully anyway is the wrong trade. Cost and ops win, so Cloudflare wins — and this holds even correcting the Azure side to its real (ACI, not Container Apps) shape, because the deciding factors are scale-to-zero and $0-vs-~$15/month, not registry or credential provisioning, which flunge has already paid for in the shared cluster.

The honest cost is the `workers-rs` friction. It is bounded — one crate, one wasm target already in the toolchain, one non-bazel glue step behind an `sh_binary` — but it is real, and the operator asked for "a simple Rust HTTP service", which on Cloudflare means Rust compiled to wasm with a JS shim rather than a Rust binary serving HTTP. **This is flagged for an operator decision** in the attentions manifest, with the fallback spelled out: if `workers-rs` under bazel proves worse than budgeted during the deploy task, the choice is between a TypeScript Worker with the wire format still owned by the Rust `relay_protocol` crate (keeps $0 and the ops story, loses "it's Rust"), and Azure Container Instances (keeps Rust, costs ~$10–20/month always-on plus a downtime window per deploy, though it reuses flunge's existing ACR/`oci_push`/Azure-SP path rather than adding a new one). Deciding that now, in the abstract, would be guessing.

### Blast radius

This section covers the relay being compromised at runtime. It is a distinct threat model from a compromised or malicious CI agent reading the relay's secrets straight out of the shared Buildkite cluster before the relay ever runs — that is the concern raised in [Repo placement](#repo-placement) and the Buildkite secret-scoping item in [Risks / open questions](#risks--open-questions), and it is not mitigated by anything below.
...
**The GitHub API instrumentation chore is the gate.** Without its per-call, per-subsystem GraphQL counters, the before/after table above is arithmetic rather than measurement, and the cutover's effect is unfalsifiable. Nothing here ships before that instrumentation is landed and has produced a baseline. That is a hard cross-project dependency, not a preference.
...
**Phase 1 — drive, sweep untouched.** Deliveries publish `PrReconcileRequested`; the adaptive per-PR timer is disabled for repos with a healthy channel. The 60s sweep does not move. Expected: ~1,600 pts/hr, a 59–77% reduction, measured on the instrumentation chore's counters. Rollback is flipping one flag.
...
**The metric that says this is working:** GraphQL points/hour attributable to the merge poller, from the instrumentation chore's counters, at or below 1,600 — **with** median transition-detection latency at or below today's and `webhook_shadow_poll_only_total` at zero. All three, or it is not working. The latency and missed-delivery terms are what stop "spend less" from being satisfiable by simply polling less.

## Relationship to the instrumentation and merge-poller-drain chores

**The GitHub API instrumentation chore — hard prerequisite, no overlap.** It measures; this changes what is measured. Nothing here ships first.

**The merge-poller drain chore — overlapping, and the split is proposed explicitly here.** Its description reserves event-driving (_"where GitHub can tell us (webhooks…), react to the event and drop the corresponding poll"_) while flagging the blocker (_"the engine runs locally, so webhook delivery is a real infrastructure problem (no public URL)… If that turns out to be the bulk of the work, raise an attention rather than absorbing it silently — it may deserve its own design."_). It did, and this is that design.

Proposed split, confirming the brief's:

- **The merge-poller drain chore owns:** batching the adaptive path, the ±5s cross-PR coalescing window, the due-set size distribution measurement, and `PR_PROBE_FIELDS` trimming (`labels(first: 30)` → 10, `reviews(last: 20)` → 5). All inside the poller. **These stay valuable whether or not webhooks ever ship** — they are what determines the quality of the fallback when the relay is unreachable, and they make the backstop sweep cheaper, which is 97% of post-cutover spend.
- **This project owns:** the relay service, the transport, engine enrollment and identity, the webhook → `PrReconcileRequested` producer, the defer-not-drop fix, the per-PR debounce, shadow mode, and the sweep-cadence relaxation.
- **The contested seam is the coalescing window, and it is not actually contested** once split correctly: the drain chore's ±5s window batches the _due set_ (input: timers), this design's 40s debounce bounds _per-PR reconcile frequency_ (input: deliveries). Different inputs, different purposes, and they compose. Both are needed.
...
**Renegotiation requested:** the merge-poller drain chore's "Event-driving" paragraph should be **struck from that chore** and treated as absorbed here, leaving it scoped to "scope + batch". Flagged in the attentions manifest as an operator decision, because retitling another active work item is not this design's call to make unilaterally.

**The `boss pr status` / `boss pr body` chore** — complementary. Webhooks make its stored state fresher, and its `observed_at` contract matters _more_ under a push model, where staleness is bursty rather than uniform.

**The PR/CI/conflict reconciliation project** — owns the reconciliation _domain_; this owns _ingress and transport_ into it. The event-bus doc flagged this seam as unconfirmed (`:248`, _"Want a reviewer/coordinator to confirm the split"_). This design does not resolve it and does not need to: it produces onto `PrReconcileRequested` and touches nothing downstream of the publish.
...
**Recorded for operator visibility, not routed for approval — and here is why that differs from the [renegotiation requested above](#relationship-to-the-instrumentation-and-merge-poller-drain-chores).** The two decisions look comparable in reach, so the asymmetry should be argued rather than left to be noticed. The difference is that the merge-poller drain chore's "Event-driving" paragraph is **live text on an active work item**, editable only by its owner, so this design can ask and nothing more — hence the attentions entry. The investigation is **absent**: there is no merged surface to retire, this design deletes nothing, and the five code comments that cite it keep pointing exactly where they point today until the separate repointing PR lands. Superseding a document nobody can currently read changes nothing an operator would need to approve _before_ it happens, and the repointing PR is itself reviewable. If the operator would rather see this as a manifest question too, it is a one-line addition.
...
- **`workers-rs` under bazel is the largest unknown.** Budgeted as one `sh_binary` wrapper around `wrangler`/`worker-build`. If it exceeds that, the fallback is a TypeScript Worker (keeps $0 and the ops story, loses "it's Rust") or Azure Container Instances (keeps Rust, costs ~$15/month always-on plus a delete/recreate downtime window per deploy, though it reuses flunge's existing ACR/`oci_push`/Azure-SP path). **Operator decision — in the attentions manifest.** Recommendation: attempt `workers-rs`, time-box it inside the deploy task, and escalate rather than absorb. Note this is a genuinely un-derisked path regardless of hosting choice: flunge's own Cloudflare usage is `wrangler pages deploy` for static assets only — zero mentions of `workers-rs`, `worker-build`, or wasm anywhere in flunge's deploy tooling. The account and CI-plumbing risk is retired by flunge precedent; the Workers/Durable-Objects-under-bazel risk is not, and this doc's framing of it as the largest unknown survives unchanged. A Workers deploy additionally needs a Cloudflare API token scoped `Workers Scripts: Edit`; whether the existing Cloudflare Pages tokens (used by flunge's frontend release pipelines) carry that scope is not determinable from either repo — budget for a new, separately-scoped token rather than assuming reuse.
- **The before/after numbers assume N ≈ 25 PRs and ~20 PRs/day.** The sweep term scales linearly in N; the event term scales in PR _throughput_, not count. If Boss's throughput doubles, v1 goes to ~1,650 pts/hr (the sweep still dominates) — the model is robust in the direction that matters. The instrumentation chore's counters are what confirm it.
...
- **Repo-level webhook vs GitHub App for v1.** Recommended repo-level for reversibility and zero new credential machinery, with the App as a tracked follow-up that should unify with the GitHub-App installation-token chore. **Operator decision — in the attentions manifest**, since it interacts with that chore's sequencing.
...
- **`is_rate_limit_error` is a bare substring match** (`metrics.rs:265-267`: `stderr.to_ascii_lowercase().contains("rate limit")`) and misses a bare `HTTP 429`. Not caused by this design and not fixed by it, but it becomes more visible when total spend drops and each remaining rate-limit event carries more diagnostic weight. Worth a line in the instrumentation chore or the merge-poller drain chore rather than a task here.
- **Whether Buildkite secret-level pipeline scoping is available on the org's plan tier is unverified.** All 25 existing secrets in the shared cluster have `policy: ""` — no pipeline or queue restriction — and nothing in either repo exercises per-pipeline secret scoping today. "The deploy step runs only on `main`" bounds _when_ a secret is read, not _which pipelines can read it_; a relay secret added to the cluster is, by default, readable by all seven of flunge's pipelines as well as the relay's own. If scoping is available on the plan, it should be applied to the relay's secrets before they're created — retrofitting scoping onto a live secret is strictly easier to defer than to discover after a leak. **Operator decision — needs a Buildkite plan/support check, not a design decision.** Recommendation: verify before task 8 lands; if unavailable, land unscoped with this risk explicitly recorded rather than blocking the whole project on it, since the relay itself holds no GitHub credential (see [Blast radius](#blast-radius)) and a compromised relay secret's blast radius stays bounded even if the _cluster-membership_ exposure is broader than ideal.
- **The deploy pipeline's trigger model is an open operator decision, not picked here.** Two in-org precedents pull in different directions: flunge's staging release triggers on every push to `main` (`.buildkite/release-staging.pipeline.yml`, single-product repo, no path-filtering concern); `mono-checkleft-release` runs push/PR-triggers **off**, on a cron schedule plus manual trigger, specifically because mono is a monorepo and an unfiltered push trigger would fire on every merge regardless of whether anything under the relay changed. A naive push-to-`main` deploy trigger for the relay inherits that exact problem — it needs either a path gate scoped to the relay's crates plus a durable release tag (so re-deploys are intentional, not incidental to unrelated PR merges), or a schedule-and-manual trigger matching `mono-checkleft-release`'s shape. **Do not pick silently when implementing task 8** — surface both options with their precedents, as this section does, and let the operator choose.
- **Whether the relay's deploy pipeline may share the `linux-release` queue with flunge's release train is itself an open question, not a settled yes.** The queue is proposed in [Deploy pipeline](#deploy-pipeline) because it already exists, is unused by mono today, and is proven green for the relevant tooling — but it is _flunge's_ queue, with a single agent, and flunge's release pipelines depend on it being available when their own deploys run. Consuming it for mono's relay deploys too means the two release trains now contend for one agent, which is a cross-team operational commitment, not just a mono-side implementation detail. **Operator decision** — confirm with whoever owns flunge's release infrastructure before task 8 provisions against it; the alternative is a new dedicated queue, at the cost of provisioning a new agent.
...
**Cross-project gate:** every entry from _Engine: publish `PrReconcileRequested` from relay deliveries_ onward is blocked on **the GitHub API instrumentation chore** landing, because the cutover's effect is unmeasurable without it. Entries before that point may proceed in parallel with it.
...
**Depends on:** Fix the `PrReconcileRequested` quiesce. **Edits the same files as the merge-poller drain chore's batching work (`schedule.rs`, `sweep.rs`)** — land after entry 3, and forward-port that chore's changes preservingly if it lands first.
...
### 8. Relay: own Buildkite pipeline, deploy shape, and secret management

**Scope:** A dedicated pipeline, not a step in `.buildkite/pipeline.yml` — see [Deploy pipeline](#deploy-pipeline) for the full design this task implements. Concretely:

- `.buildkite/pipeline-relay-release.yml`: read-only preflight step, then a `depends_on` mutation/deploy step with `concurrency: 1` and `concurrency_group: "mono-relay-release-mutation"` (relay-specific, not shared with flunge's `flunge-release-mutation`), secrets declared only on the mutating step, both steps on the `linux-release` queue.
- `wrangler.toml` with the Durable Object binding, and a bazel `sh_binary` deploy target wrapping `worker-build`/`wrangler`. **Time-box the `workers-rs`-under-bazel integration inside this task and escalate rather than absorb** if it exceeds budget — the fallback fork (TypeScript Worker vs. Azure Container Instances) is an operator decision recorded in this design's open questions.
- The deploy artifact stays bazel-built: if the Azure fallback is ever taken instead of Cloudflare, mirror flunge's `oci_image` + `oci_push` (`backend/BUILD:32-84` in `spinyfin/flunge`) pushed via `bazel run` with BuildBuddy registry headers (`mutate.rs:61-80`) — no Dockerfile.
- **Pipeline registration in-repo, not just a manual checklist result:** `infrastructure/buildkite/pipelines/mono-relay-release.stub.yml` (the bootstrap stub Buildkite's stored web config points at), following flunge's pattern at `infrastructure/scripts/sync-buildkite-pipeline.sh`. The drift-proofing comes from the stub being the _only_ stored config, not from a checker. Ship the `sync-buildkite-pipeline.sh`-equivalent `--check` mode as an **operator-run** bootstrap/verification tool and **do not wire it into the relay pipeline** — REST `GET /pipelines/{slug}` accepts only a Buildkite user API access token, so a CI-side drift check would provision exactly the long-lived org-scoped credential this arrangement exists to remove (see [Deploy pipeline](#deploy-pipeline)). Carry over both gotchas: the stub's `agents: queue:` must be a literal (Buildkite's steps editor 422s on `${VAR:-default}` interpolation), and an untargeted stub step fails "no queue has been targeted".
- The webhook secret and the Cloudflare API token (scoped `Workers Scripts: Edit`, likely a new token — see the `workers-rs` risk entry) become Buildkite cluster secrets. **This task does not resolve the secrets-boundary open question** (the Buildkite secret-scoping item in [Risks / open questions](#risks--open-questions) — cluster secrets are currently unscoped, so this widens what every flunge pipeline can read, not just this one). If secret-level pipeline scoping is confirmed available on the plan tier before this task starts, apply it here; if not, land with the secret unscoped and the risk recorded, per the operator decision needed on that item.
- Trigger model (push-to-`main`, path-gated push, or cron+manual) is **not** decided by this task — implement whichever the operator picks per the trigger-model item in [Risks / open questions](#risks--open-questions), including a path gate or durable release tag if push-triggered is chosen, since this is a monorepo pipeline.

**Effort:** `medium`

**Depends on:** Relay: engine enrollment, credential issue, rotate, revoke. Also depends on the operator resolving the trigger-model and secret-scoping open questions before the corresponding sub-scope lands — the rest of the task (pipeline shape, stub-sync, artifact) is not blocked on that resolution.
...
**Depends on:** Engine: publish `PrReconcileRequested` from relay deliveries; Relay: own Buildkite pipeline, deploy shape, and secret management.
...
**Scope:** Register a new GitHub App (explicitly **not** `boss shake`'s, whose credentials are compile-time-embedded in a distributed binary at `github/src/lib.rs:56-58`), move the webhook subscription to it, and evaluate unifying its installation token with the GitHub-App installation-token chore's opt-in polling-auth work so one App owns both delivery and polling. Gives org-wide subscription instead of per-repo configuration.

**Effort:** `medium`

**Depends on:** Relay: own Buildkite pipeline, deploy shape, and secret management; **the GitHub-App installation-token chore (cross-project, not started)**.

**Scope: deferred (future / not a v1 blocker)** — v1 deliberately uses a repo-level webhook for one-click reversibility, and the GitHub-App installation-token chore has not started and is opt-in, so v1 must not depend on it.
...
**Depends on:** Relay: own Buildkite pipeline, deploy shape, and secret management; `relay_client` crate.
...
**Depends on:** Relay: own Buildkite pipeline, deploy shape, and secret management.
...
- **Depth 1 (parallel):** 5 and 9, after 2. Also 4, after 3 — but 4 shares `schedule.rs` / `sweep.rs` with the merge-poller drain chore's batching work; serialize at the file level and forward-port preservingly.
- **Depth 2:** 6 after 5; 10 after 9 and 2. Parallel with each other.
- **Depth 3:** 7 after 6.
- **Depth 4:** 8 after 7. Parallel with 11 once the instrumentation chore has landed.
- **Depth 5:** 11, gated on 10, 3, 4, and the instrumentation chore. Single-threaded — everything downstream depends on it.
