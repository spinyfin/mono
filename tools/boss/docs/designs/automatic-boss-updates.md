...
- **Changing the release pipeline's tag/version scheme.** We consume `boss-v1.0.N` + `Boss-1.0.N.zip` as they exist. (We *do* require one pipeline change — notarizing the zip — but the naming/versioning is unchanged.)
...
> **Quarantine note:** programmatic `URLSession` downloads do **not** set the `com.apple.quarantine` xattr (that's set by LaunchServices-aware apps like browsers). We additionally `xattr -d com.apple.quarantine` defensively on the extracted bundle before swap so a relaunch is never gated by a Gatekeeper first-run prompt.
...
5. **Notarization / Gatekeeper** — `spctl --assess --type execute` must pass. **This requires the released zip to be notarized** (see Risks — current pipeline does not notarize the zip; this is a prerequisite).

There is **no published checksum or detached signature asset** today (only the `.zip`). Integrity therefore rests on HTTPS + Apple code-signing/notarization, which is the canonical macOS trust chain anyway. *If* we later want defense-in-depth, the cheapest addition is a `Boss-1.0.N.zip.sha256` asset emitted by `boss-release.sh` and checked at step 3; this is noted as an optional pipeline enhancement, not a v1 requirement.
...
**Gatekeeper / quarantine / signing / notarization.**
- The swapped-in bundle must be Developer-ID signed **and notarized + stapled**, or macOS will block launch (especially after any quarantine). We verify this at *stage* time (§3) so we never swap in something Gatekeeper will reject.
- We strip `com.apple.quarantine` on the staged bundle defensively.
- Because we replace in place at a path the user already trusts and we do not set quarantine, no first-run "downloaded from the internet" prompt should appear; the notarization check is the belt-and-braces guarantee.
...
| **Integrity check fails** (bad unzip / `codesign`/`spctl` reject / Team-ID mismatch) | §3 steps 3–5 | Mark `failed`, delete the staged dir, do **not** swap, surface "update could not be verified". This is the guard against a compromised/non-notarized artifact. |
...
1. **🔴 The published `Boss-1.0.N.zip` is not notarized — this blocks Automatic install.** `boss-release.sh` zips the bazel build output without running `notarytool`/`stapler`; only the unused `installer/release.sh` `.pkg` path notarizes. An auto-swapped bundle that isn't notarized will be blocked by Gatekeeper. **This is a prerequisite, not part of the updater itself:** the release pipeline must be extended to Developer-ID-sign + notarize + staple the `.app` before zipping (essentially folding `release.sh`'s steps 1–6 into `boss-release.sh`, or notarizing the zipped app). *Reviewer: confirm we'll fund this pipeline change; the updater's verify step (§3.5) will otherwise reject every release.* Until then, "Manual/Notify + manual download" still works (the user double-clicks and Gatekeeper prompts once), but unattended auto-install does not.
...
4. **🟡 Default mode.** Proposed default is **Notify** (no silent mutation of installed apps). Once notarization (risk 1) lands, is **Automatic** the better default for an internal tool where everyone wants latest? *Reviewer decision.*
...
These are the follow-on tasks to file once this design is approved. Roughly ordered; T1 is a hard prerequisite for the auto-install tasks (T6–T7).

- **T0 (prereq, pipeline):** Extend `boss-release.sh` to Developer-ID-sign + notarize + staple the `Boss.app` before zipping (fold in `release.sh` steps 1–6), so `Boss-1.0.N.zip` passes `spctl --assess`. *Blocks T6/T7.*
...
- **T6:** `UpdateDownloader` — background `URLSession` download to `Updates/staging/`, size + `ditto` + `codesign --verify` + Team-ID + `spctl --assess` verification, atomic rename to `Updates/<version>/`, quarantine strip, manifest state machine, and the "delete ≤ current staged version" cleanup rule. *Depends on T0 for the `spctl` gate to pass on real releases.*
- **T7:** `UpdateInstaller` — `relaunch-helper.sh` resource, swap-on-quit hook, swap-on-startup fallback (before engine launch), `.bak` rollback + "first launch OK" flag + failed-version blocklist, `/Applications`-not-writable graceful degradation. *Depends on T6.*
