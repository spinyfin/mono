...
This grant is scoped to `--sandbox workspace-write` for Standard/Triage/AnswerAgent. Codex reviewers intentionally receive no OS sandbox, matching Claude and local Grok reviewers. The former output-rooted reviewer seatbelt permitted `boss propose review-report` but excluded the events socket and `boss-event` shim, so no driver hook events could arrive; without a driver-start signal, the startup fuse would reap a legitimate review. The reviewer prompt and shared `reviewer_static_analysis_guard` remain in force.
...
| **Reviewer read-only**                 | per-kind deny rules (`reviewer_deny_rules`)                               | reviewer instructions plus the shared `reviewer_static_analysis_guard` PreToolUse hook; no Codex OS sandbox                                                                                                                       | **Advisory, matching Claude and local Grok reviewers.** No OS sandbox is used because hook events must reach the engine.                                                   |
