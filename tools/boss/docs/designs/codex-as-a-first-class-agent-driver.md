...
The [Non-goals](#non-goals) section names this alongside Codex Cloud, `app-server`, and `remote-control` as out of scope, but only `app-server` got an individual writeup ([Alternative 3](#alternative-3-drive-codex-app-server-over-json-rpc)). This doc's earlier text also invokes the interactive TUI only as a comparison point for Esc/abort semantics ([G-10](#g-10-controlverbs), citing pane-viability Q5). It was never itself argued as a competing execution mode. Assessed here, re-verified against `codex --help` on `codex-cli 0.145.0` (`~/.local/bin/codex`, 2026-07-29):

**Rejected — but not for the reason usually reached for.** The obvious guess is that the TUI forces human approval loops. **That is not quite right, and the CLI disproves it directly:** the bare TUI accepts the identical `-a, --ask-for-approval <untrusted|on-request|never>` that `codex exec` does, so `-a never` removes interactive approval from the TUI exactly as it does from `exec`. Approval is not the disqualifier.

The real disqualifiers are structural, and independent of the approval flag:

- **It is a persistent, multi-turn session, not a one-shot process.** `codex exec` runs one turn and exits (`turn.completed`, then the process terminates — see [G-7](#g-7-turnboundary)). The bare TUI's own help text says "options will be forwarded to the interactive CLI" and takes only an _initial_ prompt (`[PROMPT] Optional user prompt to start the session`); every subsequent turn is a human typing into a live session that stays open. Boss's worker model spawns one process per turn/dispatch and observes it to completion via an exit — there is no CLI switch that makes the TUI behave that way.
- **It renders a full alt-screen UI by default, with no machine-readable output mode.** `--no-alt-screen` ("Disable alternate screen mode... preserving terminal scrollback history") only exists because the default _is_ alt-screen painting. Critically, the bare `codex`/TUI help has **no `--json` flag at all** — unlike `exec`, `remote-control`, and `codex cloud list`, which all expose one. There is no supported way to get a parseable event stream out of the interactive mode; the whole surface is designed to be watched by a person, not tailed by an engine.
- **Interrupt is a live keystroke, not a signal.** The pane-viability spike already established (line 541 of this doc) that Esc-driven abort (`rollout turn_aborted`, `reason: "interrupted"`) is TUI-only and was never observed on `exec`. That is a keyboard event delivered into the pty, not something Boss can send programmatically without faking a terminal client — the same class of problem [G-10](#g-10-controlverbs) already flags for mid-turn injection.

So the accurate framing is: the interactive TUI is not disqualified by approval semantics (those are configurable, same as `exec`), it is disqualified by being a persistent, screen-painting, keystroke-driven session with no batch exit and no structured-output mode — none of which fits a process Boss spawns once per turn and tails for machine-readable progress.
...
The pane-viability spike (PR #2392) then established that the engine cannot read pane-hosted stdout at all under Boss's actual topology — 0 bytes, no `/proc/<pid>/fd`, opening the slave tty is not the master stream (see [G-5](#g-5-progressobservation--the-top-gap)). [G-5](#g-5-progressobservation--the-top-gap) and [Chosen approach](#chosen-approach) responded by moving production progress ingress to `ProgressIngress::AgentJsonlFile` — tailing `$CODEX_HOME/sessions/**/rollout-*.jsonl`, which `codex exec` writes unconditionally whether or not `--json` is passed. That is exactly what `CodexDriver::progress_observation_wiring` does today: it unconditionally constructs `ProgressIngress::AgentJsonlFile` (`engine/driver/src/codex.rs:1310-1325`), never `StdoutJsonl`. The stdout-dialect parser that `--json` exists to feed (`ProgressStreamSource::StdoutJsonl`, `CodexProgressSession::new`, `engine/driver/src/codex.rs:1346`) remains reachable code but is not wired into any path the driver actually selects.

**What this means, stated plainly: "Boss must use `codex exec`" does not imply "the pane must display raw JSON."** Those two were conflated in practice — the spawn line (`engine/driver/src/codex.rs:735`, `engine/core/src/conformance/fixtures.rs:174`, as of this writing) still carries `--json` even though nothing production reads it as progress input — even though no sentence in this doc ever asserted the implication. Once ingress is rollout-based, `--json`'s only live effect is on the pane a human or reviewer watches: it replaces `codex exec`'s default human-readable ANSI transcript with raw JSONL on stdout, which is strictly worse for anyone looking at the pane and buys nothing for progress fidelity.

**Rejected (keeping `--json`).** There is no remaining reason to pass it. **This is being handled as a code change, tracked separately, not settled by this doc revision:** `mono#2532`, "Drop vestigial `--json` from Codex pane spawn line," removes `--json` from the spawn line, adds `--color always` to keep the transcript colorized under a pane pty (where Codex's own tty auto-detection is unreliable), and forbids `--json` outright in the spawn-line contract test (`CODEX_EXEC_FORBIDDEN_LONG_FLAGS`, `engine/core/src/conformance/fixtures.rs`). As of this writing that PR is open against `main` and has not merged. This doc's conclusion holds independent of that PR's merge state: `--json` was never required for progress tracking once ingress moved to `AgentJsonlFile`, whether or not the flag has already been removed from the spawn line by the time this section is read.
...
**The fix is `AgentDriver::worker_process_lifetime()`** (`Persistent` by default, `OneTurnPerProcess` for `codex`) — the same driver-conditioned shape as `mid_turn_pane_input()` below, and for the same reason: only the driver knows what its foreground process is contracted to do. The exemption is _evidence-gated_, not driver-gated: a one-shot exit is expected only when the run also has a delivered turn boundary recorded (`work_runs.turn_boundary_at`), and an exit without one is still reaped. Ordering is resolved **causally rather than with a grace period**: the process exiting is what makes its rollout file final, so on an exit the engine drains that file to EOF (`AgentJsonlProgressManager::finish_run`) — a read bounded by the file's own size — which delivers `turn.completed` through the normal fan-out and runs completion detection _before_ the reap verdict is taken. See `engine/core/src/worker_process_exit.rs`.
