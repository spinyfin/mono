...
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Reviewer read-only                                   | `--sandbox workspace-write`, output-rooted (`--cd` relocated to engine scratch, `writable_roots` narrowed with `exclude_tmpdir_env_var`/`exclude_slash_tmp`) | **OS-enforced against the checkout**, which stays outside every writable root even though the sandbox mode itself is `workspace-write`, not `read-only` |
...
This grant is scoped to `--sandbox workspace-write` for Standard/Triage/AnswerAgent. Reviewer also runs `--sandbox workspace-write` now (its original `read-only` posture proved unworkable for report delivery — see the "Update (Claude-parity change)" note further down), with its own `[sandbox_workspace_write]` table (`render_reviewer_sandbox_workspace_write_toml` in `engine/driver/src/codex.rs`): `network_access = true` (the reviewer's only report-delivery channel, `boss propose review-report`, is a Unix-domain-socket connect, which Codex's seatbelt classifies as network-outbound) and `exclude_tmpdir_env_var` / `exclude_slash_tmp` (so the workspace-write default's blanket `$TMPDIR`/`/tmp` grant doesn't hand the reviewer write access to concurrent executions' artifacts or the engine control socket) narrow the filesystem grant to the `--cd`-relocated output root instead of the checkout.
...
| **Reviewer read-only**                 | per-kind deny rules (`reviewer_deny_rules`)                               | `--sandbox workspace-write`, output-rooted via `--cd` and a narrowed `[sandbox_workspace_write]` table, always, independent of `codex_sandbox_enforced`                                                                           | **Preserved against the checkout, by construction rather than sandbox mode.** The checkout stays outside every writable root; the sandbox mode itself is `workspace-write`, not `read-only` (see [Bazel under the Codex sandbox](#bazel-under-the-codex-sandbox)). |
