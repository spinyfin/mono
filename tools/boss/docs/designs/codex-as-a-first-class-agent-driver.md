...
The real blocker is one layer down: **Boss's only progress ingress is a unix socket fed by the `boss-event` shim, which only exists because Claude can be made to run a command on every hook.** Codex's signal arrives on the worker's _stdout_. PR #2363 landed an engine-side stdout JSONL reader, but that reader only helps when the **engine owns the pipe/pty master**. Under the current pane-hosted Boss shape the **app** owns the pty and the engine receives only `shell_pid` — and an outsider with only `shell_pid` **cannot** read that stdout on macOS (pane-viability spike Q1). So the gap is not "Codex lacks turn boundaries" — it is **"`ProgressObservation` abstracts event _normalisation_ but not event _transport_, and the transport for pane-hosted workers into the engine is still an open seam."** That is the single amendment that most changes P1422's remaining work.
...
- **Driver-aware kanban / Swift UI.** The kanban already reads abstract `WorkerActivity`; nothing in the product surface needs to know which driver ran. That is **not** a claim that the app process is uninvolved in progress transport: under the current pane topology the app owns GhosttyKit, and pane-to-engine progress handoff remains an [undecided seam](#the-five-engine-seams-this-needs) (the pane-viability spike showed the embedder can observe surface text; wiring that — or any other channel — into the engine is design work, not decided here).
...
So the earlier design sentence _"the transport split and reader are no longer future work"_ is **true for engine-owned streams** and **false for pane-hosted progress into the engine**. Candidates for engine ingress under the current app/engine split, none chosen here:

1. **`AgentJsonlFile` / rollout tail** — file exists, grows live, discoverable by `thread_id` glob or sessions-dir watch (spike Q7). Different event dialect from stdout JSONL; needs its own normaliser mapping.
...
- **T1476 is insufficient in one respect:** PR URL capture is not in its scope and is `PostToolUse`-derived. Under Codex there is no `PostToolUse`, so **PR URL capture breaks entirely** — and the PR URL is the acceptance criterion for essentially every Boss work item. Codex's stdout `command_execution` items _do_ carry `aggregated_output` (verified in the spike capture above), so the same regex can run against the **stdout** dialect when that stream reaches the engine. Against a **rollout** tail the equivalent content is `custom_tool_call_output`, not `aggregated_output` — extractor work is dialect-dependent, not transport-agnostic. That is new work on whichever progress channel [G-5](#g-5-progressobservation--the-top-gap) lands, not in T1476.
...
**Remaining work:** glob-based Codex rollout path discovery from `thread_id` + `CODEX_HOME`; generalise `engine/transcript-tail` at container level only; keep a **separate** Codex rollout normaliser from the stdout normaliser; retain hook-provided transcript paths only as a cross-check.
...
**Rejected — and the 0.145.0 delta strengthens the rejection rather than weakening it.** This was originally rejected partly because no hook could be made to fire. Hooks now demonstrably do fire ([D-2](#deltas-that-change-the-design)), so that objection is gone — but the decisive one remains and is now better evidenced: an untrusted or misconfigured hook is skipped in **complete silence**. Routing progress ingress through that mechanism would mean a worker that silently reports nothing and never completes, with no signal to distinguish it from a hung agent. It is the wrong dependency for the same underlying reason it was before: it makes Boss's progress ingress contingent on the most fragile, least-observable part of Codex's surface, when Codex is _already_ emitting a typed, in-band, structural event stream on stdout (and a richer rollout file) that requires no installation and no trust record — **provided the observer can attach to that channel**. Pane-hosted topology still needs [seam 5](#the-five-engine-seams-this-needs) for engine attachment; that is not an argument for routing through silent-fail hooks. It would also entrench the assumption that progress arrives via callback, which is exactly the abstraction gap ([G-5](#g-5-progressobservation--the-top-gap)) this project exists to surface.
...
- **Pane-hosted** worker (app owns GhosttyKit/pty; engine receives `shell_pid` only): the engine **cannot** attach to that stdout. Naming "stdout JSONL" as the engine progress transport for this shape over-claims. **Pane-to-engine progress handoff is an undecided seam** ([seam 5](#the-five-engine-seams-this-needs)) — candidates are rollout tail, app-forwarded surface scrape / master tee, or a topology change so the engine owns the stream. This doc records the options; it does **not** pick one.

What remains decided for v1: non-interactive `codex exec --json` is the **agent CLI shape**; progress **normalisation** targets the stdout dialect when that dialect is available, and must not pretend rollout is the same parser; hooks carry guardrails; structured output uses the file contracts above.
...
How that argv is **hosted** (pane shell vs engine-owned spawn) is coupled to [seam 5](#the-five-engine-seams-this-needs). The CLI line above is the process to run; it is not by itself a complete progress-ingress design for pane-hosted workers.

### The five engine seams this needs

1. **A progress reader matched to topology.** For **engine-owned** stdout: the landed #2363 JSONL reader + `ProgressIngress::StdoutJsonl` — feeds the same normaliser and activity machine as the socket path; PR-URL capture can scan `command_execution.aggregated_output`. For **pane-hosted** workers into the engine: **not** shell_pid→stdout; needs seam 5's chosen channel (file dialect or app-forwarded bytes) before the normaliser has anything to read.
...
5. **Pane-to-engine progress transport (undecided).** The missing handoff when the app owns the pty and the engine only has `shell_pid`. Options (evidence only; **no product pick in this doc**): rollout/`AgentJsonlFile` discovery and tail; app scrapes `ghostty_surface_read_text` (or a master tee) and forwards over an IPC path not yet specified; or engine-owned spawn so #2363 attaches directly. **PR-URL capture and turn-boundary delivery both depend on which dialect and which process first sees the events.** Until this seam is chosen, T-11 can spawn a Codex process and T-12 can decode a stream **in isolation**, but end-to-end pane-hosted observation into the engine is not specified.
...
| ---- | ------------------------------------------------------------------------------------ | --------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A-1  | `ProgressObservation`: abstract the ingress transport, not just normalisation        | `large`   | **Amends T3328** (materially widens it; residual for pane topology)               | Partially landed: `ProgressIngress` separates hook callbacks from stdout JSONL, the hookless empty-map case is supported, the events socket receives an injected driver, and PR #2363 added the engine-side stdout reader — **valid when the engine owns the stream**. Pane-viability spike: with app-owned pty and engine `shell_pid` only, that reader has nothing to attach to. Residual: choose and implement [seam 5](#the-five-engine-seams-this-needs) (rollout/`AgentJsonlFile`, app-forwarded surface/master channel, or engine-owned spawn) before pane-hosted Codex progress is end-to-end.                             |
...
| A-6  | `TranscriptAccess`: driver-supplied path discovery, and actually call the normaliser | `small`   | **New**                                                                           | The trait now has `transcript_path_for_session()`, and `live_status_loop` calls `normalize_transcript_entry`. Remaining work: **glob** Codex rollout path (`rollout-*-{thread_id}.jsonl` — local timestamp in filename blocks pure construction), generalise `engine/transcript-tail` at **container** level only, and keep a separate rollout-line normaliser (schema ≠ stdout JSONL).                                                                                                                                                                                                                                            |
...
Map Codex's stream envelopes onto `WorkerEvent`: `thread.started`, `turn.started`, `turn.completed`, `item.started` / `item.completed` across the `TurnItem` variants. It builds on the landed stdout reader (**when a stream is available**) and in-flight PR #2361 turn boundary. This makes the driver capable of decoding the **stdout** dialect. If seam 5 chooses rollout tail instead, a **separate** rollout→`WorkerEvent` mapping is required (do not force one parser onto both dialects).
...
Make PR-URL capture driver-supplied rather than `PostToolUse`-derived (`pr_url_capture.rs`). Codex path depends on progress dialect: scan `command_execution.aggregated_output` on the **stdout** stream when that is the ingress; scan `custom_tool_call_output` (or equivalent) if seam 5 lands on **rollout** tail. Without this a Codex work item can never satisfy its acceptance criterion. Separate from T-12 because it changes an engine-side capture contract, not the normaliser.
