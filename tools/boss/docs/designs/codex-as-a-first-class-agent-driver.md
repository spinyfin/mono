...
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Reviewer read-only                                   | reviewer instructions plus the shared `reviewer_static_analysis_guard` PreToolUse hook            | **Advisory, matching Claude and local Grok reviewers.** Codex reviewers intentionally have no OS sandbox so `boss-event` hook traffic can reach the engine. |
...
This grant is scoped to `--sandbox workspace-write` for Standard/Triage/AnswerAgent. Codex reviewers intentionally receive no OS sandbox at all. The former output-rooted reviewer seatbelt relocated the sandbox root to engine scratch via `--cd` so a single report-body write was permitted while the checkout stayed OS read-only — but that same `--cd` also relocated the hooks `cwd` Boss arms and trust-attests against away from the checkout, desyncing it from the pane's real working directory and making every Codex reviewer appear never-started, before the driver's own progress/rollout signal ever came into it. Removing the sandbox removes the `--cd` that caused the mismatch. The reviewer prompt and shared `reviewer_static_analysis_guard` remain in force, and mutation/publication is now fenced by a Codex-specific `PreToolUse` guard (`reviewer_publish_guard`) standing in for Claude/Grok's declarative `reviewer_deny_rules`, which Codex has no equivalent surface for.
...
| **Reviewer read-only**                 | per-kind deny rules (`reviewer_deny_rules`)                               | reviewer instructions plus the shared `reviewer_static_analysis_guard` and Codex-only `reviewer_publish_guard` PreToolUse hooks; no Codex OS sandbox                                                                              | **Advisory, Codex-specific mechanism.** No OS sandbox is used because the sandbox required relocating the session's cwd, which broke driver-start detection; `reviewer_publish_guard` replaces the declarative deny rules Codex has no equivalent surface for. |
...
Codex reviewers have no OS sandbox at all — not because they match Claude/Grok's no-sandbox posture on the merits, but because the former output-rooted seatbelt's `--cd` desynced the attested hooks `cwd` from the pane's real cwd and broke driver-start detection. `reviewer_publish_guard` is the Codex-specific replacement for Claude/Grok's declarative `reviewer_deny_rules`. The data-dir row is flag-dependent for Standard/Triage/AnswerAgent: kernel-enforced only when `codex_sandbox_enforced` is on, advisory-hook-only by default — matching, not exceeding, Claude's posture in that default case. The middle two are now a straight reuse of the existing guard scripts behind Codex's hook config rather than a rewrite into shims. Only the editorial row changes character.
...
**Update (Claude-parity change):** Standard/Triage/AnswerAgent sandbox defaults to `danger-full-access`, not `workspace-write` as originally designed here. Codex's seatbelt template hardcodes a mach-service allowlist that excludes LaunchServices, so `xcode-locator` fails with `kLSExecutableIncorrectFormat` under `workspace-write` — this broke every bazel build in mono for Codex workers, a strictly worse posture than the Claude driver has ever run at (Claude workers get `--permission-mode auto` with no OS sandbox). Those worker kinds are gated behind the `codex_sandbox_enforced` feature flag (default off) so the OS-enforced `workspace-write` fence can be turned back on without a rebuild once the LaunchServices gap is fixed upstream. Reviewers intentionally have no Codex OS sandbox: the previous output-rooted seatbelt's `--cd` into engine scratch (needed to permit the one report-body write) also relocated the hooks `cwd` Boss trust-attests against, desyncing it from the pane's real cwd and producing a false driver-start timeout. `reviewer_publish_guard` fences mutation/publication in its place. The Boss-data-dir guard is advisory by default; it falls back to the `PATH_GUARD_SCRIPT` PreToolUse hook, the same fence the Claude driver relies on. `model_reasoning_effort` is resolved against the selected model's `supported_reasoning_levels` rather than assumed, since the ladder is per-model and now reaches `ultra` on some models. The pivot spike verified `--sandbox`, `-m`, `-c model_reasoning_effort=` are all still accepted by the bare TUI, unchanged from `exec`.
...
**Phase 3 — review and conflict resolution.** Review needs `--sandbox read-only` to be verified as a real reviewer-read-only equivalent, and structured `ReviewResult` output. Conflict resolution needs write access plus the merge-conflict telemetry path. Acceptance (original): a Codex reviewer produces a structured `ReviewResult` on a real PR that a human agrees with, and demonstrably cannot write to the workspace. **Status: `ReviewResult` round-trip verified live (through a transcript fallback that was silently dead until it was fixed); the OS-enforced read-only posture was withdrawn.** The reviewer OS sandbox this acceptance criterion depended on was removed: its `--cd` into engine scratch, needed to permit the one report-body write, also desynced the hooks `cwd` Boss trust-attests from the pane's real cwd, making every Codex reviewer appear never-started. Reviewer read-only is now enforced advisory-only, by the Codex-specific `reviewer_publish_guard` `PreToolUse` hook rather than the OS. See [T-25](#t-25-codex-eligibility-for-review-and-conflict-resolution-kinds) — `REVIEWER_POOL_DRIVER` also still hardcodes `"claude"` for every review-pool dispatch, so there is currently no seam to select Codex as a reviewer at all, independent of the sandbox question.
