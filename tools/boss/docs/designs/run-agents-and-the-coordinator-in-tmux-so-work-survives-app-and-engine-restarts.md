...
**Rollback.** Flipping a pool back to app-hosted must first **drain** it: existing tmux sessions for that pool are torn down through the normal teardown path, not orphaned. A rollback that leaves sessions running produces the leak class this design otherwise handles, at the worst possible time.
