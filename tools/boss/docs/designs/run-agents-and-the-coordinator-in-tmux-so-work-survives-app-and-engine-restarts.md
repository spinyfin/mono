...
**Pane color environment (assumption, recorded).** Every Boss session — worker _and_ coordinator, not per-driver — also carries `-e CLAUDE_CODE_TMUX_TRUECOLOR=1 -e COLORTERM=truecolor` on that same atomic `new-session`. Claude Code clamps its chalk level to 2 whenever `$TMUX` is set, ignoring `COLORTERM` and `FORCE_COLOR`; `CLAUDE_CODE_TMUX_TRUECOLOR` is the escape hatch and is read at module load, so it must be a real environment variable at process launch (`settings.json` `env` is injected too late). `COLORTERM=truecolor` is set explicitly rather than inherited: tmux 3.6a injects it itself, but the installer floor is tmux ≥ 3.2 (`tools/boss/installer/scripts/postinstall`), and a pane on a version that does not inject it falls through to `TERM=tmux-256color` and a genuine level-2 downgrade independent of the clamp. `FORCE_COLOR` is deliberately not set — the clamp overrides it, and some values disable a separate TERM-allowlist truecolor path. These land on `new-session -e` rather than a later `set-environment -g` or a shared tmux config, because a post-create write would miss the already-launched agent. A sibling doc ([`docs/claude-tmux-pane-controller.md`](../../../../docs/claude-tmux-pane-controller.md), "Sanitize Environment") already called for preserving `TERM` / `COLORTERM` / `TERM_PROGRAM` through tmux; this is the durability-surface counterpart of that policy. Do not add `terminal-features` / `terminal-overrides` keyed on `tmux-256color`: those patterns match the _outer_ client's terminal name (here typically `xterm-256color`), so such an entry matches nothing, and tmux already grants attached clients RGB.
