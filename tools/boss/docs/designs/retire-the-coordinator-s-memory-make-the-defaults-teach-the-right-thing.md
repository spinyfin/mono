...
**Superseded / fixed-bug notes — deleted (7):** `feedback_bossctl_probe_doesnt_reach_live_workers` (fixed #461/#463), `feedback_notify_live_worker_on_chore_update` (engine propagates; T899/T623), `feedback_cost_aware_dispatch` (effort/model system shipped), `project_fable_floor_for_design_family` (superseded T2986), `feedback_chore_update_needs_full_id` (short-id selector shipped; the A4 sub-item was already dropped), `project_no_comment_subcommand` (prompt now covers the description-tag guidance), `reference_bossctl_reap_broken_for_coordinator` (T1110 fixed).
