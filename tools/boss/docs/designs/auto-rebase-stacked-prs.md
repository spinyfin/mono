...
2. **Lease fails.** Cube's `workspace lease` surfaces a real fault — repo not registered, remote unreachable, or a clone failure (e.g. disk full). "No free workspace" is not a failure mode: cube provisions a fresh workspace on demand whenever none is free. Mark the attempt `failed` with cube's error verbatim; retry on the next merge poller pass (bounded to 3 retries, then auto-promote to a worker chore) — same policy as sub-case 3 below.
...
**R4 — Worker slot / disk pressure.** Auto-rebase consumes worker slots and disk (cube workspaces themselves are not a bounded resource — a lease always succeeds by provisioning a new one); if 5 PRs merge in quick succession and 5 stacked dependents each kick off a rebase, we could starve the worker pool or fill disk with concurrent checkouts. Mitigation: cap concurrent engine-direct attempts at 2 (Q4). If the cap is hit, attempts queue as `pending` and the next merge poller pass picks them up.
