...
- **Per-event allocation pressure / unbounded growth.**
  ...
  _do_ grow with session length and account for the long-tail
  sluggishness.
...
references. Keep `bossTimeline` only if something _outside_
...
- **(c)** Drop `@Published` entirely and emit a manual
  `objectWillChange.send()` only when the _set_ of slot ids
...
notoriously order-sensitive (publisher fires _before_ the
...
together cut the _cost per event_ on the MainActor side by
