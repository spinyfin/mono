...
Boss already runs five kinds of enforcement against workers. The wrapper does not need new machinery; it needs to be wired into the machinery that exists.
...
| 2     | A `PreToolUse` interception guard that tokenises argv and denies a build-class command not already wrapped                                                                    | `PR_REDIRECT_GUARD_COMMAND`, `REVISION_PR_GUARD_COMMAND`, `BOSS_LAUNCH_GUARD_COMMAND`, `boss-checkleft-push-guard.py`                | prevents, precisely                  | Claude full; Codex deny-only, enumerable-list residual                   |
...
| 4     | Detect a build-class command that ran outside an obligation; file an attention item                                                                                           | `worker_sandbox_audit.rs`; the unobserved-command audit trail                                                                        | detects only                         | all drivers — engine-side, but only for routes that emit a tool boundary |
...
Enforcement claims are worth exactly as much as their weakest driver, and Boss has already paid for that lesson in evidence. [`codex-pretooluse-guard-coverage-2026-07-29.md`](../investigations/codex-pretooluse-guard-coverage-2026-07-29.md) set out to confirm the Codex guards fire and found two live bypasses that had been sitting behind a prompt sentence asserting the opposite — and closed both in the same change:

- **`tools.write_stdin` fired no hook at all.** A cell could start a long-lived shell with `exec_command` (which the guards approve, because `sh -s` is innocuous) and then feed it arbitrary command lines that no guard ever saw. Demonstrated end-to-end against the push guard, not theorised.
- **App/MCP tools arrived under an `mcp__…` tool name** that no `matcher = "Bash"` guard matched.

Both are closed, and the closure is in the tree, not proposed. The investigation shipped a Codex-only tool-surface guard, `codex_tool_surface_guard` (`codex.rs:1009-1019`), armed with `matcher = ".*"` — "the only guard that sees non-`Bash` tool names" — for every Codex worker kind. It denies every `mcp__*` call outright, and separately denies the invocations that open a stdin-driven command channel in the first place (a bare interpreter, `sh -s`, `-i`, `-`, and interactive editors/pagers with shell escapes) — the only hookable point on the `write_stdin` path, since there is no session left to write into once opening one is denied. Both closures are validated live against `gpt-5.6-terra` and `gpt-5.6-sol` in the investigation's §6 and pinned as regression fixtures in `guard_conformance.rs:308-371` (`boss-probe-4`, the `sh -s` + `write_stdin` bypass attempt, and `boss-probe-5`, the MCP app tool call — both assert `block`).

The residual is narrower than "two open routes," and the investigation says so directly under its own limitations: **the interactive-interpreter denylist is a list, not a structural guarantee** — it covers the shells, the common REPLs, and the editors/pagers with shell escapes, but "a determined agent could find an interactive stdin consumer that is not on it." The structural fix would be Codex disabling `write_stdin` per-config, which `--strict-config` shows no key for on 0.145.0 (`tools.write_stdin` is rejected as unknown) — so today the guard is enumerated coverage standing in for a config knob that does not exist yet.

Codex separately declares `ToolUseInterception` as **deny-only** (`codex.rs:1275`): it honours `permissionDecision: deny` but rejects `allow` / `ask` / `updatedInput`, so a guard there can block but cannot rewrite — no "silently upgrade the bare command into a wrapped one" path exists on that driver. And a driver that declares the capability not at all lands on `AbsenceDisposition::Degrade`, where `dispatch_post_hoc_interception_on_post_tool_use` can only flag an artefact after the fact, logging in terms that this "is not equivalent to pre-hoc interception". That characterisation is unaffected by the closure above and stands as written.

Two consequences follow, and the design takes both:

1. **Layer 2 is enumerable coverage, not a structural guarantee, and the design must not rest solely on it.** With the tool-surface guard shipped, layer 2 now closes the two routes a command matcher cannot reach by construction; what is left is a maintained list, which is a weaker foundation than a route that cannot exist at all. That is a narrower and more defensible dependency than "two structurally open bypasses" — but it is still a dependency, and it is why the safety argument keeps a layer, layer 3, whose guarantee does not rest on any driver-specific list staying complete.
...
That narrower residual changes the _strength_ of the case for layer 3, not its direction. Layer 3 is no longer needed to patch two known holes — those are patched. It remains the recommended eventual layer because its guarantee does not depend on an enumerable list staying complete against a model surface that has already been shown to change without notice (the guard-coverage investigation's own occasion was a prior generation's live capture going stale under the next model rollout). The manifest's guard-scoping question is restated on this corrected premise.

### What v1 commits to

Four of the five layers ship in v1; layer 3 remains deferred.
...
- **Enforcement is layered, and the strongest layer is deferred to a human decision.** [Enforcement](#enforcement-what-stops-a-worker-sidestepping-the-wrapper) sets out five layers; v1 ships four of them (the prompt contract, the deny rules, the interception guard, and detection of unwrapped build commands — layers 0, 1, 2, and 4). Layer 3 — refusing a validation claim that cites no observed exit code — is the only one that removes the _payoff_ from sidestepping rather than obstructing the act, and it is a policy decision about how Boss treats its workers. It stays deferred as entry 16 and is put to the operator in the questions manifest. If the answer is "advisory", the residual risk is precisely that a worker can sidestep the wrapper and make an unverifiable claim — entry 12's detection makes that visible after the fact but does not stop it.
- **Layer 2 on Codex now closes both structurally-open routes, but rests on an enumerable list rather than a structural guarantee, and that residual is load-bearing, not incidental.** The shipped `codex_tool_surface_guard` denies every `mcp__*` call and denies the invocations that would open a `write_stdin`-fed session, closing both bypasses the 2026-07-29 guard-coverage investigation measured open. What remains: the interactive-interpreter denylist is a maintained list, not a structural guarantee, so a determined agent could find an unlisted interactive stdin consumer — and Codex's `ToolUseInterception` is still deny-only, so a guard can block but never silently upgrade a bare command into a wrapped one. Any future argument that leans on the interception guard as _the_ enforcement answer still rests on that list staying current, which is a narrower dependency than before but not zero. The guard is real prevention for the common and accidental case; the safety argument still must not terminate at layer 2.
...
Breakdown size: 16 entries (13 in-scope, 3 deferred) — this spans four subsystems (engine/core, the `boss` CLI, the worker prompt, and the worker-setup guard wiring) plus a shared protocol crate, a schema change, and a discovery step. That is two entries above the 8–14 anchor band, and the two are the enforcement pair (11 and 12) added to answer the enforcement question; the five busyness consumers still collapse into two cutover entries instead of five.

**Parallelism summary.** Depth 0: entries 1 and 2 in parallel. Depth 1: entries 3 and 4 in parallel. Depth 3: entry 6 may run in parallel with entry 7. Depth 4: entry 8. Depth 5: entry 9 — functionally independent of entry 8 but serialised behind it, because both edit `app.rs`'s engine wiring; see entry 9's note. Entries 11 and 12 are independent of each other and of the consumer cutovers (8, 9); entry 11 needs only the wrapper (6), entry 12 needs only the obligation store and the verdict function (5, 7), so both can run alongside the cutover work.
...
Add deny rules for bare build-class argv (`Bash(bazel:*)`, `Bash(cargo:*)`, and the checkleft/test shapes) to `deny_rules()` in `worker_setup.rs`, leaving `boss run --` as the only spelling that passes, and add a `PreToolUse` interception guard that catches the shapes a leading-token match cannot — wrapper commands, shell-variable indirection, `sh -c`. Build it from the existing `python_command_guard!` seam, copying `BOSS_LAUNCH_GUARD_COMMAND`'s delimiter splitting, wrapper stripping, and single-level variable expansion, and copying `boss-path-guard.py`'s fail-closed posture: a payload the guard cannot parse is blocked, not approved. Wire it through `AgentDriver::tool_use_interception_wiring` so per-driver capability decides whether it is installed, and assert in tests that it denies rather than rewrites (Codex's `ToolUseInterception` is deny-only).

Two mechanics the 2026-07-29 guard-coverage investigation established as load-bearing carry over to this guard and are in scope here, not left implicit:

- **Matcher choice.** `codex_tool_surface_guard` is armed with `matcher = ".*"` rather than `Bash` because that is "the only guard that sees non-`Bash` tool names" (`codex.rs:1009-1013`; [`codex-pretooluse-guard-coverage-2026-07-29.md`](../investigations/codex-pretooluse-guard-coverage-2026-07-29.md) §5 item 3). Do not inherit a `Bash` matcher from the `BOSS_LAUNCH_GUARD_COMMAND` precedent by default — pick this guard's matcher on the same reasoning, so it sees the tool surface the design is actually worried about rather than only the `Bash`-named slice of it.
- **The trace shim.** On Codex, a guard that crashes, exits non-zero, or prints something that is not a decision is read as _approval_, not a block (same investigation, §5 item 4) — a fail-closed posture written inside the Python script does not survive the script dying. Wire this guard through the existing trace shim (`codex/guard_trace.rs`'s `boss_guard_trace.py`) exactly as the other Codex guards already are, so a crash converts to a recorded `block` rather than a silent pass.

The guard's deny message must name the wrapped form to run instead — a guard that blocks without naming the sanctioned alternative just costs a turn.
...
Run an isolated engine and drive a real Claude worker and a real Codex worker, each executing a genuine multi-minute `bazel` build through `boss run`, and demonstrate both directions: neither is reaped or retired across sweep passes, and a `SIGKILL`ed worker is still reclaimed on the next pass. Also exercise the enforcement path on both drivers: a bare build-class command is denied on Claude, and on Codex confirm what the guard actually does rather than assuming parity — the deny-only capability and entry 11's own matcher and trace-shim choices mean the Codex result is a finding to record, not a box to tick. Record raw artifacts. Deliberately end-to-end rather than a reconstructed harness, because the failures on this path have all been integration failures.
