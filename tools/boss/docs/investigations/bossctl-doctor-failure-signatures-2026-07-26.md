...
- The writer takes a bounded cross-process `flock` per append (`boss_engine_jsonl_append`), so the interleave shape does not recur. Acquisition is bounded — a wedged lock holder must not stall the dispatch path — and on timeout the append proceeds unlocked with a stderr warning.
