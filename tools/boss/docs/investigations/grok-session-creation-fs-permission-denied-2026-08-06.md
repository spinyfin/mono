...
Pass 2 now genuinely covers Grok whether the worker was engine-spawned or tmux-re-adopted. It reads `unverified_driver_starts`, which consults only `driver_signal_at` and `spawned_at` — blind to `shell_pid`, to `activity`, and to the driver's capability set. A tmux adoption based only on a live login shell receives its normal fresh 300-second window but remains eligible afterward; only a durable semantic-progress checkpoint or a newly received hook restores the driver-signal proof across an engine restart. `spawn_ack_sweep_induced_failure_tests` demonstrates the whole path end to end against a **real** OS process, including a live-shell re-adoption: the three older guards each decline (dead-pid finds the shell alive; `mark_stalled_spawns` declines because Grok omits `AwaitingInputSignal`; pass 1 declines because a re-adopted slot is not a new pane awaiting acknowledgement), then pass 2 fires, marks the execution `orphaned`, reaps the pane, releases the pool slot, force-releases the cube lease, and raises an attention item.

Before the readoption correction, that conclusion applied only to engine-spawned registrations: a live-shell tmux re-adoption was permanently exempt despite still lacking `driver_signal_at`. With the correction, this pane is on a 300s (`DRIVER_START_GRACE_SECS`) fuse after either registration path; the observation "`spawning` indefinitely" beyond that window is now evidence that the sweep or its inputs need investigation.
