...
The gap is still open today. Nothing in the trait requires a driver to declare its process lifetime relative to a turn, and nothing rejects a driver whose lifetime contradicts the mechanisms the engine will apply to it. Codex now declares one-turn-per-process behaviour in prose comments (`codex.rs:1557` and following) after #2474 had to teach the reaper about it. **A third driver could repeat this exactly.** Restating the abstraction's execution-model invariant in terms of session lifecycle rather than pane containment is the durable fix, and it belongs to the abstraction project, not to any driver.
...
3. **Restate the abstraction's execution-model invariant** in terms of session lifecycle rather than pane containment, and add a trait declaration for process lifetime relative to a turn (§6).
