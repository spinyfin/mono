...
- **Earlier measurement:** [All-pool tmux confidence gate — 2026-09-04](./tmux-all-pool-confidence-gate-2026-09-04.md)
- **Measurement:** 2026-09-10 15:50 CDT
- **Report corrections:** 2026-09-13; source and consistency review only. Production counts and drill observations retain their September 10 cutoff; this is not a new production measurement.
- **Verdict: gate NOT met.** Deletion tasks (`Collapse local engine spawn and teardown to tmux only` onward) remain blocked.

## Summary

The all-pool tmux soak started when [#2903](https://github.com/spinyfin/mono/pull/2903) merged at **2026-09-04 14:11:26 CDT**. At measurement it had run for **6d 1h 39m**; its initial seven-day window closes **2026-09-11 14:11:26 CDT**. The status table describes that measured window; its passing rows do not certify a later seven-day stable-release window.

Three criteria moved to met since the earlier report: #6–#8 have six days of production exposure. Criterion #4 now has production auto-recovery and redispatch evidence, but the controlled supported-driver drill remains unrecorded, so it is still only partially met. Criterion #3 moved from unobserved to demonstrably failing: the engine reaps its own live tmux workers on shutdown. Criterion #2 is a structural blocker that waiting cannot clear: automation has zero of its required five terminal executions because dispatch remains operator-paused.
...
| --- | ------------------------------------------------------------------------------------------------------------ | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Seven consecutive days, all local pools tmux-hosted                                                          | **NOT MET**                                     | 6d 1h 39m had elapsed at measurement. On the strict stable-release reading, four builds ran, with three changes in-window; the longest one-build run was 3d 16h 30m.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 4   | Successful two-threshold automatic recovery and redispatch through the genuine path                          | **PARTIALLY MET — controlled drill unrecorded** | Four production `stale-worker sweep: two-hour token-verified auto-reap firing` events used `auto_reap_threshold_secs: 7200`: **2026-09-05 00:00:17 CDT** (slot 2), **2026-09-07 15:11:29 CDT** (slot 3), and **2026-09-07 21:39:19 CDT** (slots 6 and 7). Seven orphan redispatches followed, including a **2026-09-05 00:00:17 CDT** reap followed by redispatch at **00:07:07 CDT**; review recovery also refired at **2026-09-05 22:59:33 CDT** and respawned at **22:59:39 CDT**. These events corroborate production recovery; they do not record the controlled stale-idle drill with test thresholds required by the design’s Phase 3. |
...
The Bazel recovery integration target exists at the measurement revision but remains manual (`tags = ["manual"]` in `tools/boss/engine/core/BUILD.bazel`) because Linux agents may lack host tmux. The PR’s three green Buildkite checks do not exercise it; its last recorded manual pass was **2026-09-04 CDT**. This is supporting evidence, not an additional numbered gate criterion or a substitute for the controlled supported-driver drill.
...
The twelve `pr_review` zombies terminalized in the **2026-09-07 11:53:30–11:56:09 CDT** bulk sweep are not a tmux-soak failure. The cited repairs address review-batch lifecycle and driver routing: [#2920](https://github.com/spinyfin/mono/pull/2920) finalizes reporting members and permits supervisor/leaf admission, while [#2923](https://github.com/spinyfin/mono/pull/2923) resolves each leaf’s driver from its member row so incoming hooks use the correct decoder. These public PRs provide the source trail for the diagnosis. The secondary tmux-shaped contributor is #2921, now running but unexercised.
...
- #4: a controlled stale-idle recovery drill through the supported-driver path, with commands, versions, thresholds, and outcomes recorded.
...
- #1 on the strict stable-release reading: the latest observed build (`281f869f`) started **2026-09-10 15:28:51 CDT**, so its earliest seven-day completion is **2026-09-17 15:28:51 CDT**, conditional on an unchanged build and continuous all-pool tmux exposure. The September 9 start marks deployment of the review fixes, not the latest build in the table. The volume floor and all incident criteria must be measured again for that same qualifying window; the September 4 aggregate cannot certify it.
