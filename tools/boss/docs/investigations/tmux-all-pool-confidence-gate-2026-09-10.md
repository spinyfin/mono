...
| 3   | Successful real engine-restart and app-restart drills with work continuing                                   | **NOT MET — positive counter-evidence**   | The engine reaped all **3/3** live tmux workers across two shutdowns: two workers at **2026-09-04 14:02:33 CDT**, then slot 25 (`boss-25-18d29eefff06`, pid 4914) at **2026-09-05 22:59:12 CDT**, preceded by `release_worker_pane` treating the absent app session as unconfirmed. The app-only restart at **2026-09-10 15:33:03–15:33:06 CDT** preserved engine pid 6114, `boss-coordinator`, and Claude pid 45198, but no worker pane was live; app-restart worker continuity remains unobserved. |
| 4   | Successful two-threshold automatic recovery and redispatch through the genuine path                          | **MET**                                   | Four production `stale-worker sweep: two-hour token-verified auto-reap firing` events used `auto_reap_threshold_secs: 7200`: **09-05 00:00:17** (slot 2), **09-07 15:11:29** (slot 3), and **09-07 21:39:19 CDT** (slots 6 and 7). Seven orphan redispatches followed, including a **00:00:17** reap followed by redispatch at **00:07:07 CDT**; review recovery also refired at **09-05 22:59:33** and respawned at **22:59:39 CDT**.                                                               |
| 5   | Successful normal-exit observation through retained `pane_dead` state                                        | **INSUFFICIENT EVIDENCE — telemetry gap** | `pane_dead` is probed on every adoption pass in `tmux_adoption.rs`, but a true value is neither logged at INFO/WARN nor persisted. Zero trace hits therefore cannot prove normal-exit observation. This is not evaluatable, not passing.                                                                                                                                                                                                                                                             |
...
| 7   | No unexplained token mismatch or tmux probe failure                                                          | **MET**                                   | Zero `TokenMismatch` and zero tmux probe-failure emissions appeared in-window from `boss_engine::tmux_adoption*`. The 3,757 unrelated `merge_poller: probe failed` warnings are GitHub merge-state probes, not tmux evidence.                                                                                                                                                                                                                                                                        |
...
| 9   | Known re-adoption repair present, no periodic live-state displacement                                        | **MET, confirmed at HEAD**                | `TMUX_RUN_ADOPTABLE_PREDICATE` in `tools/boss/engine/core/src/work/run_rows.rs:18-24` still filters terminal execution status rather than `r.status = 'active'`. There were 9,532 in-window retained-live-state re-adoptions and zero displacement events.                                                                                                                                                                                                                                           |
...
- Fifteen local runs have `tmux_hosted=0`, but all are pre-spawn Cube lease/network failures (`created_at = started_at = finished_at`) that never reached a pane. They are not app-hosted panes and do not break the all-pool soak.
...
The review-batch fixes entered the running engine only at **2026-09-09 15:29:34 CDT**. Only 20 tmux executions (13 review and seven interactive) have run on a build containing them.
...
- #10: the still-open quit-dialog correction.
