...
**As measured, the round-trip did not complete at all.** The live harness was raw `codex` under `tmux`, driving only the model side: it showed the model recognizing the write failure and delivering a valid fenced `ReviewResult` JSON block, exactly as the prompt's "Also (fallback)" instruction asks for. It never exercised the engine's ingestion side. That matters because `CodexDriver::structured_output_fallback` (`driver/src/codex.rs`) — the function `finalize_pr_review_pass` calls to recover a `ReviewResult` from the transcript when the artifact is absent — returned `Vec::new()` unconditionally at the time of this run, deliberately, because Codex had no prose-scrape convention wired up yet. So both channels were dead for a Codex-driven `PrReview` execution: the artifact write is denied by `--sandbox read-only` (established above), and the fenced-JSON transcript copy was discarded before it was ever parsed, `review_result_from_candidates` receiving zero candidates every time.

**That gap is fixed as part of this change.** `CodexDriver::structured_output_fallback` now delegates `ReviewResult` recovery to the same driver-neutral fenced/balanced-object scraper (`boss_engine_structured_output::fallback::json_object_candidates`) that the Claude driver already rides for this kind — it was never Claude-specific, just never wired up for Codex. A unit test (`codex_structured_output_fallback_recovers_review_result_from_fenced_json`, `driver/src/codex.rs`) feeds the exact transcript text captured in this investigation's live run and confirms the engine now recovers a valid candidate from it. The round-trip is therefore verified in two halves, not one live end-to-end run: the model-emission half by the live CLI harness below, and the engine-ingestion half by that unit test replaying the captured transcript. Codex reviewers still depend on the transcript-fallback machinery the engine's own comments call "TRANSITIONAL", on every run, not as a rare degrade path — but that machinery is now actually wired for Codex rather than silently discarding its output.
...
**Verdict: as measured live here, the primary artifact channel is structurally unreachable for a Reviewer-postured Codex worker on every run (a property of `--sandbox read-only`, not a flake), and the transcript-fallback channel was — until this change — silently dead on the engine side, because `CodexDriver::structured_output_fallback` returned no candidates.** With that fallback now implemented (see [Bug found and fixed](#bug-found-and-fixed-below)), the transcript-fallback channel is the one Codex reviewers depend on for every review, verified by replaying this section's captured transcript through a driver unit test rather than by a live end-to-end engine run.
...
**Codex's transcript fallback was a no-op for the exact channel the probe fix above offers.** `CodexDriver::structured_output_fallback` (`driver/src/codex.rs`) returned `Vec::new()` for every `StructuredOutputKind`, `ReviewResult` included, so a Codex reviewer that complied with the fixed probe still yielded zero candidates — the probe fix alone did not close the loop. Fixed by delegating `ReviewResult` recovery to `boss_engine_structured_output::fallback::json_object_candidates`, the same driver-neutral fenced/balanced-object scraper the Claude driver already uses for this kind (it was already driver-agnostic; Codex just never called it). Covered by a unit test that replays this section's exact captured transcript text and confirms a valid candidate comes back.
...
2. When that seam opens, a Codex reviewer's `ReviewResult` will round-trip correctly, but only via the transcript fallback — this investigation's driver unit test covers the fallback's parsing logic against the exact captured transcript, but a live engine run through a real dispatch to Codex has still never been exercised end-to-end; budget for that in whatever validates the rollout (the general Codex acceptance sweep's cross-transport conformance harness is the natural place to assert it, since it already asserts stdout-JSONL/hook-ingress equivalence for the same kind of channel-parity question).
3. Both bugs fixed here — the probe wording and the previously-empty Codex transcript fallback — should stay fixed regardless of when/whether that seam opens; they were real bugs in shared, driver-agnostic code and in the Codex driver, independent of the dispatch-policy question.
...
| Does structured `ReviewResult` output round-trip through the environment-file contract the driver actually uses?           | **Yes, but only via the transcript fallback, and only as of this change** — the primary artifact channel is structurally unreachable under Reviewer posture (not a flake; true on every run), and `CodexDriver::structured_output_fallback` returned zero candidates for `ReviewResult` before this investigation fixed it. The fix is verified by a driver unit test replaying the exact transcript captured here, not by a live engine-ingestion run. |
