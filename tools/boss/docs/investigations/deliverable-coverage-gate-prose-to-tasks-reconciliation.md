...
- **Planner** — reusable LLM "mini-coordinator". `Planner::plan(api_key, &PlannerInput) -> PlannerOutcome` (`engine/core/src/planner.rs`). Reads `PlannerInput { design_doc, design_doc_ref: DocRef, project, product, existing_tasks, max_tasks }` (`protocol/src/planner.rs:60`) and emits a **task-graph proposal** `PlannerOutput { tasks, edges, merge_order_hints, confidence, breakdown_found, notes, effort_audit }` (`:173`); each `ProposedTask { handle, name, description, kind, effort, ordinal }` (`:116`). It never writes rows.
- **Validation** — pure gate `planner-validation::validate(output, max_tasks)` (`engine/planner-validation/src/lib.rs`): `NoBreakdown` / `EmptyBreakdown` / `RejectedTooMany` / `RejectedDuplicateHandle` / `RejectedUnknownHandle` / `RejectedCycle`. The former per-task decomposition gate (`detect_oversize_tasks` and `OVERSIZE_DELIVERABLE_CLAUSES`) was deleted in mono#2695, so this proposal derives its premise from the whole-design coverage model rather than assuming that gate exists.
...
The key observation: the Planner already reads the whole doc in one pass. The extraction and coverage work proposed here is a **second output of that same pass** plus a **new closeout-time consumer** — not a new agent.
