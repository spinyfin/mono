...
- [`incident-004-live-revision-workers-reaped-mid-turn.md`](incident-004-live-revision-workers-reaped-mid-turn.md) — 2026-08-08. The merge-poller's staged-PR-URL fast path (`pr_recheck_staged`) finalized and reaped live revision workers mid-turn, discarding post-push work or entire unpushed diffs, while board status could still report review success. The path short-circuits protections written after a prior race of the same class. Measured over the retained trace window: 63 of 64 staged-path finalizations reaped a mid-turn worker (98.4%), 6 total losses, and 5 work items left claiming review status having contributed nothing.
