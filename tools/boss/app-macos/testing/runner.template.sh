...
# Derived from rules_apple's macos_test_runner.template.sh.
# Fix: dropped `variant=macos` from the xcodebuild destination specifier, which
# was removed in Xcode 26.5 and caused "Supported platforms for the buildables
# in the current scheme is empty" followed by "Failed to create a bundle
# instance". See mac-app-build incident.
...
# failed.
#
# Note: `variant=macos` was removed from the destination. It was used to
# distinguish native macOS from Mac Catalyst, but was dropped in Xcode 26.5
# and caused "Supported platforms for the buildables in the current scheme is
# empty". `platform=macOS,arch=<arch>` is sufficient for native macOS tests.
