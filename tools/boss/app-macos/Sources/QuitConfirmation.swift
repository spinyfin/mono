...
///
/// Local workers are always tmux-hosted, so quit does not touch tmux. The
/// engine is launched detached and `EngineProcessController.stop()` is a
...
enum QuitConfirmation {
    ...
    static func informativeText(agentCount: Int) -> String {
    ...
    /// alert is still produced — quit is not silent just because it is
    /// non-destructive to the agents.
    @MainActor
    static func alert(agentCount: Int) -> NSAlert? {
        guard agentCount > 0 else { return nil }
        let alert = NSAlert()
        alert.messageText = messageText
        alert.informativeText = informativeText(agentCount: agentCount)
        ...
        alert.buttons[1].hasDestructiveAction = false
        return alert
    }
}
