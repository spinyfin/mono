...
fn runs_from_list_event(event: FrontendEvent) -> Result<Vec<WorkRun>, CliError> {
    match event {
        ...
        other => Err(unexpected_event("execution runs", &other)),
    }
}
...
pub(crate) fn format_dependency_edge_line(edge: &DependencyEdge, mark_incomplete: bool) -> String {
    ...
    if edge.status == "archived" {
        ...
        let timestamp = edge.archived_at.as_deref().unwrap_or("unknown time");
...
pub(crate) fn ensure_patch_present(patch: &WorkItemPatch, message: &str) -> Result<(), CliError> {
    let has_fields = patch.name.is_some()
        ...
        || patch.effort_matched_rule.is_some()
        || patch.effort_reasons.is_some()
