...
pub(crate) enum Commands {
    ...
    /// Read-only reporting over recorded per-run token spend
    /// (`work_runs.input_tokens` / `output_tokens` / `cache_creation_tokens`
    /// / `cache_read_tokens`). Token counts are the measured figures the
    /// system recorded; `--usd` adds an optional, clearly-labelled estimate
    /// against a human-maintained pricing table — never billing truth.
    Cost {
        #[command(subcommand)]
        command: CostCommand,
    },
    /// Read-only performance series over the primary tables: review
    /// duration, execution duration, task lead time, execution outcomes,
    /// and PRs generated. `boss metrics catalog` lists series and
    /// dimensions; `boss metrics series` buckets one series. `--json`
    /// renders the wire shape. Distinct from `bossctl metrics`, which
    /// still lists the live counter registry.
    Metrics {
        #[command(subcommand)]
...
pub(crate) enum TaskCommand {
    ...
    Show(TaskIdArg),
    /// Update any leaf work item (task or chore) by id. Boxed: avoids bloating every other variant.
