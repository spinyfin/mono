...
pub(crate) fn build_cli_reference() -> Result<CliReferenceDocument, CliError> {
    ...
    Ok(CliReferenceDocument {
        ...
        usage_rules: vec![
            ...
            "Kind-agnostic verbs (show, update, move, cancel, delete, restore, depend, bind-pr, link-external, unlink-external) accept any leaf work item id under either `boss task` or `boss chore` — a chore is a kind of task. Use whichever noun reads more naturally for the call site; the engine resolves the kind from the id.",
        ...
        status_semantics: vec![
            "Task and chore status uses the board (kanban) names: backlog, doing, review, done, blocked, archived, cancelled. These are the canonical values shown in --status help and emitted in --json.",
            ...
            "boss task|chore update --status and --status list filters accept either vocabulary; boss task|chore move --to backlog|doing|review|done|blocked|archived|cancelled (legacy names also accepted).",
            ...
            "cancelled is another terminal status for leaf work items, distinct from both archived and delete: `boss task|chore cancel <id>` (shorthand for `move --to cancelled`) marks a row as abandoned/won't-do while keeping it queryable and NOT soft-deleted (deleted_at stays NULL). Prefer it over `delete` when the work was consciously abandoned rather than filed in error — `delete` soft-deletes and is meant for the latter, recoverable via `restore`. Like archived, cancelled rows are hidden from `boss task|chore list` by default; pass `--include-archived` or filter `--status cancelled` explicitly to see them (the flag name is shared with archived — both are terminal statuses meant to fall off the default `list` output; the engine-side kanban board still excludes only archived).",
