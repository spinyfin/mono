...
/// Resolve `boss comment list`'s `--task`/`--artifact`/`--artifact-kind`
/// flags to a single `(artifact_kind, artifact_id)` pair. Mirrors the
/// bossctl helper of the same shape so the two CLIs stay interchangeable.
pub(crate) fn resolve_comments_artifact(
    task: Option<String>,
    artifact: Option<String>,
    artifact_kind: String,
) -> Result<(String, String), CliError> {
    match (task, artifact) {
        (Some(_), Some(_)) => Err(CliError::usage("pass only one of --task or --artifact")),
...
/// `boss comment list` — every comment on an artifact, each paired with
/// its thread-entry count and answer-agent running/failed flags. Engine
/// RPC over `CommentsList` (folds the bossctl direct-DB verb onto the
/// engine surface).
pub(crate) async fn run_comment_list(ctx: &RunContext, args: CommentListArgs) -> Result<(), CliError> {
    let (artifact_kind, artifact_id) = resolve_comments_artifact(args.task, args.artifact, args.artifact_kind)?;
    let mut client = connect_for_work(ctx).await?;
    match client
        ...
        .map_err(CliError::internal)?
    {
        FrontendEvent::CommentsList {
            ...
            || {
                if comments.is_empty() {
                    println!("no comments on {artifact_kind}:{artifact_id}");
                    return;
                }
        ...
        other => Err(unexpected_event("comment list", &other)),
    }
}

/// `boss comment show <id>` — one comment's full detail + thread +
/// answer-agent-run history. Engine RPC over `CommentsGet`.
pub(crate) async fn run_comment_show(ctx: &RunContext, args: CommentShowArgs) -> Result<(), CliError> {
    let mut client = connect_for_work(ctx).await?;
    match client
        ...
        .map_err(CliError::internal)?
    {
        FrontendEvent::CommentsGetResult {
            ...
            &serde_json::json!({
                ...
                "answer_agent_runs": answer_agent_runs,
            }),
            || {
                ...
                if thread_entries.is_empty() {
                    ...
                    println!("thread ({}):", thread_entries.len());
                ...
                print_answer_agent_runs(&answer_agent_runs);
            },
        ),
        FrontendEvent::WorkError { message } | FrontendEvent::Error { message, .. } => {
            Err(CliError::application(message))
        }
        other => Err(unexpected_event("comment show", &other)),
    }
}

/// `boss comment runs <id>` — every `answer_agent_runs` row for a comment.
/// Engine RPC over `ListAnswerAgentRuns`.
pub(crate) async fn run_comment_runs(ctx: &RunContext, args: CommentRunsArgs) -> Result<(), CliError> {
    let mut client = connect_for_work(ctx).await?;
    match client
        ...
        .map_err(CliError::internal)?
    {
        ...
        other => Err(unexpected_event("comment runs", &other)),
    }
}
