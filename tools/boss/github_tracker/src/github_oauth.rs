...
const FILE_FALLBACK_NAME: &str = "github-oauth-token";

/// Error type for [`KeychainTokenStore`] operations.
#[derive(Debug, thiserror::Error)]
pub enum TokenStoreError {
    #[error("keychain error: {0}")]
    Keychain(#[from] boss_keychain::TokenStoreError),
...
/// The backend selection (macOS Data Protection Keychain vs. dev-build file
/// fallback vs. plain `keyring` elsewhere) lives in [`boss_keychain`], shared
/// with `boss-trunk-auth`. Production code constructs this with
/// [`KeychainTokenStore::new`]; tests supply a [`FakeStore`] via
/// [`KeychainTokenStore::with_backend`].
pub struct KeychainTokenStore {
    inner: boss_keychain::KeychainStore,
}
...
impl KeychainTokenStore {
    ...
    pub fn new() -> Self {
        Self {
            inner: boss_keychain::KeychainStore::new(KEYCHAIN_SERVICE, KEYCHAIN_ACCOUNT, FILE_FALLBACK_NAME),
        }
    }
    ...
    /// `#[cfg(test)]` builds.
    #[cfg(test)]
    pub(crate) fn with_backend(backend: impl boss_keychain::KeystoreBackend + 'static) -> Self {
        Self {
            inner: boss_keychain::KeychainStore::with_backend(backend),
        }
    }

    /// Returns the stored [`TokenRecord`], or `None` if no token is present.
    pub fn get(&self) -> Result<Option<TokenRecord>, TokenStoreError> {
        match self.inner.get_raw()? {
...
/// In-memory [`boss_keychain::KeystoreBackend`] for tests.  Never touches the
/// real keychain.
#[cfg(test)]
...
impl boss_keychain::KeystoreBackend for FakeStore {
    fn get_raw(&self) -> Result<Option<String>, boss_keychain::TokenStoreError> {
        Ok(self.0.lock().unwrap().clone())
    }

    fn set_raw(&self, value: &str) -> Result<(), boss_keychain::TokenStoreError> {
        *self.0.lock().unwrap() = Some(value.to_owned());
        Ok(())
    }

    fn delete_raw(&self) -> Result<(), boss_keychain::TokenStoreError> {
