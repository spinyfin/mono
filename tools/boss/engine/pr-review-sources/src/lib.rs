...
use boss_github::trees::encode_repo_path;
...
async fn fetch_pinned_tree_entries<T: SourceTransport + Sync + Send + Clone + 'static>(
    ...
    let pinned_sha = sha.to_owned();
    fetch_directory_entries(repository, sha, paths, side, move |directory, names| {
...
where
    ...
    Fut: std::future::Future<
        ...
        + 'static,
{
    let pending = group_paths(paths).into_iter().map(|(directory, names)| {
        let result = fetch(directory.clone(), names.clone());
        async move { (directory, names, result.await) }
    });
    let resolved = resolve_bounded(pending).await?;
    let mut entries = HashMap::new();
    let mut omissions = Vec::new();
    for (directory, names, result) in resolved {
        match result {
...
/// Keep at most eight GitHub subprocess reads active at once.
async fn resolve_bounded<F, T>(requests: impl IntoIterator<Item = F>) -> Result<Vec<T>>
where
    F: std::future::Future<Output = T> + Send + 'static,
    T: Send + 'static,
{
    let caller = boss_gh_telemetry::current_caller();
    ...
    loop {
        while pending.len() < 8 {
            ...
            pending.spawn(boss_gh_telemetry::scope(caller, request));
        }
        let Some(result) = pending.join_next().await else { break };
        results.push(result.context("pinned source read task failed")?);
    }
    Ok(results)
}
...
async fn fetch_source<T: SourceTransport + Clone + Send + Sync + 'static>(
    transport: &T,
    repository: &str,
    sha: &str,
    path: &str,
