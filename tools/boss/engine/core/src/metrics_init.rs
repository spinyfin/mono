...
mod tests {
    ...
    fn init_all_registers_all_declared_counters() {
        ...
        // Codex unobserved-command detection counters.
        for expected in ["codex.unobserved_command", "codex.unobserved_command_overflow"] {
            assert!(
                names.contains(&expected.to_owned()),
                "init_all must register {expected}"
            );
        }
        // Codex PreToolUse guard observation counters.
        for expected in ["codex.guard_trace.reported", "codex.guard_trace.silent"] {
            assert!(
                names.contains(&expected.to_owned()),
                "init_all must register {expected}"
            );
        }
        // incident-004 AI-4: mid-turn-reap aggregate counter. Per-source
        // breakdown (`completion.mid_turn_reap.<source>.count`) is
        // registered dynamically on first use, like the github_api
        // per-caller breakdown above.
        assert!(
            names.contains(&"completion.mid_turn_reap.total".to_owned()),
            "init_all must register completion.mid_turn_reap.total"
        );
        assert_eq!(
            names.len(),
            103,
            "expected 6 answer_agent + 6 pr_url_capture + 5 worker_proposals fallback_hit + 3 cube_workspace_lease + \
             ...
             11 worker_proposals submit + 1 worker_proposals channel_error + \
