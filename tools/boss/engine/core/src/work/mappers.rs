...
///  intent_classification_failed_at, intent_classification_error, reopened_at`.
...
pub(crate) fn map_comment(row: &Row<'_>) -> rusqlite::Result<WorkComment> {
    ...
    Ok(WorkComment {
        ...
        reopened_at: row.get::<_, Option<String>>(21)?.filter(|s| !s.is_empty()),
    })
}
...
pub(crate) fn map_task(row: &Row<'_>) -> rusqlite::Result<Task> {
    ...
    Ok(Task {
        ...
        // Not part of the base 33-column SELECT; populated only by the
        ...
        effort_level,
        // Effort provenance is not part of the base 33-column SELECT;
        // populated only by the extended mappers below (same pattern as
        // blocked_detail / deferred / tags).
        effort_matched_rule: None,
        effort_reasons: None,
        ...
        // Resolved per-task doc-link state. Computed by
        // attach_task_doc_link_state from the task's `doc_*` columns on
        ...
        // dispatch decision.
        deferred: false,
        // Human-driven classification. Same trailing-column pattern as
        // deferred; `work_item_is_human_driven` does a targeted read for
        // dispatch/completion gates.
        human_driven: false,
