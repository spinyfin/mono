...
impl WorkDb {
    ...
    pub fn tmux_run_for_execution(&self, execution_id: &str) -> Result<Option<TmuxRunHandle>> {
        ...
        conn.query_row(
            ...
            |row| {
                Ok(TmuxRunHandle {
                    run_id: row.get(0)?, execution_id: row.get(1)?, agent_id: row.get(2)?,
                    transcript_path: row.get(3)?, tmux_server_label: row.get(4)?,
                    tmux_session_name: row.get(5)?, tmux_spawn_token: row.get(6)?,
                    tmux_spawn_state: row.get(7)?, tmux_pane_pid: row.get(8)?,
                })
            },
        )
        .optional()
