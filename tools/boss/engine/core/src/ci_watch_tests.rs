...
use crate::work::{TaskStatus, WorkDb, WorkItem, WorkItemPatch};
...
fn ci_revision_description_empty_slice_is_generic() {
    assert_eq!(super::ci_revision_description(&[]), "Fix failing CI");
}
...
fn ci_revision_description_all_blank_names_is_generic() {
    ...
    assert_eq!(super::ci_revision_description(&failures), "Fix failing CI");
}

#[test]
fn ci_revision_description_lists_up_to_three_names() {
    let one = vec![failure("ci/test", "FAILURE")];
    assert_eq!(super::ci_revision_description(&one), "Fix failing CI: ci/test");
    ...
    assert_eq!(
        super::ci_revision_description(&three),
...
fn ci_revision_description_more_than_three_appends_more_tail() {
    ...
    assert_eq!(
        super::ci_revision_description(&five),
...
fn ci_revision_description_excludes_blanks_from_list_and_count() {
    ...
    assert_eq!(
        super::ci_revision_description(&mixed),
        "Fix failing CI: ci/test, ci/lint, ci/build",
    );
    ...
    assert_eq!(
        super::ci_revision_description(&mixed_overflow),
