...
/// `exec_18c50a0a900baa40_54` / spinyfin/mono#2070). GitHub reported
/// `mergeable: CONFLICTING` / `mergeStateStatus: DIRTY` the whole time.
/// Divergent `jj` change ids made two diagnostic revsets resolve to two
/// *different* commits — one conflict-free but stacked on stale `main`,
/// one on current `main` but conflicted — and the worker ANDed the two
/// answers into a conclusion no single commit supported. `jj` flagged the
/// hazard with a `??` suffix from its first line of output; nothing in
/// the brief told the worker what that meant.
///
...
fn compose_conflict_ground_truth_fragment(attempt: &ConflictResolution) -> String {
    format!(
         ...
         **DIVERGENT**: change-id revsets resolve to an arbitrary copy, so every \
         ...
         hard-fail with \"resolved to more than one revision\". Abandon the stale duplicates \
         by full commit id before continuing.\n\n",
...
mod compose_prompt_tests {
    ...
    fn conflict_fragment_hard_gates_the_ground_truth_commands_before_local_jj_reasoning() {
        // 2026-07-23 incident (exec_18c50a0a900baa40_54 /
        // spinyfin/mono#2070): the worker went straight to `jj log`, never
        // queried `mergeable`, never ran `cube workspace rebase`, and
        // concluded "already resolved" from two divergent-change-id revsets
        // that answered about two different commits. The brief must order
        // the two authoritative commands first, forbid the local-state-only
        // conclusion, and name the `??` divergence hazard.
        let work_item = revision_task_with_created_via(None, "merge-conflict:crz_frag_01");
        let attempt = sample_conflict_attempt();
        let prompt = compose_execution_prompt(
            ExecutionPromptParams::builder()
