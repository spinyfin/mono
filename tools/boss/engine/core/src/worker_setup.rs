...
mod worker_setup_heal;

use worker_setup_heal::heal_single_settings_json;

#[cfg(test)]
use crate::ssh_transport::shell_quote;
#[cfg(test)]
pub(crate) use worker_setup_heal::heal_hook_command;
...
pub fn worker_kind_for_execution(kind: &ExecutionKind) -> WorkerKind {
    match kind {
        ...
        ExecutionKind::PrReviewGuide => WorkerKind::ReviewGuide,
...
fn deny_rules(input: &WorkerSetupInput, fence: DataDirFence) -> Vec<String> {
    ...
    match input.worker_kind {
        ...
        WorkerKind::ReviewGuide => rules.extend(review_guide_deny_rules()),
        WorkerKind::Standard => {}
    }
