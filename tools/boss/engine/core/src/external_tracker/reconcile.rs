...
mod tests {
    ...
    use async_trait::async_trait;
    use boss_protocol::{CreateProductInput, WorkItemPatch};
    use serde_json::json;
    ...
    async fn b8_auto_syncs_when_only_upstream_changed() {
        ...
        let tracker = SpyTracker::new(vec![open_item(100, "Original Title")]);
        let registry = spy_registry(Arc::clone(&tracker));
        let metrics = Registry::new();
        register_metrics(&metrics);
        let outcome1 = run_one_pass(&db, &registry, &metrics, &noop_pub(), &ambient_resolver()).await;
        ...
        let mut updated = open_item(100, "Updated Title");
        updated.body = "Updated body text".to_owned();
        let tracker2 = SpyTracker::new(vec![updated]);
        let registry2 = spy_registry(tracker2);
        let metrics2 = Registry::new();
        register_metrics(&metrics2);
        let publisher = Arc::new(RecordingPublisher::default());
        let outcome2 =
            run_one_pass(&db, &registry2, &metrics2, publisher.as_ref(), &ambient_resolver()).await;
    ...
    async fn b8_skips_sync_when_both_sides_changed() {
        ...
        // Tick 1: import the item.
        let tracker = SpyTracker::new(vec![open_item(101, "Original Title")]);
        let registry = spy_registry(Arc::clone(&tracker));
        let metrics = Registry::new();
        register_metrics(&metrics);
        run_one_pass(&db, &registry, &metrics, &noop_pub(), &ambient_resolver()).await;
        ...
        let tracker2 = SpyTracker::new(vec![open_item(101, "Upstream Changed Title")]);
        let registry2 = spy_registry(tracker2);
        let metrics2 = Registry::new();
        register_metrics(&metrics2);
        let outcome2 =
            run_one_pass(&db, &registry2, &metrics2, &noop_pub(), &ambient_resolver()).await;
    ...
    async fn b8_no_op_when_nothing_changed() {
        ...
        // Tick 1: import.
        let tracker = SpyTracker::new(vec![open_item(102, "Stable Title")]);
        let registry = spy_registry(Arc::clone(&tracker));
        let metrics = Registry::new();
        register_metrics(&metrics);
        run_one_pass(&db, &registry, &metrics, &noop_pub(), &ambient_resolver()).await;
        ...
        let tracker2 = SpyTracker::new(vec![open_item(102, "Stable Title")]);
        let registry2 = spy_registry(tracker2);
        let metrics2 = Registry::new();
        register_metrics(&metrics2);
        let outcome2 =
    ...
    async fn b8_import_stores_upstream_baseline() {
        ...
        let tracker = SpyTracker::new(vec![open_item(103, "Baseline Title")]);
        let registry = spy_registry(Arc::clone(&tracker));
        let metrics = Registry::new();
        register_metrics(&metrics);
        run_one_pass(&db, &registry, &metrics, &noop_pub(), &ambient_resolver()).await;
