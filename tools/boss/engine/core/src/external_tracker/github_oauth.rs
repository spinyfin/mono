...
//!
//! The durable half — [`TokenRecord`] and [`KeychainTokenStore`] — lives in
//! `boss-external-tracker` and is re-exported here, so the credential
//! resolver can read a stored token without depending on this module (which
//! needs [`WorkDb`] to record org-authorization state).
...
pub use boss_external_tracker::token_store::{KeychainTokenStore, TokenRecord, TokenStoreError};
...
mod tests {
    ...
    use boss_external_tracker::token_store::FakeStore;
