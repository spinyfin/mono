...
pub use server::{ServeOverrides, process_is_alive, run, serve, serve_with_merge_probe, serve_with_overrides};
...
impl ServerState {
    ...
    /// Resolve the tmux handle for a run recorded with `server_label`:
    /// `socket_tmux` itself, unless the label is the pre-move
    /// [`boss_tmux::SERVER_LABEL`], in which case a `-L boss` handle from the
    /// same executable and runner. Shared by teardown, pane delivery, and
    /// [`crate::stale_worker_sweep::TmuxWorkerTerminalInspector::tmux_for_run`].
    /// An empty/unset label resolves to `socket_tmux`.
    fn tmux_for_run(&self, socket_tmux: &boss_tmux::Tmux, server_label: &str) -> anyhow::Result<boss_tmux::Tmux> {
        if server_label == boss_tmux::SERVER_LABEL {
            boss_tmux::Tmux::for_legacy_label_server_with_runner(
                socket_tmux.program().to_path_buf(),
                socket_tmux.runner(),
            )
        } else {
    ...
    fn new_arc_with_app_pid_and_merge_probe(
        ...
        let server_state = Arc::new_cyclic(move |weak_self: &Weak<ServerState>| {
            ...
            ServerState::builder()
                ...
                .event_bus(event_bus)
                .worker_registry(worker_registry_override.unwrap_or_else(|| {
                    WorkerRegistry::with_sleep_assertion(Arc::new(
...
async fn handle_frontend_connection(
    ...
    loop {
        ...
        let ctx = Dispatch::builder()
            ...
            .decode_ms(decode_ms)
            .build();
        // Box each arm's future individually so the outer match holds a
        // single pointer rather than a generator sized to the sum of every
        // arm. Boxing only the whole `match` as one `async move` block (the
        // prior mitigation here) still materializes the summed state machine
        // as a stack temporary before moving it to the heap; the stack
        // requirement grows with every new `FrontendRequest` variant and
        // previously overflowed Linux CI's sandbox thread once Ideas added
        // more variants (`control_verbs_test` aborted with `has overflowed
        // its stack`).
        let dispatch_fut: Pin<Box<dyn Future<Output = ()> + Send>> = match request {
            ...
            r @ FrontendRequest::ActionAttentionGroup { .. } => {
                Box::pin(attentions::handle_action_attention_group(ctx, r))
            }
            ...
            r @ FrontendRequest::CreateInvestigation { .. } => {
                Box::pin(work_items::handle_create_investigation(ctx, r))
            }
            ...
            r @ FrontendRequest::ExecutionTranscript { .. } => {
                Box::pin(executions::handle_execution_transcript(ctx, r))
            }
            ...
            r @ FrontendRequest::GetDriverTrafficSplit => {
                Box::pin(engine_meta::handle_get_driver_traffic_split(ctx, r))
            }
            ...
            r @ FrontendRequest::GetDispatchConcurrency => {
                Box::pin(engine_meta::handle_get_dispatch_concurrency(ctx, r))
            }
            ...
            r @ FrontendRequest::GetWorkItemCostReport { .. } => {
                Box::pin(cost::handle_get_work_item_cost_report(ctx, r))
            }
            ...
            r @ FrontendRequest::ListAnswerAgentRuns { .. } => {
                Box::pin(comments::handle_list_answer_agent_runs(ctx, r))
            }
            r @ FrontendRequest::ListAttentionGroups { .. } => {
                Box::pin(attentions::handle_list_attention_groups(ctx, r))
            }
            ...
            r @ FrontendRequest::ListAttentionMerges { .. } => {
                Box::pin(attentions::handle_list_attention_merges(ctx, r))
            }
            ...
            r @ FrontendRequest::ListAutomationRuns { .. } => {
                Box::pin(automations::handle_list_automation_runs(ctx, r))
            }
            ...
            r @ FrontendRequest::RecordEffortEscalation { .. } => {
                Box::pin(effort::handle_record_effort_escalation(ctx, r))
            }
            ...
            r @ FrontendRequest::RegisterCapabilities { .. } => {
                Box::pin(engine_meta::handle_register_capabilities(ctx, r))
            }
            ...
            r @ FrontendRequest::RetryCiRemediation { .. } => {
                Box::pin(ci_remediation::handle_retry_ci_remediation(ctx, r))
            }
            ...
            r @ FrontendRequest::SetProjectDesignDoc { .. } => {
                Box::pin(projects::handle_set_project_design_doc(ctx, r))
            }
            ...
            r @ FrontendRequest::MoveWorkItemOnBoard { .. } => {
