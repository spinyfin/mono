...
async fn handle_frontend_connection(
    ...
    loop {
        ...
        let ctx = Dispatch::builder()
            ...
            .decode_ms(decode_ms)
            .build();
        // Heap-allocate the dispatch future. The match is one state machine
        // over every `FrontendRequest` arm; putting it on the stack overflowed
        // Linux CI's sandbox thread once Ideas added more variants
        ...
        Box::pin(async move {
            match request {
                ...
                r @ FrontendRequest::CommentsPostFollowup { .. } => {
                    comments::handle_comments_post_followup(ctx, r).await
                }
                ...
                r @ FrontendRequest::GetDriverQuotaUsage { .. } => {
                    engine_meta::handle_get_driver_quota_usage(ctx, r).await
                }
                r @ FrontendRequest::GetDriverTrafficSplit => {
                    engine_meta::handle_get_driver_traffic_split(ctx, r).await
                }
                ...
                r @ FrontendRequest::GetDispatchConcurrency => {
                    engine_meta::handle_get_dispatch_concurrency(ctx, r).await
                }
                ...
                r @ FrontendRequest::GetWorkItemCostReport { .. } => {
                    cost::handle_get_work_item_cost_report(ctx, r).await
                }
                ...
                r @ FrontendRequest::ListAnswerAgentRuns { .. } => {
                    comments::handle_list_answer_agent_runs(ctx, r).await
                }
                ...
                r @ FrontendRequest::ListAttentionMerges { .. } => {
                    attentions::handle_list_attention_merges(ctx, r).await
                }
                ...
                r @ FrontendRequest::RegisterCapabilities { .. } => {
                    engine_meta::handle_register_capabilities(ctx, r).await
                }
                ...
                r @ FrontendRequest::SpawnCapabilityRestored => {
                    sessions::handle_spawn_capability_restored(ctx, r).await
                }
                ...
                r @ FrontendRequest::MoveWorkItemOnBoard { .. } => {
                    work_items::handle_move_work_item_on_board(ctx, r).await
                }
