...
impl WorkerCompletionHandler {
    ...
    pub async fn recheck_for_pr(&self, execution_id: &str) -> StopOutcome {
                ...
                .await
        {
            ...
            // A staged URL is recovery evidence. Declaration enforcement is
            // owned by the Stop-boundary backstop, which supplies the
            // required ask/park outcome for an undeclared run.
            tracing::info!(
                execution_id,
                pr_url = %staged_url,
        ...
        // - `head_now != revision_stop_contributed_head` (including NULL) → the
        //   current head has no matching Stop attribution; defer without
