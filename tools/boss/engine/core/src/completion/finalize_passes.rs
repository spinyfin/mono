...
impl WorkerCompletionHandler {
    /// Proposal acknowledgement is the delivery boundary; no transcript is read.
    pub async fn finalize_submitted_review_guide(&self, execution_id: &str) -> StopOutcome {
        let execution = match self.work_db.get_execution(execution_id) {
            Ok(execution) => execution,
            Err(_) => return StopOutcome::DbError,
        };
        ...
        self.finalize_review_guide(&execution).await
    }
    ...
    /// Finalize a `pr_review_guide` execution: consume its submitted
    /// Markdown proposal, validate it against the
    ...
    pub(super) async fn finalize_review_guide(&self, execution: &crate::work::WorkExecution) -> StopOutcome {
        ...
        let published = if let Some(attempt) = &attempt {
            match self.work_db.submitted_review_guide(&execution.id, &attempt.id) {
                Err(err) => {
                    let reason = format!("could not load the submitted review guide: {err:#}");
                    if let Err(error) = self.work_db.fail_pr_review_guide_attempt(&attempt.id, &reason) {
                        tracing::error!(?error, execution_id = %execution.id, "failed to record guide submission lookup failure");
                    }
                    false
                }
                Ok(None) => {
                    if let Err(err) = self.work_db.fail_pr_review_guide_attempt(
                        &attempt.id,
                        "run ended without submitting a guide via boss propose review-guide",
                    ) {
                        tracing::error!(execution_id = %execution.id, attempt_id = %attempt.id, ?err, "review-guide finalizer: failed to record the missing-submission failure");
                    }
                    false
                }
        ...
        .await;
        self.publisher
            .publish(&execution.id, &comparison_id, "completed", "review_guide_completed")
            .await;
    ...
    async fn read_final_triage_message_inner(
        ...
        match self.read_assistant_text_turns_inner(execution_id, driver).await {
            AssistantTurnRead::Turns(turns) => TriageTranscript::FinalMessage(turns.join("\n")),
            AssistantTurnRead::NoPath => TriageTranscript::NoPath,
            AssistantTurnRead::Unreadable => TriageTranscript::Unreadable,
            AssistantTurnRead::NoAssistantText { event_count } => TriageTranscript::NoAssistantText { event_count },
        }
    }

    async fn read_assistant_text_turns_inner(
        &self,
        execution_id: &str,
        driver: Option<&dyn crate::driver::AgentDriver>,
    ) -> AssistantTurnRead {
        ...
        for attempt in 1..=TRIAGE_TRANSCRIPT_READ_ATTEMPTS {
            ...
            if !all_text.is_empty() {
                ...
                return AssistantTurnRead::Turns(all_text);
            }
        ...
        AssistantTurnRead::NoAssistantText {
            event_count: last_event_count,
        }
    }
    ...
    pub(super) async fn check_pure_rebase_skip(
        ...
        pure_rebase_skip_gate(
            &self.work_db,
            self.branch_verifier.as_ref(),
            pr_url,
            producing,
            cycle_root_id,
        )
    ...
    pub(super) async fn check_noop_skip(
        ...
        noop_skip_reason(
            ...
            last_reviewed_sha,
            head_oid_hint,
            self.min_review_changed_lines,
        )
        .await
    }
}

/// Free-function body of [`WorkerCompletionHandler::check_pure_rebase_skip`] —
/// needs only `work_db` and `branch_verifier`, not a whole `Arc<Self>`, so
/// the declared-delivery completion path
/// ([`super::run_done_declaration::maybe_enqueue_declared_delivery_reviewer`])
/// can share the exact same gate instead of reimplementing it. See the
/// method's doc for the full rule description.
pub(super) async fn pure_rebase_skip_gate(
    ...
    let task = match work_db.get_work_item(&producing.work_item_id) {
    ...
    let (pre_head, base_ref) = if created_via.starts_with(CREATED_VIA_MERGE_CONFLICT_PREFIX) {
        let cr = match work_db.list_conflict_resolutions(None, &[], Some(root), None) {
            Ok(rows) => rows
                .into_iter()
        ...
        let attempt = match work_db.list_ci_remediations(None, &[], Some(root), None) {
            Ok(rows) => rows
                .into_iter()
                .find(|r| r.revision_task_id.as_deref() == Some(producing.work_item_id.as_str())),
            Err(err) => {
    ...
    let post_head = match branch_verifier.fetch_pr_head_oid(&repo_slug, pr_number).await {
        Ok(sha) => sha,
        Err(err) => {
            tracing::warn!(
                pr_url,
    ...
    let base_ref = match base_ref {
        Some(base) => base,
        None => match branch_verifier.fetch_pr_base_ref(&repo_slug, pr_number).await {
            Ok(base) => base,
            Err(err) => {
                tracing::warn!(
                    pr_url,
    ...
    let diff_before = match branch_verifier
    ...
    let diff_after = match branch_verifier
    ...
    record_pure_rebase_skip(work_db, root, &created_via, &pre_head, &post_head);
    PureRebaseGateOutcome {
...
/// Best-effort `[pr-review-skip]` audit line — see [`pure_rebase_skip_gate`].
/// A failure here never blocks the skip decision itself; it is logged and
/// swallowed, mirroring `append_reconcile_audit_best_effort`.
fn record_pure_rebase_skip(
    work_db: &crate::work::WorkDb,
    work_item_id: &str,
    created_via: &str,
    pre_head: &str,
    ...
    if let Err(err) = crate::reconcile_audit::append_description_line(work_db, work_item_id, &line) {
        tracing::warn!(
            work_item_id,
            ?err,
...
/// Free-function body of [`WorkerCompletionHandler::check_noop_skip`] — see
/// that method's doc for the four-rule description. Shared with the
/// declared-delivery completion path
/// ([`super::run_done_declaration::maybe_enqueue_declared_delivery_reviewer`])
/// so a future change to the skip rules cannot silently apply to only one of
/// the two completion paths.
pub(super) async fn noop_skip_reason(
    branch_verifier: &dyn BranchVerifier,
    pr_url: &str,
    producing: &crate::work::WorkExecution,
    review_cycle: i64,
    ...
    let current_head = match head_oid_hint {
        Some(sha) => sha,
        None => match branch_verifier.fetch_pr_head_oid(&repo_slug, pr_number).await {
            Ok(sha) => sha,
            Err(err) => {
                tracing::warn!(
                    pr_url,
    ...
    let diff_lines = match branch_verifier
    ...
    if min_review_changed_lines > 0 && diff_lines < min_review_changed_lines {
