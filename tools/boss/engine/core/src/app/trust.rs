...
impl ServerState {
    ...
    /// session's tmux pane pid as the canonical trust root. When that
    ...
    /// (or directly under the app shell) does not. That distinction
    /// is enough to keep workers out of `BossOnly` even with an
    /// unregistered Boss pid.
