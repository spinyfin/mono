...
impl ServerState {
    ...
    /// process. It must never see a `Live` or `LiveProcessNoRegistry` pane,
    /// since two-pass-confirming either of those and retiring it would kill a
    /// worker that might still be doing real work.
    ///
    /// **Test-only today.** This was the automated husk sweep's candidate
    /// set; [`crate::husk_pane_sweep`] now enumerates the tmux server's own
    /// inventory instead (see its module docs for why resource identity beats
    /// app-RPC inventory there), which left this with no production caller.
    /// Marked `#[cfg(test)]` rather than left in the shipped build as dead
    /// code — the same treatment
    /// [`ServerState::pop_pending_probe`](crate::app::ServerState) gets for
    /// the same reason. The classification it filters on is still exercised
    /// through [`Self::list_hosted_pane_statuses`], which every caller uses.
    #[cfg(test)]
    pub async fn list_husk_panes(&self) -> Result<Vec<crate::protocol::HostedPaneEntry>, RetirePaneError> {
        Ok(self
