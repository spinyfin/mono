//! Foreign-bucket takeover: when a row is blocked on some other watcher's
//! reason but the live probe reports CONFLICTING, conflict_watch takes over a
//! lower-priority bucket (ci_failure) and declines a higher-priority one
//! (dependency).
