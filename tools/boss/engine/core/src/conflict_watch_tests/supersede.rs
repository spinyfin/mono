//! The pre-flight staleness check on an existing `pending` crz: superseding
//! it when the head SHA moved, when the base advanced too, or when the linked
//! revision went terminal — and the two cases that must NOT supersede (live
//! revision at the same head; a create-time gate refusal, which abandons).
