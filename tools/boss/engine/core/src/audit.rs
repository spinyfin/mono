...
mod tests {
    ...
    /// resolves to this test process's isolated root (`TEST_TMPDIR` under
    /// `bazel test`, via `boss_log_files::default_state_root()` — see
    /// `boss_log_files::is_test_process`). `TEST_STATE_ROOT` is a
    /// first-writer-wins OnceLock that sibling tests in this binary may
    /// populate, so the path is stable on every call regardless of which
    /// test in this binary calls it first. Other tests elsewhere in this
    /// binary (e.g. `coordinator_tmux`'s reset tests) append their own,
    /// differently-shaped records to that same shared file concurrently,
    /// so this test locates its own record by the `engine_version: "test"`
    /// marker below — a value no production code path ever sets — rather
