...
/// On-disk file shape. The established settings remain booleans (or, for
/// `workers.tmux_hosting`, a deliberately typed pool set), plus a nested
/// `[text]` table for [`TEXT_REGISTRY`] overrides. The nested table keeps
/// the existing flat shape byte-for-byte backward compatible — an
