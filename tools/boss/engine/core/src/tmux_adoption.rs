...
/// Run the startup invocation of [`run_adoption_pass`].
///
/// Kept as a named entry point so the startup ordering in `app/server.rs`
/// remains explicit; periodic callers use [`run_adoption_pass`] directly.
...
/// `worker_readoption`; a non-terminal row missed by the broad query is sent
/// through the same adoption routine rather than being reaped.
#[allow(clippy::too_many_arguments)]
