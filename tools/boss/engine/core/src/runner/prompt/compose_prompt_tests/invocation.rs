...
#[test]
fn run_done_seam_on_worker_signal_seam_off_teaches_blocked_marker_not_verb() {
    ...
    assert!(
        ...
        "worker_signal seam off: run_done directive must fall back to the [blocked] marker:\n{prompt}",
    );
    assert!(
        prompt.contains("Emit a `[blocked] reason=\"...\"` marker alongside it"),
        "worker_signal seam off: the blocked outcome bullet must name the marker, not the verb:\n{prompt}",
    );
}
