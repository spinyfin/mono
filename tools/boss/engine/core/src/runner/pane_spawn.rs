...
///
/// The directory always holds thin `boss` and `cube` launchers. Each `exec`s
...
fn ensure_worker_bin_dir(settings_dir: &Path, workspace_path: &Path) -> Option<PathBuf> {
    ...
    match boss_engine_worker_bin::write_cube_launcher(&dir, cube_resolved.as_deref()) {
        ...
        Err(err) => {
            ...
            // derived-PR compose wrapper generated by an older engine.
            // A failed write means that stale wrapper was NOT cleared, so a bare
