...
async fn run_one_pass_filtered(
    ...
    for work_item_id in candidates {
        ...
        // Deliberately does NOT `continue` on `Ok(true)` the way this used
        // to: that early exit is exactly what made a parked row invisible —
        // it skipped past the churn read below before that code ever ran,
        // so a row that was BOTH parked and churning never reached the
        // mutating bounce that would have surfaced either condition. Instead
        // this carries `is_deliberately_parked` forward through the churn
        // read, the pause gate, and the two liveness guards, and only the
        // combined decision after those guards (below) decides whether to
        // bounce. `Err` still fails closed exactly as before: an unreadable
        // park state is not a licence to put a second worker on the row, and
        // there is nothing useful to combine it with.
        let is_deliberately_parked =
