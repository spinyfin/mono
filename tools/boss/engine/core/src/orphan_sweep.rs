...
async fn run_one_pass_filtered(
    ...
    for work_item_id in candidates {
        ...
        // Do not continue on a positive park read: churn evaluation and the
        // halted-state bounce must still run. Decide after both liveness
        // guards; dispatch capacity and pause govern only minting executions.
        // An unreadable park state fails closed.
