...
mod tests {
    ...
    fn app_or_boss_rejects_worker_descendant_outside_app_subtree() {
        ...
        // app_pid is set to i32::MAX (never a real PID) so the trust
        // subtree check always fails — unlike pid 1 (init/launchd) which
        // IS an ancestor of every process in bazel's sandbox.
        let server_state = server_state_with_app_pid(libc::pid_t::MAX);
        let self_pid = std::process::id() as libc::pid_t;
        server_state
            .worker_registry
            .register(self_pid, "fake-run".to_owned());
...
"#;
    ...
    async fn dispatch_probe_reply_emits_probe_replied_after_followup_stop() {
        ...
        // can resolve the path. In production BOSS_RUN_ID carries execution.id
        // (exec_*), not run.id (run_*), so all probe/slot operations use
        // execution.id as the run key.
        server_state
            .work_db
