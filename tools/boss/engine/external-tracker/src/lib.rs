...
//!
//! The [`ExternalTracker`] trait is the seam between Boss's work-item
//! taxonomy and any upstream tracker (GitHub Projects today; Jira, Linear
//! later). All tracker-specific logic lives in a sub-module; the reconciler
//! only touches the types and trait defined here.
//!
//! The reconciler itself lives in `boss-engine`, which depends on this
//! crate. Nothing here may depend on `boss-engine` in return.
