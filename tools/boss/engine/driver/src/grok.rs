...
// The catalog selection below must be refreshed when xAI changes its default:
// update engine_default / model_for_reasoning /
...
static GROK_DESCRIPTOR: DriverDescriptor = DriverDescriptor {
    ...
    model_menu: ModelMenu {
        // Current authenticated `grok models` default (1.0.5, 2026-08-17).
        // Step-5 fall-through and every classified row resolve here.
...
mod tests {
    ...
    #[test]
    fn grok_model_menu_uses_current_catalog_three_rung_effort() {
        let driver = GrokDriver::default();
        let menu = &driver.descriptor().model_menu;
        // Live grok 1.0.0 / grok-4.6 accepts only low|medium|high. Large and
        ...
        // Both reasoning modes use the current catalog selection.
