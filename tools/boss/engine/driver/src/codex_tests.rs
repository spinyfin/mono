...
#[test]
fn reviewer_has_no_os_sandbox_or_relocated_cwd() {
    assert_eq!(codex_sandbox_for_worker_kind(WorkerKind::Reviewer, false), None);
    assert_eq!(codex_sandbox_for_worker_kind(WorkerKind::Reviewer, true), None);
    assert!(codex_sandbox_extra_args(WorkerKind::Reviewer, false).is_empty());

    let plan = CodexDriver::default().spawn_invocation(spawn_request("gpt-5.6-terra", "run-review-no-sandbox"));
    assert!(
        !plan.command.contains("--sandbox") && !plan.command.contains("--cd"),
        "reviewer must receive neither an OS sandbox nor a relocated cwd: {}",
        plan.command
    );
    let merged =
