...
pub fn resolve_change_plan(
    ...
    // For PushToBranch we use origin/<default_branch> so that shallow-clone
    // deepening fetches from the remote and select_base gets a fresh ref
    // (matching what the legacy checks.sh did with `git fetch origin main`).
