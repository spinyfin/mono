...
This is enforced by the built-in check `checkleft/no-check-level-file-scoping`, which inspects every checkleft check implementation's source (both built-in Rust checks and WASM guest checks) and fails the build if the check's top-level config struct declares a field named from a denylist covering both bespoke words (`paths`, `path`, `path_globs`, `include_globs`, `file_globs`, `files`, `only`, `targets`, `scope`, `globs`) and the two live framework spellings (`include`, `applies_to`) — a check parsing the framework's own word for a check-level scope is re-implementing the framework's job under a name that reads as legitimate, which is _harder_ to catch in review than a novel one.
