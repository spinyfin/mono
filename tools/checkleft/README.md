...
| `--all`                   | Scan the entire repository regardless of what changed. Manual use only — catching and fixing pre-existing violations that per-diff runs would miss. Never run `--all` in the per-PR pipeline; the only sanctioned automatic use is the dedicated integrity pipeline that sweeps for pre-existing violations separately from per-PR checks. |
