...
| `1`  | Checks ran and at least one `Error`-severity finding was reported, **or** checkleft failed for any other reason (bad config, git failure, etc.). Note this code is not exclusive to check findings.                                                                                                                                                                                                                                                                              |
| `3`  | checkleft could not gather the inputs needed to run: it could not determine what changed — a missing merge base (root commit, unrelated histories, an unresolvable `--base-ref`), a detached HEAD with no accessible parent commit, or a shallow clone whose history never reaches the base even after deepening — or the GitHub API timed out while retrieving a PR description. This is **never** treated as "no changes"; it always fails loudly instead of silently passing. |
