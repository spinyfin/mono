...
(oxfmt's / Prettier's native ignore), tracked as a flunge chore. That
...
to the tree at design time. Later amendments that re-verify a row's current
behaviour against a specific commit cite that commit's sha inline next to the
claim, rather than relying on this preamble's "design time" framing.

| #   | Mechanism                                             | Where                                                                                                                                                                                                                                                                       | Scope / granularity       | Path coordinate                                           | Check kinds it covers                                                                                                                                                                                                                                                            | Limits                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `applies_to` positive globs                           | `src/external/declarative/executor.rs:280` (`select_files`); per-repo override `src/external/declarative/resolve.rs:280` (`override_applies_to`); component-mode override `src/external/runtime.rs` (`narrow_by_applies_to`, called from `execute`/`execute_with_progress`) | per-check                 | repo-root-relative                                        | declarative (definition-side list + per-repo override) and component (per-repo override only — honoured as of `mono#2554`; a check _package manifest_ still cannot declare `applies_to` for a component, enforced by `src/external/mod.rs:444-447`, `reject_declarative_fields`) | Positive-only — cannot _subtract_ a path. Per-repo override **replaces** the list entirely (declarative) or is the entire selection (component, which has no definition-side list to fall back to), so you cannot "keep the definition's set but remove three files". Zero-match handling now also matches across both paths: an override that selects none of the files it's checked against is a clean, empty result for either kind (not a failure), whether that's none of _this run's changeset_ or none _anywhere in the repo_ — a guard check deliberately scoped ahead of time to a file type the repo does not yet contain must stay green, not fail. A **malformed** override (a non-list value, an empty list, a blank string, or invalid glob syntax) still fails loudly; that distinction is unchanged. Component-mode behaviour in this row (host-side narrowing, zero-match never an error) verified against `mono#2554`. |
...
- **Matcher mismatch.** `applies_to` compiles to a `globset::GlobSet`, which is
  ...
  all of that.
- **Doesn't unify.** At design time, `applies_to` was declarative-only;
  programmatic and component checks had no `applies_to`, so negation-in-
  `applies_to` would still have left them needing a separate mechanism — the
  exact fragmentation this design removes. That specific gap has since been
  closed for component checks (`mono#2554`: a per-repo `applies_to` override in
  a component check's `CHECKS.yaml` `config:` is now honoured, narrowing the
  changeset the same way it does on the declarative path — see row 1 of the
  inventory table above, including its zero-match note; component-mode
  behaviour verified against `mono#2554`). Programmatic (built-in Rust) checks still have no
  `applies_to`. This argument's substance is unaffected either way: negation
  belongs in `exclude`, not folded into `applies_to`, regardless of how many
  check kinds `applies_to` itself reaches — the matcher-mismatch and
  override-replace arguments above hold independently of that coverage
  question.
...
positive(C, f)  := f is selected by C's positive rule
                   (declarative: applies_to / its override globset;
                    component: its applies_to override globset if the
                    CHECKS.yaml config sets one, else the non-deleted
                    changed files in scope (its "intrinsic target set");
                    programmatic: C's intrinsic target set — by default
                    "the non-deleted changed files in scope", no applies_to)
...
- **`forbidden-imports-deps` per-rule selectors stay.** Its per-_rule_
  ...
  future work.
- **Replace the `.prettierignore` stopgap (a flunge chore).** The framework now owns
...
**7. Migration: replace the `.prettierignore` stopgap (a flunge chore) + mono root**
