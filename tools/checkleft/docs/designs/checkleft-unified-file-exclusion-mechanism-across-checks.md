...
| --- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `applies_to` positive globs                           | `src/external/declarative/executor.rs:280` (`select_files`); per-repo override `src/external/declarative/resolve.rs:280` (`override_applies_to`); component-mode override `src/external/runtime.rs` (`narrow_by_applies_to`, called from `execute`/`execute_with_progress`) | per-check                 | repo-root-relative                                        | declarative (definition-side list + per-repo override) and component (per-repo override only — honoured as of `mono#2554`; a check _package manifest_ still cannot declare `applies_to` for a component, enforced by `src/external/mod.rs:444-447`, `reject_declarative_fields`) | Positive-only — cannot _subtract_ a path. Per-repo override **replaces** the list entirely (declarative) or is the entire selection (component, which has no definition-side list to fall back to), so you cannot "keep the definition's set but remove three files".                    |
...
- **Doesn't unify.** At design time, `applies_to` was declarative-only;
  ...
  inventory table above). Programmatic (built-in Rust) checks still have no
